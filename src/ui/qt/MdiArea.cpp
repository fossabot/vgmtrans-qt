...
void MdiArea::closeButtonClicked() { qDebug() << "CLOSE BUTTON CLICKED"; }

QTabBar *MdiArea::getTabBar() {
  QList<QTabBar *> tabBarList = findChildren<QTabBar *>();
  QTabBar *tabBar = tabBarList.at(0);
  //    QList<QAbstractButton*> buttonList =
  //    tabBar->findChildren<QAbstractButton*>() ;
  //
  //    QStyleOptionTab opt;
  //    QRect rect = style()->subElementRect(QStyle::SE_TabBarTabRightButton,
  //    &opt, this); qDebug() << "TABBAR RIGHT: " << rect.left() << rect;

  //    qDebug() << "buttonList: " << buttonList.count();
  //    for (auto *button : buttonList ) {
  //        qDebug() << "button position: " << button->pos() << "  typeid: " <<
  //        typeid(button).name(); button->move(0, 0); button->setGeometry(10,
  //        10, 50, 50);
  //    }
