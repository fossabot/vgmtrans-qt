...
HexView::HexView(VGMFile *file, QWidget *parent)
    : QAbstractScrollArea(parent), vgmfile(file), mLinesPerScreen(0) {
...
void HexView::paintEvent(QPaintEvent *event) {
  ...
  for (int line = firstLine; line < lastLine; ++line) {
    ...
    if (lineOffset >= vgmfile->dwOffset + vgmfile->unLength) break;
    ...
    for (int i = 0; i < nCount; i++) {
      ...
      painter.drawText(horzPadding + ((10 + i * 3) * charWidth), y,
                       3 * charWidth, mLineHeight, Qt::AlignCenter,
                       QString(" %1 ").arg(b[i], 2, 16, zeroChar).toUpper());
    }
