...
VGMFileItemModel::VGMFileItemModel(VGMFile *file)
    : QAbstractItemModel(), vgmfile(file) {}

QModelIndex VGMFileItemModel::index(int row, int column,
  ...
  return createIndex(
      row, column, this->vgmfile->localitems[row - vgmfile->containers.size()]);
  //    QModelIndex()
...
QModelIndex VGMFileItemModel::parent(const QModelIndex &index) const {
  ...
  //
  //    VGMItem* childItem =
  //    static_cast<LayersModelItem*>(index.internalPointer()); VGMItem*
  //    parentItem = childItem->GetItemFromOffset()
}
int VGMFileItemModel::rowCount(const QModelIndex &parent) const {
  ...
  if (!parent.isValid()) {
    ...
    for (int i = 0; i < vgmfile->containers.size(); i++) {
      ...
      //            vgmfile->containers[i]->size(); qDebug() <<
      //            "containers->localitems->size() : " <<
      //            vgmfile->containers[i]
    }

    return vgmfile->containers.size() + vgmfile->localitems.size();
  }
  VGMContainerItem *item =
      static_cast<VGMContainerItem *>(parent.internalPointer());
  return item->containers.size() + item->localitems.size();
  //    return
  //    static_cast<VGMContainerItem*>(parent.internalPointer())->localitems.size();
}

int VGMFileItemModel::columnCount(const QModelIndex &parent) const { return 1; }
