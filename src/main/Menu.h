...
/*#define DECLARE_MENU_CALLS(menuclass, origclass)
   ...
   \
        public:
   \
        virtual vector<const char*>* GetMenuItemNames() {return
   menu.GetMenuItemNames();}	\ virtual bool CallMenuItem(VGMItem* item, int
   menuItemNum){ return menu.CallMenuItem(item, menuItemNum); }
*/
...
#define MENU_ITEM(origclass, func, menutext) \
  Menu<T>::AddMenuItem(&origclass::func, menutext);
#define END_MENU() \
  }                \
  }                \
  ;
...
#define DECLARE_MENU(origclass) \
  origclass::origclassMenu<origclass> origclass::menu;
...
{
public:
        MenuItem( bool (VGMItem::*functionPtr)(void), const char* theName,
uint8_t theFlag = 0) : func(functionPtr), name(theName), flag(theFlag) {}
        ~MenuItem() {}
...
class Menu {
 public:
  ...
  void AddMenuItem(bool (T::*funcPtr)(void), const wchar_t *name,
                   uint8_t flag = 0) {
