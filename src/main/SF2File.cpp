...
SF2InfoListChunk::SF2InfoListChunk(string name) : LISTChunk("INFO") {
  ...
  AddChildChunk(
      new SF2StringChunk("ISFT", string("VGMTrans " + string(VERSION))));
}
...
SF2File::SF2File(SynthFile *synthfile) : RiffFile(synthfile->name, "sfbk") {
  ...
  // Concatanate all of the samples together and add the result to the smpl
  // chunk data
  ...
  for (size_t i = 0; i < numWaves; i++) {
    ...
    smplCk->size +=
        wave->dataSize +
        (46 * 2);  // plus the 46 padding samples required by sf2 spec
  }
  ...
  for (size_t i = 0; i < numInstrs; i++) {
    ...
    memcpy(presetHdr.achPresetName, instr->name.c_str(),
           min((unsigned long)instr->name.length(), (unsigned long)20));
    ...
    memcpy(phdrCk->data + (i * sizeof(sfPresetHeader)), &presetHdr,
  ...
  memcpy(phdrCk->data + (numInstrs * sizeof(sfPresetHeader)), &presetHdr,
         sizeof(sfPresetHeader));
  ...
  for (size_t i = 0; i < numInstrs; i++) {
    ...
    memcpy(pbagCk->data + (i * sizeof(sfPresetBag)), &presetBag,
           sizeof(sfPresetBag));
  }
  //  add terminal sfPresetBag
  sfPresetBag presetBag;
  ...
  memcpy(pbagCk->data + (numInstrs * sizeof(sfPresetBag)), &presetBag,
         sizeof(sfPresetBag));
  ...
  pgenCk->size = (uint32_t)(
      (synthfile->vInstrs.size() * sizeof(sfGenList) * ITEMS_IN_PGEN) +
      sizeof(sfGenList));
  ...
  for (size_t i = 0; i < numInstrs; i++) {
    ...
    for (size_t j = 0; j < numRgns; j++) {
      ...
      memcpy(ibagCk->data + (rgnCounter++ * sizeof(sfInstBag)), &instBag,
             sizeof(sfInstBag));
    }
  }
  ...
  igenCk->size = (uint32_t)((numRgns * sizeof(sfInstGenList) * 11) +
  ...
  for (size_t i = 0; i < numInstrs; i++) {
    ...
    for (size_t j = 0; j < numRgns; j++) {
      ...
      instGenList.genAmount.shAmount =
          (int16_t)(rgn->sampinfo->attenuation * 10);
      memcpy(igenCk->data + dataPtr, &instGenList, sizeof(sfInstGenList));
      dataPtr += sizeof(sfInstGenList);
      ...
      instGenList.genAmount.shAmount =
          (rgn->art->attack_time == 0)
              ? -32768
              : roundi(SecondsToTimecents(rgn->art->attack_time));
      memcpy(igenCk->data + dataPtr, &instGenList, sizeof(sfInstGenList));
      dataPtr += sizeof(sfInstGenList);
      ...
      instGenList.genAmount.shAmount =
          (rgn->art->decay_time == 0)
              ? -32768
              : roundi(SecondsToTimecents(rgn->art->decay_time));
      memcpy(igenCk->data + dataPtr, &instGenList, sizeof(sfInstGenList));
      dataPtr += sizeof(sfInstGenList);

      // sustainVolEnv
      instGenList.sfGenOper = sustainVolEnv;
      if (rgn->art->sustain_lev > 100.0) rgn->art->sustain_lev = 100.0;
      ...
      instGenList.genAmount.shAmount =
          (rgn->art->release_time == 0)
              ? -32768
              : roundi(SecondsToTimecents(rgn->art->release_time));
      memcpy(igenCk->data + dataPtr, &instGenList, sizeof(sfInstGenList));
      dataPtr += sizeof(sfInstGenList);
  ...
  for (size_t i = 0; i < numSamps; i++) {
    ...
    memcpy(samp.achSampleName, wave->name.c_str(),
           min((unsigned long)wave->name.length(), (unsigned long)20));
    samp.dwStart = sampOffset;
    samp.dwEnd = samp.dwStart + (wave->dataSize / sizeof(uint16_t));
    sampOffset =
        samp.dwEnd + 46;  // plus the 46 padding samples required by sf2 spec

    // Search through all regions for an associated sampInfo structure with this
    // sample
    ...
    for (size_t j = 0; j < numInstrs; j++) {
      ...
      if (sampInfo != NULL) break;
    }
    //  If we didn't find a rgn association, then it should be in the SynthWave
    //  structure.
    if (sampInfo == NULL) sampInfo = wave->sampinfo;
