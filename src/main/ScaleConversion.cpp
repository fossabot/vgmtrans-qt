...
// A lot of games use a simple linear amplitude decay/release for their
// envelope. In other words, the envelope level drops at a constant rate (say
// from 0xFFFF to 0 (cps2) ), and to get the attenuation we multiply by this
...
// the a linear decay in decibels - not amplitude - for their decay/release
// slopes. So if you were to graph it, the SF2/DLS attenuation over time graph
// would be a simple line.
...
// decay/release rates if we simply plug in a time value from 0 atten to full
// atten from a linear amplitude game.

// My approach at the moment is to calculate the time it takes to get to half
// volume
// and then use that value accordingly with the SF2/DLS decay time.  In other
// words Take the second graph, find where y = 10db, and the draw a line from
// the origin through it to get your DLS/SF2 decay/release line (the actual
// output value is time where y = 100db for sf2 or 96db for DLS, SynthFile class
// uses 100db).

// This next function converts seconds to full attenuation in a linear amplitude
// decay scale and approximates the time to full attenuation in a linear DB
// decay scale.
...
uint8_t Convert7bitPercentVolValToStdMidiVal(uint8_t percentVal) {
  // MIDI uses the following formula for db attenuation on velocity/volume
  // values. (see http://www.midi.org/techspecs/gmguide2.pdf pg9 or dls1 spec
  // page 14)
  //
  //   20*log10(x^2/127^2)
  // = 40*log10(x/127)
  ...
  //
  // So if a game uses this formula with volume values ranging from 0-127 like
  // MIDI, we would get the following chart:
  //   CC#7  Amplitude
  //   --------------
  //    127      0db
  //     96   -2.4db
  ...
  //      0  -infinity
  //
  // This doesn't offer much dynamic range, as -10db is perceived as
  // half-volume.
  //
  // To convert a range 0-127 value using the second formula to the MIDI
  // formula, solve for y using the following equation:
  ...
  // (Also stated in GM guidelines page 9
  ...
  // The result is val = 127*10^(-0.025*db).  So, by plugging in the original
  // val into  the original scale equation, we get a db that we can plug into that
  // second equation to get  the new val.

  return ConvertPercentAmpToStdMidiVal(percentVal / 127.0);
}

// Takes a percentage amplitude value - that is one using a -20*log10(percent)
// scale for db attenuation and converts it to a standard midi value that uses
// -40*log10(x/127) for db attenuation
...
double ConvertLogScaleValToAtten(double percent) {
  if (percent == 0) return 100.0;  // assume 0 is -100.0db attenuation
  double atten = 20 * log10(percent) * 2;
  return min(-atten, 100.0);
}

// Convert a percent of volume value to it's attenuation in decibels.
//  ex: ConvertPercentVolToAttenDB(0.5) returns -6.02db = half perceived
//  loudness
...
// Convert a percent of volume value to it's attenuation in decibels.
//  ex: ConvertPercentVolToAttenDB_SF2(0.5) returns -(-6.02db) = half perceived
//  loudness
...
uint8_t ConvertLinearPercentPanValToStdMidiVal(double percent,
...
uint8_t Convert7bitLinearPercentPanValToStdMidiVal(uint8_t percentVal,
                                                   double *ptrVolumeScale) {
  // how to calculate volume balance from 7 bit pan depends on each music
  // engines the method below is one of the common method, but it's not always
  // correct
  if (percentVal == 127) {
    percentVal++;
  }
  return ConvertLinearPercentPanValToStdMidiVal(percentVal / 128.0,
                                                ptrVolumeScale);
}

// Convert midi pan to L/R volume balance
void ConvertStdMidiPanToVolumeBalance(uint8_t midiPan, double &percentLeft,
                                      double &percentRight) {
...
uint8_t ConvertVolumeBalanceToStdMidiPan(double percentLeft,
                                         double percentRight,
                                         double *ptrVolumeScale) {
  uint8_t midiPan;
  if (percentRight == 0) {
  ...
  if (ptrVolumeScale != NULL) {
    ...
    *ptrVolumeScale =
        (percentLeft + percentRight) / (volumeLeftMidi + volumeRightMidi);
  }
