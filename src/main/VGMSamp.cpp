...
VGMSamp::VGMSamp(VGMSampColl *sampColl, uint32_t offset, uint32_t length,
                 uint32_t dataOffset, uint32_t dataLen, uint8_t nChannels,
                 uint16_t theBPS, uint32_t theRate, wstring theName)
  ...
  name = sampName.data();  // I would do this in the initialization list, but
                           // VGMItem() constructor is called before sampName is
                           // initialized,
...
double VGMSamp::GetCompressionRatio() { return 1.0; }
  ...
  switch (waveType) {
    ...
    case WT_PCM8:
      ...
      for (unsigned int i = 0; i < dataLength;
        ...
        buf[i] ^= 0x80;  // For no good reason, the WAV standard has PCM8
...
bool VGMSamp::OnSaveAsWav() {
  wstring filepath =
      pRoot->UI_GetSaveFilePath(ConvertToSafeFileName(name), L"wav");
  if (filepath.length() != 0) return SaveAsWav(filepath);
  return false;
}
...
bool VGMSamp::SaveAsWav(const std::wstring &filepath) {
  ...
  PushTypeOnVect<uint32_t>(
      waveBuf, 0x24 + ((bufSize + 1) & ~1) + (hasLoop ? 0x50 : 0));  // size

  // WriteLIST(waveBuf, 0x43564157, bufSize+24);			//write "WAVE"
  // list
  ...
  waveBuf.insert(waveBuf.end(), uncompSampBuf,
                 uncompSampBuf + bufSize);  // Write the sample
  if (bufSize % 2) waveBuf.push_back(0);
  ...
  if (hasLoop) {
    ...
    uint32_t loopLenInSamp = (loop.loopLengthMeasure == LM_BYTES)
                                 ? (uint32_t)((loopLength * compressionRatio) /
                                              origFormatBytesPerSamp)
                                 : loopLength;
  ...
  return pRoot->UI_WriteBufferToFile(filepath, &waveBuf[0],
                                     (uint32_t)waveBuf.size());
}
