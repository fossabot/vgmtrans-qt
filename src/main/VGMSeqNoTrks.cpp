...
void VGMSeqNoTrks::ResetVars() {
  midiTracks.clear();  // no need to delete the contents, that happens when the
                       // midi is deleted
...
bool VGMSeqNoTrks::LoadMain() {
  ...
  if (length() == 0) {
    ...
    //		length() = (aEvents.back()->dwOffset + aEvents.back()->unLength) -
    //offset();			//length == to the end of the last event
  }
...
bool VGMSeqNoTrks::LoadEvents(long stopTime) {
  ResetVars();
  if (bAlwaysWriteInitialTempo) AddTempoBPMNoItem(initialTempoBPM);
  ...
  if (bAlwaysWriteInitialPitchBendRange)
    for (int i = 0; i < 16; i++) {
      channel = i;
      AddPitchBendRangeNoItem(initialPitchBendRangeSemiTones,
                              initialPitchBendRangeCents);
    }
...
// checks whether or not we have already created the given number of MidiTracks.
// If not, it appends the extra tracks. doesn't ever need to be called directly
// by format code, since SetCurMidiTrack does so automatically.
void VGMSeqNoTrks::TryExpandMidiTracks(uint32_t numTracks) {
  if (VGMSeq::readMode != READMODE_CONVERT_TO_MIDI) return;
