...
bool VGMRoot::OpenRawFile(const wstring &filename) {
  ...
  // if the file was set up properly, apply loaders, scan it, and add it to our
  // list if it contains vgmfiles
  return SetupNewRawFile(newRawFile);
}

// creates a virtual file, a RawFile that was data was created manually,
// not actually opened from the filesystem.  Used, for example, when
// decompressing the contents of PSF2 files
bool VGMRoot::CreateVirtFile(uint8_t *databuf, uint32_t fileSize,
  ...
  VirtFile *newVirtFile = new VirtFile(databuf, fileSize, filename.c_str(),
...
bool VGMRoot::SetupNewRawFile(RawFile *newRawFile) {
  ...
  if (newRawFile->processFlags & PF_USESCANNERS) {
    ...
    if (lScanners) {
      ...
      for (list<VGMScanner *>::iterator iter = lScanners->begin();
           iter != lScanners->end(); iter++)
...
bool VGMRoot::CloseRawFile(RawFile *targFile) {
  if (targFile == NULL) return false;
  vector<RawFile *>::iterator iter =
      find(vRawFile.begin(), vRawFile.end(), targFile);
...
// Adds a a VGMFile to the interface.  The UI_AddVGMFile function will handle
// the interface-specific stuff
...
void VGMRoot::RemoveVGMFile(VGMFile *targFile, bool bRemoveFromRaw) {
  ...
  if (fmt) fmt->OnCloseFile(targFile);

  vector<VGMFile *>::iterator iter =
      find(vVGMFile.begin(), vVGMFile.end(), targFile);
  if (iter != vVGMFile.end())
    ...
    pRoot->AddLogItem(
        new LogItem(std::wstring(L"Error: trying to delete a VGMFile which "
                    ...
                    LOG_LEVEL_DEBUG, L"Root"));
  if (bRemoveFromRaw) targFile->rawfile->RemoveContainedVGMFile(targFile);
...
void VGMRoot::RemoveVGMColl(VGMColl *targColl) {
  ...
  vector<VGMColl *>::iterator iter =
      find(vVGMColl.begin(), vVGMColl.end(), targColl);
...
bool VGMRoot::UI_WriteBufferToFile(const wstring &filepath, uint8_t *buf,
                                   uint32_t size) {
#if _MSC_VER < 1400  // if we're not using VC8, and the new STL that supports
                     // widechar filenames in ofstream...
