...
void SeqSlider<TNumber>::write(uint32_t time) const {
  if (changesAt(time)) writeMessage(get(time));
}
...
VolSlider::VolSlider(SeqTrack *track, uint32_t time, uint32_t duration,
...
MasterVolSlider::MasterVolSlider(SeqTrack *track, uint32_t time,
                                 uint32_t duration, uint8_t initialValue,
                                 uint8_t targetValue)
    : SeqSlider(track, time, duration, initialValue, targetValue) {}
...
ExpressionSlider::ExpressionSlider(SeqTrack *track, uint32_t time,
                                   uint32_t duration, uint8_t initialValue,
                                   uint8_t targetValue)
    : SeqSlider(track, time, duration, initialValue, targetValue) {}
...
PanSlider::PanSlider(SeqTrack *track, uint32_t time, uint32_t duration,
                     uint8_t initialValue, uint8_t targetValue)
    : SeqSlider(track, time, duration, initialValue, targetValue) {}
