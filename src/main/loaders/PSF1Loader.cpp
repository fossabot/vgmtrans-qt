...
const wchar_t *PSF1Loader::psf_read_exe(RawFile *file, unsigned char *exebuffer,
  ...
  if (fileSize >= 0x10000000) return L"PSF too large - likely corrupt";

  PSFFile psf;
  if (!psf.Load(file)) return psf.GetError();

  // Now we get into the stuff related to recursive psflib loading.
  // the actual size of the header is 0x800, but we only need the first 0x20 for
  // the text section offset/size
  DataSeg *psfExeHeadSeg;
  if (!psf.ReadExeDataSeg(psfExeHeadSeg, 0x20, 0)) return psf.GetError();
  ...
  if (textSectionStart + textSectionSize > 0x200000)
    return L"Text section start and/or size values are corrupt in PSX-EXE "
           L"header.";
...
const wchar_t *PSF1Loader::load_psf_libs(PSFFile &psf, RawFile *file,
