...
const wchar_t *GSFLoader::psf_read_exe(RawFile *file, unsigned char *&exebuffer,
  ...
  if (!psf.ReadExeDataSeg(gsfExeHeadSeg, 0x0c, 0)) return psf.GetError();
  ...
  if (gsfRomRegion != 0x08000000)
    return L"GSF ROM offset points to unsupported region. (multi-boot GSF is "
           L"not supported yet)";
  if (gsfRomStart + gsfRomSize > exebuffersize ||
...
const wchar_t *GSFLoader::load_psf_libs(PSFFile &psf, RawFile *file,
