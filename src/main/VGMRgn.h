...
class VGMRgn : public VGMContainerItem {
 public:
  ...
  VGMRgn(VGMInstr *instr, uint32_t offset, uint32_t length, uint8_t keyLow,
         uint8_t keyHigh, uint8_t velLow, uint8_t velHigh, int sampNum,
         const std::wstring &name = L"Region");
  ~VGMRgn(void);
  ...
  void SetRanges(uint8_t keyLow, uint8_t keyHigh, uint8_t velLow = 0,
                 uint8_t velHigh = 0x7F);
  void SetUnityKey(uint8_t unityNote);
  void SetSampNum(uint8_t sampNumber);
  void SetLoopInfo(int theLoopStatus, uint32_t theLoopStart,
                   uint32_t theLoopLength);
  void SetADSR(long attack_time, uint16_t atk_transform, long decay_time,
               long sustain_lev, uint16_t rls_transform, long release_time);

  void AddGeneralItem(uint32_t offset, uint32_t length,
                      const std::wstring &name);
  void AddUnknown(uint32_t offset, uint32_t length);
  void SetFineTune(int16_t relativePitchCents) {
    fineTune = relativePitchCents;
  }
  ...
  void AddFineTune(int16_t relativePitchCents, uint32_t offset,
                   uint32_t length = 1);
  ...
  // void SetWaveLinkInfo(uint16_t options, uint16_t phaseGroup, uint32_t
  // theChannel, uint32_t theTableIndex);
 ...
 public:
  ...
  int sampNum;
  uint32_t sampOffset;  // optional value. If a sample offset is provided, then
                        // find the sample number based on this offset.
  // This is an absolute offset into the SampColl.  It's not necessarily
  // relative to the beginning of the actual sample data, unless the sample data
  // begins at offset 0.
  // int sampCollNum;	//optional value. for formats that use multiple
  // sampColls and reference samples base 0 for each sampColl (NDS, for instance)
  VGMSampColl *sampCollPtr;

  // long attenuation;
  double volume;  // as percentage of full volume.  This will be converted to to
                  // an attenuation when we convert to a SynthFile
  ...
  double sustain_time;   // in seconds (we don't support positive rate here, as
                        // is possible on psx)
