...
class VGMRoot {
 ...
 public:
  ...
  bool CreateVirtFile(uint8_t *databuf, uint32_t fileSize,
  ...
  virtual void UI_AddItem(VGMItem *item, VGMItem *parent,
                          const std::wstring &itemName, void *UI_specific) {}
  virtual void UI_AddItemSet(void *UI_specific, std::vector<ItemSet> *itemset) {
  }
  virtual std::wstring UI_GetOpenFilePath(
      const std::wstring &suggestedFilename = L"",
      const std::wstring &extension = L"") = 0;
  virtual std::wstring UI_GetSaveFilePath(
      const std::wstring &suggestedFilename,
      const std::wstring &extension = L"") = 0;
  virtual std::wstring UI_GetSaveDirPath(
      const std::wstring &suggestedDir = L"") = 0;
  virtual bool UI_WriteBufferToFile(const std::wstring &filepath, uint8_t *buf,
                                    uint32_t size);
