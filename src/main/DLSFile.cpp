...
//{
//	name += (char)0x00;
//	if (name.size() % 2)						//if the size of
//the name string is odd 		name += (char)0x00;
////add another null byte
//}
...
DLSInstr *DLSFile::AddInstr(unsigned long bank, unsigned long instrNum) {
  aInstrs.insert(aInstrs.end(),
                 new DLSInstr(bank, instrNum, "Unnamed Instrument"));
  return aInstrs.back();
}
...
DLSWave *DLSFile::AddWave(uint16_t formatTag, uint16_t channels,
                          int samplesPerSec, int aveBytesPerSec,
                          uint16_t blockAlign, uint16_t bitsPerSample,
                          uint32_t waveDataSize, unsigned char *waveData,
                          string name) {
  aWaves.insert(
      aWaves.end(),
      new DLSWave(formatTag, channels, samplesPerSec, aveBytesPerSec,
...
uint32_t DLSFile::GetSize(void) {
  ...
  size +=
      COLH_SIZE;  // COLH chunk (collection chunk - tells how many instruments)
  size += LIST_HDR_SIZE;  //"lins" list (list of instruments - contains all the
                          //"ins " lists)
  ...
  size +=
      (uint32_t)aWaves.size() * sizeof(uint32_t);  // each wave gets a poolcue
  size +=
      LIST_HDR_SIZE;  //"wvpl" list (wave pool - contains all the "wave" lists)
...
int DLSFile::WriteDLSToBuffer(vector<uint8_t> &buf) {
  ...
  PushTypeOnVect<uint32_t>(
      buf, (uint32_t)aInstrs.size());  // cInstruments - number of instruments
  ...
  for (uint32_t i = 0; i < (uint32_t)aWaves.size(); i++) {
    PushTypeOnVect<uint32_t>(buf,
                             theDWORD);  // write the poolcue for each sample
  ...
  theDWORD =
      12 + (uint32_t)name.size();  //"INFO" + "INAM" + size + the string size
...
// I should probably make this function part of a parent class for both Midi and
// DLS file
bool DLSFile::SaveDLSFile(const std::wstring &filepath) {
  vector<uint8_t> dlsBuf;
  WriteDLSToBuffer(dlsBuf);
  return pRoot->UI_WriteBufferToFile(filepath, &dlsBuf[0],
                                     (uint32_t)dlsBuf.size());
}
...
DLSInstr::DLSInstr(uint32_t bank, uint32_t instrument, string instrName,
                   vector<DLSRgn *> listRgns)
    : ulBank(bank), ulInstrument(instrument), name(instrName) {
  RiffFile::AlignName(name);
  aRgns = listRgns;
}

DLSInstr::~DLSInstr() { DeleteVect(aRgns); }
...
uint32_t DLSRgn::GetSize(void) {
  ...
  if (Wsmp) size += Wsmp->GetSize();
  size += WLNK_SIZE;
  if (Art) size += Art->GetSize();
  return size;
}

void DLSRgn::Write(vector<uint8_t> &buf) {
  RiffFile::WriteLIST(buf, 0x72676E32,
                      (uint32_t)(GetSize() - 8));  // write "rgn2" list
  ...
  if (Wsmp) Wsmp->Write(buf);  // write the "wsmp" chunk
...
void DLSRgn::SetRanges(uint16_t keyLow, uint16_t keyHigh, uint16_t velLow,
                       uint16_t velHigh) {
  usKeyLow = keyLow;
  usKeyHigh = keyHigh;
  usVelLow = velLow;
...
void DLSRgn::SetWaveLinkInfo(uint16_t options, uint16_t phaseGroup,
                             uint32_t theChannel, uint32_t theTableIndex) {
  fusOptions = options;
...
DLSArt::~DLSArt() { DeleteVect(aConnBlocks); }
...
void DLSArt::Write(vector<uint8_t> &buf) {
  ...
  PushTypeOnVect<uint32_t>(buf, 8);                              // cbSize
  PushTypeOnVect<uint32_t>(buf,
                           (uint32_t)aConnBlocks.size());  // cConnectionBlocks
  for (uint32_t i = 0; i < aConnBlocks.size(); i++)
    aConnBlocks[i]->Write(buf);  // each connection block
}

void DLSArt::AddADSR(long attack_time, uint16_t atk_transform, long decay_time,
                     long sustain_lev, long release_time,
  ...
  aConnBlocks.insert(aConnBlocks.end(),
                     new ConnectionBlock(CONN_SRC_NONE, CONN_SRC_NONE,
...
uint32_t DLSWsmp::GetSize(void) {
  ...
  if (cSampleLoops) size += 16;  // plus the loop info
  return size;
}
...
void DLSWsmp::Write(vector<uint8_t> &buf) {
  ...
  PushTypeOnVect<uint32_t>(buf, GetSize() - 8);  // size
  PushTypeOnVect<uint32_t>(
      buf, 20);  // cbSize (size of structure without loop record)
...
void DLSWsmp::SetLoopInfo(Loop &loop, VGMSamp *samp) {
  ...
  // If the sample loops, but the loop length is 0, then assume the length
  // should extend to the end of the sample.
  ...
  // if the val is a raw offset of the original format, multiply it by the
  // compression ratio
  ulLoopStart = (loop.loopStartMeasure == LM_BYTES)
                    ? (uint32_t)((loop.loopStart * compressionRatio) /
                                 origFormatBytesPerSamp)
                    : loop.loopStart;

  ulLoopLength = (loop.loopLengthMeasure == LM_BYTES)
                     ? (uint32_t)((loop.loopLength * compressionRatio) /
                                  origFormatBytesPerSamp)
                     : loop.loopLength;
}

void DLSWsmp::SetPitchInfo(uint16_t unityNote, short fineTune,
                           long attenuation) {
...
uint32_t DLSWave::GetSize() {
  ...
  size +=
      this->GetSampleSize();  // dataSize;			    //size of
                              // sample data
...
void DLSWave::Write(vector<uint8_t> &buf) {
  ...
  PushTypeOnVect<uint16_t>(
      buf, 0);  // cbSize DLS2 specific.  I don't know anything else
