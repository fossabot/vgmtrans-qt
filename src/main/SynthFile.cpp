...
//  **********************************************************************************
//  SynthFile - An intermediate class to lay out all of the the data necessary
//  for Coll conversion
//				to DLS or SF2 formats.  Currently, the structure is
//identical to DLS.
//  **********************************************************************************
...
SynthWave *SynthFile::AddWave(uint16_t formatTag, uint16_t channels,
  ...
  vWaves.insert(
      vWaves.end(),
      new SynthWave(formatTag, channels, samplesPerSec, aveBytesPerSec,
...
SynthInstr::~SynthInstr() { DeleteVect(vRgns); }
...
void SynthRgn::SetRanges(uint16_t keyLow, uint16_t keyHigh, uint16_t velLow,
                         uint16_t velHigh) {
  usKeyLow = keyLow;
  usKeyHigh = keyHigh;
  usVelLow = velLow;
...
void SynthRgn::SetWaveLinkInfo(uint16_t options, uint16_t phaseGroup,
...
void SynthArt::AddADSR(double attack, Transform atk_transform, double decay,
                       double sustain_level, double sustain, double release,
...
void SynthArt::AddPan(double thePan) { this->pan = thePan; }
...
void SynthSampInfo::SetPitchInfo(uint16_t unityNote, short fineTune,
                                 double atten) {
