...
void VGMColl::RemoveFileAssocs() {
  if (seq) seq->RemoveCollAssoc(this);
...
const wstring *VGMColl::GetName(void) const { return &name; }

void VGMColl::SetName(const wstring *newName) { name = *newName; }

VGMSeq *VGMColl::GetSeq(void) { return seq; }

void VGMColl::UseSeq(VGMSeq *theSeq) {
  if (theSeq != NULL) theSeq->AddCollAssoc(this);
...
void VGMColl::UnpackSampColl(DLSFile &dls, VGMSampColl *sampColl,
  ...
  for (size_t i = 0; i < nSamples; i++) {
    ...
    if (samp->ulUncompressedSize)
      ...
      bufSize = (uint32_t)ceil((double)samp->dataLength *
                               samp->GetCompressionRatio());
    uint8_t *uncompSampBuf = new uint8_t[bufSize];  // create a new memory space
...
void VGMColl::UnpackSampColl(SynthFile &synthfile, VGMSampColl *sampColl,
  ...
  for (size_t i = 0; i < nSamples; i++) {
    ...
    if (samp->loop.loopStatus == -1) return;
...
bool VGMColl::MainDLSCreation(DLSFile &dls) {
  ...
  if (!instrsets.size() /*|| !sampcolls.size()*/) return false;
  ...
  if (finalSamps.size() == 0) return false;
      ...
      for (uint32_t j = 0; j < nRgns; j++) {
        ...
        if (rgn->sampOffset != -1) {
          ...
          if (!bFoundIt) {
            ...
            pRoot->AddLogItem(new LogItem(std::wstring(message.c_str()),
        ...
        for (unsigned int k = 0; k < finalSampColls.size(); k++) {
          if (finalSampColls[k] == sampColl) sampCollNum = k;
        }
        ...
        //effectively) 	for (uint32_t i=0; i < finalSampColls.size(); i++)
        //	{
        ...
        //{									//then sampNum
        //represents the sample number in the specific sample collection
        //	for (int k=0; k < rgn->sampCollNum; k++)
        //		realSampNum += finalSampColls[k]->samples.size();
        ////so now we add all previous sample collection samples to the value to
        //get the real (absolute) sampNum
        //}
        ...
        if (realUnityKey == -1) realUnityKey = 0x3C;
        ...
        if (rgn->volume == -1 && samp->volume == -1)
          ...
          realAttenuation = (long)(-(ConvertLogScaleValToAtten(samp->volume) *
                                     DLS_DECIBEL_UNIT * 10));
        else
          realAttenuation = (long)(-(ConvertLogScaleValToAtten(rgn->volume) *
                                     DLS_DECIBEL_UNIT * 10));

        long convAttack =
            (long)roundi(SecondsToTimecents(rgn->attack_time) * 65536);
        long convDecay =
            (long)roundi(SecondsToTimecents(rgn->decay_time) * 65536);
        long convSustainLev;
        ...
        long convRelease =
            (long)roundi(SecondsToTimecents(rgn->release_time) * 65536);
    ...
    for (size_t i = 0; i < nInstrs; i++) {
      ...
      for (uint32_t j = 0; j < nRgns; j++) {
        ...
        for (uint32_t i = 0; i < finalSampColls.size(); i++) {
          if (finalSampColls[i] == sampColl) sampCollNum = i;
        }
        ...
        newArt->AddADSR(rgn->attack_time, (Transform)rgn->attack_transform,
                        rgn->decay_time, sustainLevAttenDb, rgn->sustain_time,
                        rgn->release_time, (Transform)rgn->release_transform);
...
bool VGMColl::OnSaveAllDLS() {
  ...
  if (dirpath.length() != 0) {
    DLSFile dlsfile;
    wstring filepath =
        dirpath + L"\\" + ConvertToSafeFileName(this->name) + L".dls";
...
bool VGMColl::OnSaveAllSF2() {
  ...
  if (dirpath.length() != 0) {
    wstring filepath =
        dirpath + L"\\" + ConvertToSafeFileName(this->name) + L".sf2";
