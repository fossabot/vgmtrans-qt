...
MidiFile::~MidiFile(void) { DeleteVect<MidiTrack>(aTracks); }
...
MidiTrack *MidiFile::InsertTrack(uint32_t trackNum) {
  if (trackNum + 1 > aTracks.size()) aTracks.resize(trackNum + 1, NULL);
...
int MidiFile::GetMidiTrackIndex(MidiTrack *midiTrack) {
  std::vector<MidiTrack *>::iterator it =
      std::find(aTracks.begin(), aTracks.end(), midiTrack);
  if (it != aTracks.end()) {
...
void MidiFile::SetPPQN(uint16_t ppqn) { this->ppqn = ppqn; }

uint32_t MidiFile::GetPPQN() { return ppqn; }
...
bool MidiFile::SaveMidiFile(const std::wstring &filepath) {
  ...
  return pRoot->UI_WriteBufferToFile(filepath, &midiBuf[0],
                                     (uint32_t)midiBuf.size());
}
...
MidiTrack::~MidiTrack(void) { DeleteVect<MidiEvent>(aEvents); }
...
void MidiTrack::WriteTrack(vector<uint8_t> &buf) {
  ...
  for (size_t i = 0; i < numEvents; i++)
    time = finalEvents[i]->WriteEvent(
        buf, time);  // write all events into the buffer

  uint32_t trackSize = (uint32_t)(
      buf.size() - 8);  //-8 for MTrk and size that shouldn't be accounted for
...
uint32_t MidiTrack::GetDelta(void) { return DeltaTime; }

void MidiTrack::SetDelta(uint32_t NewDelta) { DeltaTime = NewDelta; }

void MidiTrack::AddDelta(uint32_t AddDelta) { DeltaTime += AddDelta; }

void MidiTrack::SubtractDelta(uint32_t SubtractDelta) {
  DeltaTime -= SubtractDelta;
}

void MidiTrack::ResetDelta(void) { DeltaTime = 0; }
...
void MidiTrack::InsertNoteOn(uint8_t channel, int8_t key, int8_t vel,
                             uint32_t absTime) {
...
void MidiTrack::AddNoteByDur(uint8_t channel, int8_t key, int8_t vel,
                             uint32_t duration) {
  PurgePrevNoteOffs(GetDelta());
  aEvents.push_back(
      new NoteEvent(this, channel, GetDelta(), true, key, vel));  // add note on
  NoteEvent *prevDurNoteOff =
      new NoteEvent(this, channel, GetDelta() + duration, false, key);
...
void MidiTrack::AddNoteByDur_TriAce(uint8_t channel, int8_t key, int8_t vel,
                                    uint32_t duration) {
  ...
  for (size_t curEvt = 0; curEvt < nNumEvents; curEvt++) {
    ...
    // If so, we're restarting an already played note. In the case of the TriAce
    // driver that means, that we resume the note, so we need to move the
    // NoteOff event.

    // Note: In previous TriAce drivers (like MegaDrive and SNES versions),
    //       a Note gets extended by a Note On event at the tick where another
    //       note expires. Valkyrie Profile: 225 Fragments of the Heart
    //       confirms, that this is NOT the case in the PS1 version.
  ...
  if (ContNote == NULL) {
    ...
    aEvents.push_back(
        new NoteEvent(this, channel, CurDelta, true, key, vel));  // add note on
    NoteEvent *prevDurNoteOff =
        new NoteEvent(this, channel, CurDelta + duration, false, key);
    prevDurNoteOffs.push_back(prevDurNoteOff);
    aEvents.push_back(prevDurNoteOff);  // add note off at end of dur
  } else {
    ContNote->AbsTime =
        CurDelta +
        duration;  // fix DeltaTime of the already inserted NoteOff event
  }
}

void MidiTrack::InsertNoteByDur(uint8_t channel, int8_t key, int8_t vel,
                                uint32_t duration, uint32_t absTime) {
  PurgePrevNoteOffs(max(GetDelta(), absTime));
  aEvents.push_back(
      new NoteEvent(this, channel, absTime, true, key, vel));  // add note on
  NoteEvent *prevDurNoteOff =
      new NoteEvent(this, channel, absTime + duration, false, key);
  prevDurNoteOffs.push_back(prevDurNoteOff);
  aEvents.push_back(prevDurNoteOff);  // add note off at end of dur
}

void MidiTrack::PurgePrevNoteOffs() { prevDurNoteOffs.clear(); }
...
{
        MidiEvent* newEvent = new VolMarkerEvent(this, channel, GetDelta(),
vol); aEvents.push_back(newEvent);
}

void MidiTrack::InsertVolMarker(uint8_t channel, uint8_t vol, uint32_t absTime,
int8_t priority)
{
...
void MidiTrack::AddControllerEvent(uint8_t channel, uint8_t controllerNum,
                                   uint8_t theDataByte) {
  aEvents.push_back(new ControllerEvent(this, channel, GetDelta(),
                                        controllerNum, theDataByte));
}

void MidiTrack::InsertControllerEvent(uint8_t channel, uint8_t controllerNum,
                                      uint8_t theDataByte, uint32_t absTime) {
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, controllerNum, theDataByte));
}
...
void MidiTrack::InsertMasterVol(uint8_t channel, uint8_t mastVol,
                                uint32_t absTime) {
...
void MidiTrack::InsertExpression(uint8_t channel, uint8_t expression,
                                 uint32_t absTime) {
...
void MidiTrack::InsertSustain(uint8_t channel, uint8_t depth,
                              uint32_t absTime) {
...
void MidiTrack::InsertPortamentoTime(uint8_t channel, uint8_t time,
                                     uint32_t absTime) {
...
void MidiTrack::InsertReverb(uint8_t channel, uint8_t reverb,
                             uint32_t absTime) {
...
void MidiTrack::InsertModulation(uint8_t channel, uint8_t depth,
                                 uint32_t absTime) {
...
void MidiTrack::InsertPitchBend(uint8_t channel, int16_t bend,
                                uint32_t absTime) {
  aEvents.push_back(new PitchBendEvent(this, channel, absTime, bend));
}

void MidiTrack::AddPitchBendRange(uint8_t channel, uint8_t semitones,
                                  uint8_t cents) {
  InsertPitchBendRange(channel, semitones, cents, GetDelta());
}

void MidiTrack::InsertPitchBendRange(uint8_t channel, uint8_t semitones,
                                     uint8_t cents, uint32_t absTime) {
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 101, 0, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 100, 0, PRIORITY_HIGHER - 1));
  aEvents.push_back(new ControllerEvent(this, channel, absTime, 6, semitones,
                                        PRIORITY_HIGHER - 1));
  aEvents.push_back(new ControllerEvent(this, channel, absTime, 38, cents,
                                        PRIORITY_HIGHER - 1));
}
...
void MidiTrack::InsertFineTuning(uint8_t channel, uint8_t msb, uint8_t lsb,
                                 uint32_t absTime) {
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 101, 0, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 100, 1, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 6, msb, PRIORITY_HIGHER - 1));
  aEvents.push_back(new ControllerEvent(this, channel, absTime, 38, lsb,
                                        PRIORITY_HIGHER - 1));
}
...
void MidiTrack::InsertFineTuning(uint8_t channel, double cents,
                                 uint32_t absTime) {
...
void MidiTrack::InsertCoarseTuning(uint8_t channel, uint8_t msb, uint8_t lsb,
                                   uint32_t absTime) {
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 101, 0, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 100, 2, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 6, msb, PRIORITY_HIGHER - 1));
  aEvents.push_back(new ControllerEvent(this, channel, absTime, 38, lsb,
                                        PRIORITY_HIGHER - 1));
...
void MidiTrack::InsertCoarseTuning(uint8_t channel, double semitones,
                                   uint32_t absTime) {
...
void MidiTrack::AddModulationDepthRange(uint8_t channel, uint8_t msb,
                                        uint8_t lsb) {
  InsertModulationDepthRange(channel, msb, lsb, GetDelta());
}

void MidiTrack::InsertModulationDepthRange(uint8_t channel, uint8_t msb,
                                           uint8_t lsb, uint32_t absTime) {
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 101, 0, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 100, 5, PRIORITY_HIGHER - 1));
  aEvents.push_back(
      new ControllerEvent(this, channel, absTime, 6, msb, PRIORITY_HIGHER - 1));
  aEvents.push_back(new ControllerEvent(this, channel, absTime, 38, lsb,
                                        PRIORITY_HIGHER - 1));
...
void MidiTrack::InsertModulationDepthRange(uint8_t channel, double semitones,
                                           uint32_t absTime) {
  semitones = max(-64.0, min(64.0, semitones));
  int16_t midiTuning = min((int)(128 * semitones + 0.5), 8191) + 8192;
  InsertFineTuning(channel, midiTuning >> 7, midiTuning & 0x7f, absTime);
...
void MidiTrack::InsertBankSelect(uint8_t channel, uint8_t bank,
                                 uint32_t absTime) {
...
void MidiTrack::AddTimeSig(uint8_t numer, uint8_t denom,
                           uint8_t ticksPerQuarter) {
  aEvents.push_back(
      new TimeSigEvent(this, GetDelta(), numer, denom, ticksPerQuarter));
  // bAddedTimeSig = true;
}

void MidiTrack::InsertTimeSig(uint8_t numer, uint8_t denom,
                              uint8_t ticksPerQuarter, uint32_t absTime) {
  aEvents.push_back(
      new TimeSigEvent(this, absTime, numer, denom, ticksPerQuarter));
  // bAddedTimeSig = true;
}
...
void MidiTrack::AddMarker(uint8_t channel, const string &markername,
                          uint8_t databyte1, uint8_t databyte2,
                          int8_t priority) {
  aEvents.push_back(new MarkerEvent(this, channel, GetDelta(), markername,
                                    databyte1, databyte2, priority));
}
...
MidiEvent::MidiEvent(MidiTrack *thePrntTrk, uint32_t absoluteTime,
                     uint8_t theChannel, int8_t thePriority)
    : prntTrk(thePrntTrk),
...
uint32_t MidiEvent::WriteSysexEvent(vector<uint8_t> &buf, uint32_t time,
...
uint32_t MidiEvent::WriteMetaEvent(vector<uint8_t> &buf, uint32_t time,
                                   uint8_t metaType, uint8_t *data,
                                   size_t dataSize) {
  WriteVarLength(buf, AbsTime - time);
  buf.push_back(0xFF);
...
uint32_t MidiEvent::WriteMetaTextEvent(vector<uint8_t> &buf, uint32_t time,
                                       uint8_t metaType, wstring wstr) {
  string str = wstring2string(wstr);
  return WriteMetaEvent(buf, time, metaType, (uint8_t *)str.c_str(),
                        str.length());
}
...
NoteEvent::NoteEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
                     bool bnoteDown, uint8_t theKey, uint8_t theVel)
...
uint32_t NoteEvent::WriteEvent(vector<uint8_t> &buf, uint32_t time) {
  ...
  if (prntTrk->bMonophonic && !bNoteDown)
    ...
    prntTrk->prevKey =
        key + ((channel == 9) ? 0 : prntTrk->parentSeq->globalTranspose);
    buf.push_back(prntTrk->prevKey);
  }
  // buf.push_back(key);
...
// DurNoteEvent::DurNoteEvent(MidiTrack* prntTrk, uint8_t channel, uint32_t
// absoluteTime, uint8_t theKey, uint8_t theVel, uint32_t theDur) :
//MidiEvent(prntTrk, absoluteTime, channel, PRIORITY_LOWER), key(theKey),
//vel(theVel), duration(theDur)
//{
//}
/*
...
{
        prntTrk->aEvents.push_back(new NoteEvent(prntTrk, channel, AbsTime,
true, key, vel));	//add note on prntTrk->aEvents.push_back(new
NoteEvent(prntTrk, channel, AbsTime+duration, false, key, vel));  //add note off
at end of dur
}*/

// uint32_t DurNoteEvent::WriteEvent(vector<uint8_t> & buf, uint32_t time)
// //we do note use WriteEvent on DurNoteEvents... this is what PrepareWrite is
// for, to create NoteEvents in substitute
//{
//	return false;
//}
...
/*VolEvent::VolEvent(MidiTrack *prntTrk, uint8_t channel, uint32_t absoluteTime,
uint8_t theVol, int8_t thePriority) : ControllerEvent(prntTrk, channel,
absoluteTime, 7), vol(theVol)
{
}
...
MastVolEvent::MastVolEvent(MidiTrack *prntTrk, uint8_t channel,
                           uint32_t absoluteTime, uint8_t theMastVol)
    : MidiEvent(prntTrk, absoluteTime, channel, PRIORITY_HIGHER),
      mastVol(theMastVol) {}

uint32_t MastVolEvent::WriteEvent(vector<uint8_t> &buf, uint32_t time) {
  uint8_t data[7] = {
      0x7F, 0x7F, 0x04, 0x01, /*LSB*/ 0, (uint8_t)(mastVol & 0x7F), 0xF7};
...
ControllerEvent::ControllerEvent(MidiTrack *prntTrk, uint8_t channel,
                                 uint32_t absoluteTime, uint8_t controllerNum,
                                 uint8_t theDataByte, int8_t thePriority)
...
ProgChangeEvent::ProgChangeEvent(MidiTrack *prntTrk, uint8_t channel,
                                 uint32_t absoluteTime, uint8_t progNum)
    : MidiEvent(prntTrk, absoluteTime, channel, PRIORITY_HIGH),
      programNum(progNum) {}
...
PitchBendEvent::PitchBendEvent(MidiTrack *prntTrk, uint8_t channel,
                               uint32_t absoluteTime, int16_t bendAmt)
    : MidiEvent(prntTrk, absoluteTime, channel, PRIORITY_MIDDLE),
      bend(bendAmt) {}
...
TempoEvent::TempoEvent(MidiTrack *prntTrk, uint32_t absoluteTime,
...
TimeSigEvent::TimeSigEvent(MidiTrack *prntTrk, uint32_t absoluteTime,
...
uint32_t TimeSigEvent::WriteEvent(vector<uint8_t> &buf, uint32_t time) {
  // denom is expressed in power of 2... so if we have 6/8 time.  it's 6 = 2^x
  // ==  ln6 / ln2
  uint8_t data[4] = {
      numer, (uint8_t)(log((double)denom) / 0.69314718055994530941723212145818),
      ticksPerQuarter, 8};
...
TextEvent::TextEvent(MidiTrack *prntTrk, uint32_t absoluteTime,
...
SeqNameEvent::SeqNameEvent(MidiTrack *prntTrk, uint32_t absoluteTime,
                           const std::wstring &wstr)
    : MidiEvent(prntTrk, absoluteTime, 0, PRIORITY_LOWEST), text(wstr) {}
...
TrackNameEvent::TrackNameEvent(MidiTrack *prntTrk, uint32_t absoluteTime,
                               const std::wstring &wstr)
    : MidiEvent(prntTrk, absoluteTime, 0, PRIORITY_LOWEST), text(wstr) {}
...
uint32_t GSResetEvent::WriteEvent(vector<uint8_t> &buf, uint32_t time) {
  uint8_t data[10] = {0x41, 0x10, 0x42, 0x12, 0x40,
...
GlobalTransposeEvent::GlobalTransposeEvent(MidiTrack *prntTrk,
