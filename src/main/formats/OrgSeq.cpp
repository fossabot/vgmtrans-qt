...
bool OrgSeq::GetHeaderInfo(void) {
  ...
  uint32_t notesSoFar =
      0;  // this must be used to determine the length of the entire seq

  for (int i = 0; i < 16; i++) {
    if (GetShort(dwOffset + 0x16 + i * 6))  // if there are notes in this track
    {
      nNumTracks++;  // well then, we might as well say it exists
      OrgTrack *newOrgTrack =
          new OrgTrack(this, dwOffset + 0x12 + 16 * 6 + notesSoFar * 8,
...
OrgTrack::OrgTrack(OrgSeq *parentFile, long offset, long length,
                   uint8_t realTrk)
    : SeqTrack(parentFile, offset, length), realTrkNum(realTrk) {}

bool OrgTrack::LoadTrack(uint32_t trackNum, uint32_t stopOffset,
                         long stopDelta) {
  ...
  if (channel == 10) AddProgramChange(0, 0, waveNum);

  bInLoop = false;
  curOffset = dwOffset;  // start at beginning of track
  curNote = 0;
  for (uint16_t i = 0; i < numNotes; i++) ReadEvent();
...
bool OrgTrack::ReadEvent(void) {
  ...
  uint8_t vel =
      GetByte(curOffset + (numNotes - curNote) * 4 + numNotes * 2 + curNote) /
      2;
  uint8_t dur =
      GetByte(curOffset + (numNotes - curNote) * 4 + numNotes + curNote);
  if (key == 0xFF) key = prevKey;
  uint32_t absTime = GetWord(curOffset);

  InsertNoteByDur(curOffset, 4, key, vel, dur, absTime);
  uint8_t pan =
      GetByte(curOffset + (numNotes - curNote) * 4 + numNotes * 3 + curNote);
  if (pan == 0xFF)
    pan = prevPan;
  else
    pan = (uint8_t)(
  ...
  // if (newPan > 0x7F)	//sometimes the value is 0xFF, even though the range
  // would seem to be 0-C according to the org editor
  //	newPan = 64;	//in this case, set it to the center position, i can't
  //distinguish it from center on hearing tests
  if (pan != prevPan) {
    InsertPan(curOffset + (numNotes - curNote) * 4 + numNotes * 3 + curNote, 1,
              pan, absTime);
    prevPan = pan;
  }
