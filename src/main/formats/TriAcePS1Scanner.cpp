...
void TriAcePS1Scanner::SearchForSLZSeq(RawFile *file) {
  ...
  for (uint32_t i = 0; i + 0x40 < nFileLength; i++) {
    ...
    if (sig1 != 0x534C5A)  // "SLZ" in ASCII
      continue;
    if (mode > 0x03) continue;  // only SLZ v0-3 is supported
    // Note: SLZ v2 is used by a few tracks in Valkyrie Profile.

    uint16_t headerBytes = file->GetShort(i + 0x11);

    if (headerBytes !=
        0xFFFF)  // First two bytes of the sequence is always 0xFFFF
      continue;

    uint32_t size1 =
        file->GetWord(i + 4);  // unknown.  compressed size or something
    uint32_t size2 = file->GetWord(i + 8);  // uncompressed file size (size of
                                            // resulting file after
                                            // decompression)
    uint32_t size3 =
        file->GetWord(i + 12);  // unknown compressed file size or something

    if (size1 > 0x30000 || size2 > 0x30000 ||
        size3 > 0x30000)  // sanity check.  Sequences won't be > 0x30000 bytes
      continue;
    if (size1 > size2 || size3 > size2)  // the compressed file size ought to be
                                         // smaller than the decompressed size
      continue;

    TriAcePS1Seq *seq = TriAceSLZDecompress(file, i);
    if (!seq) return;
    if (i < 4 || file->GetWord(i - 4) == 0) return;

    // The size of the compressed file is 4 bytes behind the SLZ sig.
    // We need this to calculate the offset of the InstrSet, which comes
    // immediately after the SLZ'd sequence.
...
void TriAcePS1Scanner::SearchForInstrSet(
    RawFile *file, vector<TriAcePS1InstrSet *> &instrsets) {
  uint32_t nFileLength = file->size();
  for (uint32_t i = 4; i + 0x800 < nFileLength; i++) {
    uint8_t precedingByte = file->GetByte(i + 3);
    if (precedingByte != 0) continue;

    // The 32 byte value at offset 8 seems to be bank #.  In practice, i don't
    // think it is ever > 1
    if (file->GetWord(i + 8) > 0xFF) continue;

    uint16_t instrSectSize = file->GetShort(i + 4);
    // The instrSectSize should be more than the size of one instrdata block and
    // not insanely large
    if (instrSectSize <= 0x20 || instrSectSize > 0x4000) continue;
    // Make sure the size doesn't point beyond the end of the file
    if (i + instrSectSize > file->size() - 0x100) continue;

    uint32_t instrSetSize = file->GetWord(i);
    if (instrSetSize < 0x1000) continue;
    // The entire InstrSet size must be larger than the instrdata region, of
    // course
    if (instrSetSize <= instrSectSize) continue;

    // First 0x10 bytes of sample section should be 0s
    if (file->GetWord(i + instrSectSize) != 0 ||
        file->GetWord(i + instrSectSize + 4) != 0 ||
        file->GetWord(i + instrSectSize + 8) != 0 ||
        file->GetWord(i + instrSectSize + 12) != 0)
      continue;
    // Last 4 bytes of Instr section should be 0xFFFFFFFF
    if (file->GetWord(i + instrSectSize - 4) != 0xFFFFFFFF) continue;
...
// file is RawFile containing the compressed seq.  cfOff is the compressed file
// offset.
TriAcePS1Seq *TriAcePS1Scanner::TriAceSLZDecompress(RawFile *file,
                                                    uint32_t cfOff) {
  uint8_t cmode = file->GetByte(cfOff + 3);    // compression mode
  uint32_t cfSize = file->GetWord(cfOff + 4);  // compressed file size
  uint32_t ufSize = file->GetWord(cfOff + 8);  // uncompressed file size (size
                                               // of resulting file after
                                               // decompression)
  uint32_t blockSize = file->GetWord(
      cfOff +
      12);  // size of entire compressed block (slightly larger than cfSize)

  if (ufSize == 0) ufSize = DEFAULT_UFSIZE;
    ...
    while (ufOff < ufSize && !bDone) {
      ...
      for (int i = 0; (i < bits) && (ufOff < ufSize); i++, cFlags >>= 1) {
        if (cFlags & 1)  // uncompressed byte, just copy it over
        {
          ...
          if (bits == 16) uf[ufOff++] = file->GetByte(cfOff++);
        } else  // compressed section
        {
          ...
          if (cmode == 2 && byte2 >= 0xF0) {
            ...
            for (; bytesToRead > 0; bytesToRead--) uf[ufOff++] = byte1;
          } else {
            uint32_t backPtr = ufOff - (((byte2 & 0x0F) << 8) + byte1);
            bytesToRead = (byte2 >> 4) + 3;

            for (; bytesToRead > 0; bytesToRead--) uf[ufOff++] = uf[backPtr++];
          }
        }
      }
    }
  ...
  if (ufOff > ufSize)
    pRoot->AddLogItem(new LogItem(std::wstring(L"ufOff > ufSize"),
                                  LOG_LEVEL_ERR, L"SNSFFile"));

  // If we had to use DEFAULT_UFSIZE because the uncompressed file size was not
  // given (Valkyrie Profile),  then create a new buffer of the correct size now
  // that we know it, and delete the old one.
  ...
  std::wstring name = file->tag.HasTitle()
                          ? file->tag.title
                          : RawFile::removeExtFromPath(file->GetFileName());
  VirtFile *newVirtFile = newVirtFile =
      new VirtFile(uf, ufOff, name + std::wstring(L" Sequence"),
