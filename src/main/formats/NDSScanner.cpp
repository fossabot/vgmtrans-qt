...
void NDSScanner::SearchForSDAT(RawFile *file) {
  ...
  for (uint32_t i = 0; i + 4 < nFileLength; i++) {
    if ((*file)[i] == 'S' && (*file)[i + 1] == 'D' && (*file)[i + 2] == 'A' &&
        (*file)[i + 3] == 'T' && (*file)[i + 4] == 0xFF &&
        (*file)[i + 5] == 0xFE && (*file)[i + 6] == 0 &&
...
// The following is pretty god-awful messy.  I should have created structs for
// the different blocks and loading the entire blocks at a time.
uint32_t NDSScanner::LoadFromSDAT(RawFile *file, uint32_t baseOff) {
  ...
  if (hasSYMB) {
    ...
    pBnkNamePtrList = file->GetWord(SYMBoff + 0x10) +
                      SYMBoff;  // get pointer to list of bank name pointers
    pWANamePtrList =
        file->GetWord(SYMBoff + 0x14) +
  ...
  for (uint32_t i = 0; i < nSeqs; i++) {
    ...
    wchar_t wtemp[32];
    if (hasSYMB) {
      file->GetBytes(file->GetWord(pSeqNamePtrList + 4 + i * 4) + SYMBoff, 32,
                     temp);
  ...
  for (uint32_t i = 0; i < nBnks; i++) {
    ...
    if (hasSYMB) {
      file->GetBytes(file->GetWord(pBnkNamePtrList + 4 + i * 4) + SYMBoff, 32,
                     temp);
      mbstowcs(wtemp, temp, 32);
    } else {
  ...
  for (uint32_t i = 0; i < nWAs; i++) {
    ...
    if (hasSYMB) {
      file->GetBytes(file->GetWord(pWANamePtrList + 4 + i * 4) + SYMBoff, 32,
                     temp);
      mbstowcs(wtemp, temp, 32);
    } else {
  ...
  for (uint32_t i = 0; i < nSeqInfos; i++) {
    ...
    // next bytes would be vol, cpr, ppr, and ply respectively, whatever the
    // heck those last 3 stand for
  }
...

  {
    ...
    vector<uint16_t>::iterator new_end =
        unique(vUniqueWAs.begin(), vUniqueWAs.end());

    vector<bool> valid;
    valid.resize(nWAs);
    for (vector<uint16_t>::iterator iter = vUniqueWAs.begin(); iter != new_end;
         iter++) {
      if ((*iter != (uint16_t)-1) && (*iter < valid.size())) valid[*iter] = 1;
    }

    for (uint32_t i = 0; i < nWAs; i++)
    // for (vector<uint16_t>::iterator iter = vUniqueWAs.begin(); iter !=
    // new_end; iter++)
    {
      ...
      NDSWaveArch *NewNDSwa =
          new NDSWaveArch(file, pWAFatData, fileSize, waNames[i]);
...

  {
    ...
    vector<uint16_t>::iterator new_end =
        unique(vUniqueBanks.begin(), vUniqueBanks.end());

    // for (uint32_t i=0; i<nBnks; i++)
    // for (uint32_t i=0; i<seqFileBnks.size(); i++)
    for (vector<uint16_t>::iterator iter = vUniqueBanks.begin();
         iter != new_end; iter++) {
      if (*iter >= bnkFileIDs.size() /*0x1000*/ ||
          bnkFileIDs[*iter] == (uint16_t)-1)  // > 0x1000 is idiot test for
                                              // Phoenix Wright, which had many
                                              // 0x1C80 values, as if they were
                                              // 0xFFFF
        continue;
      ...
      NDSInstrSet *NewNDSInstrSet =
          new NDSInstrSet(file, pBnkFatData, fileSize,
                          bnkNames[*iter] /*, WAs[bnkWAs[*iter][0]]*/);
      for (int i = 0; i < 4;
           i++)  // use first WA found.  Ideally, should load all WAs
      {
...

  {
    ...
    // vector<uint16_t>::iterator new_end = unique(vUniqueBanks.begin(),
    // vUniqueBanks.end());

    for (uint32_t i = 0; i < nSeqs; i++) {
      if (seqFileIDs[i] == (uint16_t)-1) continue;
      ...
      if (!NewNDSSeq->LoadVGMFile()) {
        pRoot->AddLogItem(
            new LogItem(FormatString<wstring>(
                            L"Failed to load NDSSeq at 0x%08x\n", pSeqFatData)
                            .c_str(),
                        LOG_LEVEL_ERR, L"NDSScanner"));
      }
      ...
      for (int j = 0; j < 4; j++) {
        ...
        if (WAnum != -1) coll->AddSampColl(WAs[WAnum]);
      }
      if (!coll->Load()) {
        delete coll;
      }
...
/*void NDSScanner::SearchForNDSSeq (RawFile* file)
{
        ...
        for (uint32_t i=0; i+4<nFileLength; i++)
        {
                if ((*file)[i] == 'S' && (*file)[i+1] == 'S' && (*file)[i+2] ==
'E' && (*file)[i+3] == 'Q')
                {
                        //if (file->GetShort(i+10) == 0 && file->GetShort(i+16)
== 0)
                        //{
