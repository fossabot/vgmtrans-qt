...
class NDSInstrSet : public VGMInstrSet {
 public:
  NDSInstrSet(RawFile *file, uint32_t offset, uint32_t length,
              std::wstring name =
                  L"NDS Instrument Bank" /*, VGMSampColl* sampColl = NULL*/);
...
class NDSInstr : public VGMInstr {
 public:
  NDSInstr(NDSInstrSet *instrSet, uint32_t offset, uint32_t length,
           uint32_t bank, uint32_t instrNum, uint8_t instrType);
...
class NDSSamp : public VGMSamp {
 public:
  ...
  static inline void process_nibble(unsigned char code, int &stepIndex,
                                    int &decompSample);
