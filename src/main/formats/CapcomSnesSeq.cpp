...
CapcomSnesSeq::CapcomSnesSeq(RawFile *file, CapcomSnesVersion ver,
                             uint32_t seqdataOffset, bool priorityInHeader,
                             wstring newName)
...
bool CapcomSnesSeq::GetHeaderInfo(void) {
  ...
  VGMHeader *seqHeader =
      AddHeader(dwOffset, (priorityInHeader ? 1 : 0) + MAX_TRACKS * 2,
                L"Sequence Header");
...
bool CapcomSnesSeq::GetTrackPointers(void) {
  for (int i = MAX_TRACKS - 1; i >= 0; i--) {
    ...
    if (trkOff != 0) aTracks.push_back(new CapcomSnesTrack(this, trkOff));
  }
  return true;
}
...
double CapcomSnesSeq::GetTempoInBPM() { return GetTempoInBPM(tempo); }
...
CapcomSnesTrack::CapcomSnesTrack(CapcomSnesSeq *parentFile, long offset,
                                 long length)
...
void CapcomSnesTrack::setNoteOctave(uint8_t octave) {
  noteAttributes = (noteAttributes & ~CAPCOM_SNES_MASK_NOTE_OCTAVE) |
                   (octave & CAPCOM_SNES_MASK_NOTE_OCTAVE);
}
...
bool CapcomSnesTrack::ReadEvent(void) {
  ...
  if (statusByte >= 0x20) {
    ...
    if (rest) {
      ...
      // actual music engine does it in 16 bit precision, because of tempo
      // handling.
      ...
      if (isNoteSlurred()) {
        ...
        // because the music engine thinks the note is already off (release
        // time).
        if (dur == 0) {
          dur = len << 8;
        }
      ...
      uint8_t key =
          (keyIndex - 1) + (getNoteOctave() * 12) + (isNoteOctaveUp() ? 24 : 0);
      uint8_t vel = 127;
    ...
    switch (eventType) {
      ...
      case EVENT_TOGGLE_TRIPLET:
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Toggle Triplet",
                        L"", CLR_DURNOTE, ICON_CONTROL);
        break;

      case EVENT_TOGGLE_SLUR:
        setNoteSlurred(!isNoteSlurred());
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Toggle Slur/Tie", L"", CLR_DURNOTE, ICON_CONTROL);
        break;

      case EVENT_DOTTED_NOTE_ON:
        setNoteDotted(true);
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Dotted Note On",
                        L"", CLR_DURNOTE, ICON_CONTROL);
        break;
      ...
      case EVENT_TOGGLE_OCTAVE_UP:
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Toggle 2-Octave Up", L"", CLR_DURNOTE, ICON_CONTROL);
        break;
      ...
      case EVENT_NOTE_ATTRIBUTES: {
        ...
        desc << L"Triplet: " << (isNoteTriplet() ? L"On" : L"Off") << L"  "
             << L"Slur: " << (isNoteSlurred() ? L"On" : L"Off") << L"  "
             << L"2-Octave Up: " << (isNoteOctaveUp() ? L"On" : L"Off");
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Note Attributes", desc.str().c_str(), CLR_DURNOTE,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_OCTAVE: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Octave",
                        desc.str().c_str(), CLR_DURNOTE, ICON_CONTROL);
        break;
      }
      ...
      case EVENT_PORTAMENTO_TIME: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Portamento Time", desc.str().c_str(),
                        CLR_PORTAMENTOTIME, ICON_CONTROL);
      ...
      case EVENT_REPEAT_BREAK_4: {
        ...
        desc << L"Note: { " << L"Triplet: "
             << (isNoteTriplet() ? L"On" : L"Off") << L"  " << L"Slur: "
             << (isNoteSlurred() ? L"On" : L"Off") << L"  " << L"2-Octave Up: "
             << (isNoteOctaveUp() ? L"On" : L"Off") << L" }  "
             << L"Destination: $" << std::hex << std::setfill(L'0')
        ...
        if (repeatCount[repeatSlot] == 1) {
          ...
          noteAttributes &=
              ~(CAPCOM_SNES_MASK_NOTE_OCTAVE_UP |
