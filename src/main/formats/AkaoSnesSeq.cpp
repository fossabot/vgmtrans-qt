...
AkaoSnesSeq::AkaoSnesSeq(RawFile *file, AkaoSnesVersion ver,
                         AkaoSnesMinorVersion minorVer, uint32_t seqdataOffset,
                         uint32_t addrAPURelocBase, std::wstring newName)
...
void AkaoSnesSeq::ResetVars(void) { VGMSeq::ResetVars(); }
...
void AkaoSnesSeq::LoadEventMap() {
  ...
  if (version == AKAOSNES_V1) {
    const uint8_t NOTE_DUR_TABLE_FF4[15] = {0xc0, 0x90, 0x60, 0x48, 0x40,
                                            0x30, 0x24, 0x20, 0x18, 0x10,
                                            0x0c, 0x08, 0x06, 0x04, 0x03};
    NOTE_DUR_TABLE.assign(std::begin(NOTE_DUR_TABLE_FF4),
                          std::end(NOTE_DUR_TABLE_FF4));
  } else if (version == AKAOSNES_V2 || version == AKAOSNES_V3) {
    const uint8_t NOTE_DUR_TABLE_RS1[15] = {0xc0, 0x90, 0x60, 0x40, 0x48,
                                            0x30, 0x20, 0x24, 0x18, 0x10,
                                            0x0c, 0x08, 0x06, 0x04, 0x03};
    NOTE_DUR_TABLE.assign(std::begin(NOTE_DUR_TABLE_RS1),
                          std::end(NOTE_DUR_TABLE_RS1));
  } else {  // AKAOSNES_V4
    const uint8_t NOTE_DUR_TABLE_RS2[14] = {0xc0, 0x60, 0x40, 0x48, 0x30,
                                            0x20, 0x24, 0x18, 0x10, 0x0c,
                                            0x08, 0x06, 0x04, 0x03};
    NOTE_DUR_TABLE.assign(std::begin(NOTE_DUR_TABLE_RS2),
                          std::end(NOTE_DUR_TABLE_RS2));
  }
  ...
  if (version == AKAOSNES_V1) {
    ...
    EventMap[0xf6] =
        EVENT_ECHO_VOLUME_FADE;  // No Operation in Seiken Densetsu 2
    EventMap[0xf7] =
        EVENT_ECHO_FEEDBACK_FIR;  // No Operation in Seiken Densetsu 2
...
double AkaoSnesSeq::GetTempoInBPM(uint8_t tempo) {
  if (tempo != 0 && TIMER0_FREQUENCY != 0) {
    ...
    // since tempo 0 cannot be expressed, this function returns a very small
    // value.
    return 1.0;
  }
}
...
bool AkaoSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_NOTE:  // 0x00..0xc3
    {
      ...
      uint8_t noteIndex =
          (uint8_t)(statusByte / parentSeq->NOTE_DUR_TABLE.size());
      ...
      if (noteIndex < 12) {
        ...
        if (percussion) {
          AddNoteByDur(beginOffset, curOffset - beginOffset,
                       noteIndex + AkaoSnesDrumKitRgn::KEY_BIAS - transpose,
                       vel, dur, L"Percussion Note with Duration");
        } else {
    ...
    case EVENT_VOLUME_FADE: {
      ...
      if (fadeLength != 0) {
        desc << L"Fade Length: " << (int)fadeLength << L"  Volume: "
             << (int)vol;
    ...
    case EVENT_PITCH_SLIDE_ON: {
      ...
      if (parentSeq->version == AKAOSNES_V1) {
        ...
        desc << L"Delay: " << (int)pitchSlideDelay << L"  Length: "
             << (int)pitchSlideLength << L"  Key: " << (int)pitchSlideSemitones
             << L" semitones";
      } else {  // AKAOSNES_V2
        pitchSlideSemitones = GetByte(curOffset++);
        pitchSlideDelay = GetByte(curOffset++);
    ...
    case EVENT_PITCH_SLIDE: {
      ...
      desc << L"Length: " << (int)pitchSlideLength << L"  Key: "
    ...
    case EVENT_VIBRATO_ON: {
      ...
      desc << L"Delay: " << (int)lfoDelay << L"  Rate: " << (int)lfoRate
    ...
    case EVENT_PAN_LFO_ON: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pan LFO",
                      desc.str().c_str(), CLR_MODULATION, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_PITCHMOD_ON: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Pitch Modulation On", desc.str().c_str(),
    ...
    case EVENT_PITCHMOD_OFF: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Pitch Modulation Off", desc.str().c_str(),
                      CLR_CHANGESTATE, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_TRANSPOSE_REL: {
      int8_t newTranspose = GetByte(curOffset++);
      AddTranspose(beginOffset, curOffset - beginOffset,
                   transpose + newTranspose, L"Transpose (Relative)");
      break;
    }
    ...
    case EVENT_GAIN_RELEASE: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Release Rate (GAIN)", desc.str().c_str(), CLR_ADSR,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_LOOP_END: {
      uint8_t prevLoopLevel =
          (loopLevel != 0 ? loopLevel : AKAOSNES_LOOP_LEVEL_MAX) - 1;
      if (parentSeq->version == AKAOSNES_V4) {
    ...
    case EVENT_SLUR_ON: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Slur On (No Key Off/On)", desc.str().c_str(),
                      CLR_CHANGESTATE, ICON_CONTROL);
      slur = true;
      break;
    }
    ...
    case EVENT_LEGATO_ON: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Legato On (No Key Off)", desc.str().c_str(),
                      CLR_CHANGESTATE, ICON_CONTROL);
      legato = true;
      break;
    }
    ...
    case EVENT_ONETIME_DURATION: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Duration (One-Time)", desc.str().c_str(), CLR_DURNOTE);
      break;
    }
    ...
    case EVENT_PLAY_SFX: {
      ...
      AddUnknown(beginOffset, curOffset - beginOffset, L"Play SFX",
                 desc.str().c_str());
      break;
    }
    ...
    case EVENT_ECHO_FEEDBACK_FIR: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Echo Feedback & FIR", desc.str().c_str(), CLR_REVERB,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_LOOP_BREAK: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Loop Break / Jump to Volta", desc.str().c_str(),
                      CLR_LOOP, ICON_ENDREP);

      uint8_t prevLoopLevel =
          (loopLevel != 0 ? loopLevel : AKAOSNES_LOOP_LEVEL_MAX) - 1;
      if (parentSeq->version !=
          AKAOSNES_V4) {  // AKAOSNES_V1, AKAOSNES_V2, AKAOSNES_V3
        loopIncCount[prevLoopLevel]++;
      }
    ...
    case EVENT_INC_CPU_SHARED_COUNTER: {
      AddUnknown(beginOffset, curOffset - beginOffset,
                 L"Increment CPU-Shared Counter", desc.str().c_str());
      break;
    }

    case EVENT_ZERO_CPU_SHARED_COUNTER: {
      AddUnknown(beginOffset, curOffset - beginOffset,
                 L"Zero CPU-Shared Counter", desc.str().c_str());
      break;
    }
    ...
    case EVENT_ECHO_FEEDBACK_FADE: {
      ...
      desc << L"Fade Length: " << (int)fadeLength << L"  Feedback: "
           << (int)feedback;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Echo Feedback Fade", desc.str().c_str(), CLR_REVERB,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_CPU_CONTROLED_SET_VALUE: {
      ...
      AddUnknown(beginOffset, curOffset - beginOffset,
                 L"Set Value for CPU-Controled Jump", desc.str().c_str());
      break;
    }
    ...
    case EVENT_CPU_CONTROLED_JUMP: {
      ...
      uint16_t dest = GetShortAddress(curOffset);
      curOffset += 2;
      desc << L"Arg1: " << (int)arg1 << L"  Destination: $" << std::hex
           << std::setfill(L'0') << std::setw(4) << std::uppercase << (int)dest;
    ...
    case EVENT_IGNORE_MASTER_VOLUME_BROKEN: {
      // multiply (1 << channel) / 256.0 to channel volume, instead of applying
      // master volume apparently, it is caused by wrong destination address of
      // conditional branch
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Ignore Master Volume (Broken)", desc.str().c_str(),
    ...
    case EVENT_IGNORE_MASTER_VOLUME: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Ignore Master Volume", desc.str().c_str(), CLR_VOLUME,
                      ICON_CONTROL);
      break;
    }
