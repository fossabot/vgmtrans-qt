...
QSoundSampleInfoTable::QSoundSampleInfoTable(RawFile *file, wstring &name,
                                             uint32_t offset, uint32_t length)
    : VGMMiscFile(QSoundFormat::name, file, offset, length, name) {}
...
bool QSoundSampleInfoTable::LoadMain() {
  uint32_t off = dwOffset;
  uint32_t test1 = 1, test2 = 1;
  if (unLength == 0) unLength = 0xFFFFFFFF - dwOffset;
  for (int i = 0;
       (test1 || test2) && ((test1 != 0xFFFFFFFF) || (test2 != 0xFFFFFFFF)) &&
...
QSoundInstrSet::QSoundInstrSet(RawFile *file, QSoundVer version,
                               uint32_t offset, int numInstrBanks,
...
bool QSoundInstrSet::GetHeaderInfo() { return true; }

bool QSoundInstrSet::GetInstrPointers() {
  // Load the instr_info tables.

  if (fmt_version <= VER_115) {
    // In these versions, there are no instr_info_table pointers, it's
    // hard-coded into the assembly code. There are two possible instr_info
    // banks stored next to each other with 256 entries in each, unlike higher
    // versions, where the number of instr_infos in each bank is variable and
    // less than 0x7F.
    ...
    for (uint32_t bank = 0; bank < num_instr_banks; bank++)
      for (uint32_t i = 0; i < 256; i++) {
        ...
        wstring name = ss.str();
        aInstrs.push_back(
            new QSoundInstr(this, dwOffset + i * 8 + (bank * 256 * 8), 8,
    ...
    if (fmt_version < VER_130)
      instr_info_length =
          sizeof(qs_prog_info_ver_103);  // 1.16 (Xmen vs SF) is like this

    vector<uint16_t> instr_table_ptrs;
    for (unsigned int i = 0; i < num_instr_banks; i++)
      instr_table_ptrs.push_back(
          GetShort(dwOffset + i * 2));  // get the instr table ptrs
    int totalInstrs = 0;
    for (uint32_t i = 0; i < instr_table_ptrs.size(); i++) {
      uint16_t endOffset;
      // The following is actually incorrect.  There is a max of 256 instruments
      // per bank
      ...
      int k = 0;
      for (int j = instr_table_ptrs[i]; j < endOffset;
           j += instr_info_length, k++) {
        if (GetShort(j) == 0 && GetByte(j + 2) == 0 && i != 0) break;
        std::wostringstream ss;
        ss << L"Instrument " << totalInstrs << k;
        wstring name = ss.str();
        aInstrs.push_back(new QSoundInstr(
            this, j, instr_info_length, (i * 2) + (k / 128), (k % 128), name));
      }
      totalInstrs += k;
    }
  }
...
QSoundInstr::QSoundInstr(VGMInstrSet *instrSet, uint32_t offset,
...
bool QSoundInstr::LoadInstr() {
  ...
  if (GetFormatVer() < VER_103) {
    ...
    QSoundArticTable *articTable =
        ((QSoundInstrSet *)this->parInstrSet)->articTable;
  ...
  if (rgn->sampNum >= 0x8000) rgn->sampNum -= 0x8000;

  // if the sample doesn't exist, set it to the first sample
  if (rgn->sampNum >=
      ((QSoundInstrSet *)parInstrSet)->sampInfoTable->numSamples)
    rgn->sampNum = 0;
  ...
  // The rate values are all measured from max to min, as the SF2 and DLS specs
  // call for.
  ...
  //  Why?  SF2 and DLS do not have a sustain rate.  If the decay rate here is
  //  practically unnoticeable because the sustain level is extremely high -
  //  meaning decay time is is super short - then let's set the sustain level to
  //  0 and substitute the sustain rate into the decay rate,  achieving the same
  //  effect as sustain rate.

  if (this->sustain_level >= 0x7E && this->sustain_rate > 0 &&
      this->decay_rate > 1) {
    // for a better approximation, we count the ticks to get from original Dr to
    // original Sl
  ...
  //    some articulations have Dr = 0, Sl = 0 (ex: mmatrix).  Dr 0 means the
  //    decay phase never ends and Sl value is irrelevant because the envelope
  //    stays at max, but we can't set an infinite decay phase in sf2 or dls
  if (Dr <= 1)
  ...
  if (rgn->sampNum == 0xFFFF ||
      rgn->sampNum >=
          ((QSoundInstrSet *)parInstrSet)->sampInfoTable->numSamples)
    rgn->sampNum = 0;
  rgn->SetUnityKey(((QSoundInstrSet *)parInstrSet)
                       ->sampInfoTable->infos[rgn->sampNum]
                       .unity_key);
  aRgns.push_back(rgn);
  return true;
}
...
QSoundSampColl::QSoundSampColl(RawFile *file, QSoundInstrSet *theinstrset,
                               ...
                               uint32_t offset, uint32_t length, wstring name)
...
bool QSoundSampColl::GetSampleInfo() {
  ...
  for (uint32_t i = 0; i < numSamples; i++) {
    ...
    uint32_t sampOffset = (sampInfo->bank << 16) +
                          (sampInfo->start_addr_hi << 8) +
                          sampInfo->start_addr_lo;
    int sampLength;
    ...
    if (sampLength < 0) sampLength = -sampLength;

    uint32_t loopOffset =
        ((sampInfo->bank << 16) + (sampInfo->loop_offset_hi << 8) +
         sampInfo->loop_offset_lo) -
        sampOffset;
    if (loopOffset > (uint32_t)sampLength) loopOffset = sampLength;
    if (sampLength == 0 || sampOffset > unLength) break;
    VGMSamp *newSamp = AddSamp(sampOffset, sampLength, sampOffset, sampLength,
                               1, 8, 24000, name.str());
