...
PSXSampColl::PSXSampColl(const string &format, RawFile *rawfile,
                         uint32_t offset, uint32_t length)
    : VGMSampColl(format, rawfile, offset, length) {}

PSXSampColl::PSXSampColl(const string &format, VGMInstrSet *instrset,
                         uint32_t offset, uint32_t length)
...
bool PSXSampColl::GetSampleInfo() {
  if (vagLocations.size() == 0) {
    // We scan through the sample section, and determine the offsets and size of
    // each sample  We do this by searching for series of 16 0x00 value bytes.
    // These indicate the beginning of a sample,  and they will never be found at
    // any other point within the adpcm sample data.

    uint32_t nEndOffset = dwOffset + unLength;
    if (unLength == 0) nEndOffset = GetEndOffset();
    ...
    while (i + 32 <= nEndOffset) {
      ...
      if (isSample) {
        ...
        if ((keyFlagByte & 0xF8) != 0) break;

        // if (filterRangeByte == 0 && keyFlagByte == 0)	// Breaking on
        // FFXII 309 - Eruyt Village at 61D50 of the WD
        if (GetWord(i + 16) == 0 && GetWord(i + 20) == 0 &&
            GetWord(i + 24) == 0 && GetWord(i + 28) == 0)
          break;
        ...
        PSXSamp *samp = new PSXSamp(
            this, beginOffset, i - beginOffset, beginOffset,
    ...
    for (std::vector<SizeOffsetPair>::iterator it = vagLocations.begin();
      ...
      PSXSamp *samp = new PSXSamp(
          this, dwOffset + it->offset, it->size, dwOffset + it->offset,
...
PSXSampColl *PSXSampColl::SearchForPSXADPCM(RawFile *file,
                                            const string &format) {
  const std::vector<PSXSampColl *> &sampColls =
      SearchForPSXADPCMs(file, format);
  if (sampColls.size() != 0) {
...
const std::vector<PSXSampColl *> PSXSampColl::SearchForPSXADPCMs(
    RawFile *file, const string &format) {
  ...
  for (uint32_t i = 0; i + 16 + NUM_CHUNKS_READAHEAD * 16 < nFileLength; i++) {
    ...
    if (file->GetWord(i) == 0 && file->GetWord(i + 4) == 0 &&
        file->GetWord(i + 8) == 0 && file->GetWord(i + 12) == 0) {
      bool bBad = false;
      uint32_t firstChunk = i + 16;
      uint8_t filterRangeByte = file->GetByte(firstChunk);
      uint8_t keyFlagByte = file->GetByte(firstChunk + 1);

      if (filterRangeByte == 0 && keyFlagByte == 0) continue;
      // if ((keyFlagByte & 0x04) == 0)
      //	continue;
      if ((keyFlagByte & 0xF8) != 0) continue;

      uint8_t maxRangeChange = 0;
      uint8_t maxFilterChange = 0;
      int prevRange = file->GetByte(firstChunk + 16) & 0xF;  //+16 because we're
                                                             //skipping the
                                                             //first chunk for
                                                             //uncertain reasons
      ...
      for (uint32_t j = 0; j < NUM_CHUNKS_READAHEAD; j++) {
        ...
        if ((file->GetWord(curChunk) == 0 &&
             ((file->GetWord(curChunk + 4) == 0) ||
              file->GetWord(curChunk + 8) == 0))) {
          bBad = true;
          break;
        }
        ...
        if (diff > maxRangeChange) maxRangeChange = diff;
        prevRange = range;
        int filter =
            (((int)file->GetByte(firstChunk + 16 + j * 16)) & 0xF0) >> 4;
        diff = abs(filter - prevFilter);
        if (diff > maxFilterChange) maxFilterChange = diff;
        prevFilter = filter;
      }
...
PSXSamp::PSXSamp(VGMSampColl *sampColl, uint32_t offset, uint32_t length,
                 uint32_t dataOffset, uint32_t dataLen, uint8_t nChannels,
                 uint16_t theBPS, uint32_t theRate, wstring name,
...
void PSXSamp::ConvertToStdWave(uint8_t *buf) {
  ...
  for (uint32_t k = 0; k < dataLength; k += 0x10)  // for every adpcm chunk
  {
    ...
    // this can be the loop point, but in wd, this info is stored in the
    // instrset
    ...
    // each decompressed pcm block is 52 bytes   EDIT: (wait, isn't it 56 bytes?
    // or is it 28?)
    DecompVAGBlk(uncompBuf + ((k * 28) / 16), &theBlock, &prev1, &prev2);
  }
}

uint32_t PSXSamp::GetSampleLength(RawFile *file, uint32_t offset,
