...
// INTR_FREQUENCY is the interval in seconds between updates to the vol for
// articulation. In the original software, this is done via a hardware interrupt
// timer. After calculating the number of volume updates that will be made for
// the articulation values, a calculation found through reverse-engineering the
// music driver code,  we can multiply the count by this frequency to find the
// duration of the articulation phases with exact accuracy.
...
NDSInstrSet::NDSInstrSet(RawFile *file, uint32_t offset, uint32_t length,
                         wstring name)
...
bool NDSInstrSet::GetInstrPointers() {
  ...
  for (uint32_t i = 0; i < nInstruments; i++) {
    ...
    aInstrs.push_back(
        new NDSInstr(this, pInstr + dwOffset, 0, 0, i, instrType));
...
NDSInstr::NDSInstr(NDSInstrSet *instrSet, uint32_t offset, uint32_t length,
                   uint32_t theBank, uint32_t theInstrNum, uint8_t theInstrType)
  ...
  switch (instrType) {
    ...
    case 0x10: {
      ...
      for (uint8_t i = 0; i < nRgns; i++) {
        VGMRgn *rgn =
            AddRgn(dwOffset + 2 + i * 12, 12,
                   GetShort(dwOffset + 2 + 2 + i * 12), lowKey + i, lowKey + i);
    ...
    case 0x11: {
      ...
      for (int i = 0; i < nRgns; i++) {
        VGMRgn *rgn = AddRgn(dwOffset + 8 + i * 12, 12,
...
void NDSInstr::GetArticData(VGMRgn *rgn, uint32_t offset) {
  ...
  uint8_t realAttack;
  long realSustainLev;
  const uint8_t AttackTimeTable[] = {0x00, 0x01, 0x05, 0x0E, 0x1A, 0x26, 0x33,
                                     0x3F, 0x49, 0x54, 0x5C, 0x64, 0x6D, 0x74,
                                     0x7B, 0x7F, 0x84, 0x89, 0x8F};

  const uint16_t sustainLevTable[] = {
      0xFD2D, 0xFD2E, 0xFD2F, 0xFD75, 0xFDA7, 0xFDCE, 0xFDEE, 0xFE09, 0xFE20,
      0xFE34, 0xFE46, 0xFE57, 0xFE66, 0xFE74, 0xFE81, 0xFE8D, 0xFE98, 0xFEA3,
      0xFEAD, 0xFEB6, 0xFEBF, 0xFEC7, 0xFECF, 0xFED7, 0xFEDF, 0xFEE6, 0xFEEC,
      0xFEF3, 0xFEF9, 0xFEFF, 0xFF05, 0xFF0B, 0xFF11, 0xFF16, 0xFF1B, 0xFF20,
      0xFF25, 0xFF2A, 0xFF2E, 0xFF33, 0xFF37, 0xFF3C, 0xFF40, 0xFF44, 0xFF48,
      0xFF4C, 0xFF50, 0xFF53, 0xFF57, 0xFF5B, 0xFF5E, 0xFF62, 0xFF65, 0xFF68,
      0xFF6B, 0xFF6F, 0xFF72, 0xFF75, 0xFF78, 0xFF7B, 0xFF7E, 0xFF81, 0xFF83,
      0xFF86, 0xFF89, 0xFF8C, 0xFF8E, 0xFF91, 0xFF93, 0xFF96, 0xFF99, 0xFF9B,
      0xFF9D, 0xFFA0, 0xFFA2, 0xFFA5, 0xFFA7, 0xFFA9, 0xFFAB, 0xFFAE, 0xFFB0,
      0xFFB2, 0xFFB4, 0xFFB6, 0xFFB8, 0xFFBA, 0xFFBC, 0xFFBE, 0xFFC0, 0xFFC2,
      0xFFC4, 0xFFC6, 0xFFC8, 0xFFCA, 0xFFCC, 0xFFCE, 0xFFCF, 0xFFD1, 0xFFD3,
      0xFFD5, 0xFFD6, 0xFFD8, 0xFFDA, 0xFFDC, 0xFFDD, 0xFFDF, 0xFFE1, 0xFFE2,
      0xFFE4, 0xFFE5, 0xFFE7, 0xFFE9, 0xFFEA, 0xFFEC, 0xFFED, 0xFFEF, 0xFFF0,
      0xFFF2, 0xFFF3, 0xFFF5, 0xFFF6, 0xFFF8, 0xFFF9, 0xFFFA, 0xFFFC, 0xFFFD,
      0xFFFF, 0x0000};
  ...
  int count = 0;
  for (long i = 0x16980; i != 0; i = (i * realAttack) >> 8) count++;
...
uint16_t NDSInstr::GetFallingRate(uint8_t DecayTime) {
  ...
  if (DecayTime == 0x7F)
    ...
    realDecay /= DecayTime;  // there is a whole subroutine that seems to
                             // resolve simply to this.  I have tested all cases
    realDecay &= 0xFFFF;
  }
...
NDSWaveArch::NDSWaveArch(RawFile *file, uint32_t offset, uint32_t length,
                         wstring name)
...
bool NDSWaveArch::GetSampleInfo() {
  ...
  for (uint32_t i = 0; i < nSamples; i++) {
    ...
    uint32_t loopOff =
        (GetShort(pSample + 6)) * 4;  //*multiplier; //represents loop point in
                                      //words, excluding header supposedly
    uint32_t nonLoopLength =
        GetShort(pSample + 8) *
        4;  // if IMA-ADPCM, subtract one for the ADPCM header
    ...
    NDSSamp *samp =
        new NDSSamp(this, pSample, dataStart + dataLength - pSample, dataStart,
    ...
    if (waveType == NDSSamp::IMA_ADPCM) {
      ...
      loopOff = loopOff - 8 +
                1;  // exclude the header's sample.  not exactly sure why 8.
...
NDSSamp::NDSSamp(VGMSampColl *sampColl, uint32_t offset, uint32_t length,
                 uint32_t dataOffset, uint32_t dataLen, uint8_t nChannels,
                 uint16_t theBPS, uint32_t theRate, uint8_t theWaveType,
                 wstring name)
...
void NDSSamp::ConvertToStdWave(uint8_t *buf) {
  if (waveType == IMA_ADPCM)
    ...
    for (unsigned int i = 0; i < dataLength;
      ...
      buf[i] ^= 0x80;  // For whatever reason, the WAV standard has PCM8
                       // unsigned and PCM16 signed
  } else
    GetBytes(dataOff, dataLength, buf);
}

// From nocash's site: The NDS data consist of a 32bit header, followed by 4bit
// values (so each byte contains two values, the first value in the lower 4bits,
// the second in upper 4 bits). The 32bit header contains initial values:
//
...
// As far as I can tell, the NDS IMA-ADPCM format has one difference from
// standard IMA-ADPCM: it clamps min (and max?) sample values differently (see
// below).  I really don't know how much of a difference it makes, but this
// implementation is, to my knowledge, the proper way of doing things for NDS.
void NDSSamp::ConvertImaAdpcm(uint8_t *buf) {
  ...
  // I'm copying nocash's IMA-ADPCM conversion method verbatim.  Big thanks to
  // him. Info is at http://nocash.emubase.de/gbatek.htm#dssound and the
  // algorithm is described as follows:
  //
  // The NDS data consist of a 32bit header, followed by 4bit values (so each
  // byte contains two values,  the first value in the lower 4bits, the second in
  // upper 4 bits). The 32bit header contains initial values:
  //
  //  Bit0-15   Initial PCM16 Value (Pcm16bit = -7FFFh..+7FFF) (not -8000h)
  //  Bit16-22  Initial Table Index Value (Index = 0..88)
  //  Bit23-31  Not used (zero)
  ...
  //
  //  Max(+7FFFh) leaves -8000h unclipped (can happen if initial PCM16 was
  //  -8000h) Min(-7FFFh) clips -8000h to -7FFFh (possibly unlike windows .WAV
  //  files?)

#define IMAMax(samp) (samp > 0x7FFF) ? ((short)0x7FFF) : samp
#define IMAMin(samp) (samp < -0x7FFF) ? ((short)-0x7FFF) : samp
#define IMAIndexMinMax(index, min, max) \
  (index > max) ? max : ((index < min) ? min : index)

void NDSSamp::process_nibble(unsigned char data4bit, int &Index,
                             int &Pcm16bit) {
...
//
// void NDSSamp::process_nibble(unsigned char code, int& stepIndex, int&
// decompSample)
//{
//    unsigned step;
//    int diff;
