    ...
    if ((*file)[i] == 'B' && (*file)[i + 1] == 'G' && (*file)[i + 2] == 'M' &&
      ...
      if (file->GetWord(i + 0x14) == 0 && file->GetWord(i + 0x18) == 0 &&
        ...
        for (int j = 0; j < nNumTracks; j++) {
          uint32_t trackSize = file->GetWord(
              pos);  // get the track size (first word before track data)
          if (trackSize + pos + j > nFileLength || trackSize == 0 ||
              trackSize > 0xFFFF) {
            bValid = false;
            break;
          }
        ...
        if (!NewBGMSeq->LoadVGMFile()) delete NewBGMSeq;
      }
    }
  }
}
...
void SquarePS2Scanner::SearchForWDSet(RawFile *file) {
  ...
  for (uint32_t i = 0; i + 0x3000 < nFileLength; i++) {
    if ((*file)[i] == 'W' && (*file)[i + 1] == 'D' && (*file)[i + 3] < 0x03) {
      if (file->GetWord(i + 0x14) == 0 && file->GetWord(i + 0x18) == 0 &&
          file->GetWord(i + 0x1C) == 0) {
        // check the data at the offset of the first region entry's sample
        // pointer.  It should be 16 0x00 bytes in a row
        numRegions = file->GetWord(i + 0xC);
        firstRgnPtr = file->GetWord(
            i + 0x20);  // read the pointer to the first region set

        // sanity check
        if (numRegions == 0 || numRegions > 500) continue;
        ...
        if (firstRgnPtr <= 0x1000) {
          ...
          // check that every region points to a valid sample by checking if
          // first 16 bytes of sample are 0
          for (unsigned int curRgn = 0; curRgn < numRegions; curRgn++) {
            ...
            if (relativeRgnSampOffset < 0x10) relativeRgnSampOffset = 0;
            uint32_t rgnSampOffset = relativeRgnSampOffset + offsetOfFirstSamp;

            if (relativeRgnSampOffset == 0) zeroOffsetCounter++;

            // if there are at least 3 zeroOffsetCounters and more than half
            // /of all samples are offset 0, something is fishy.  Assume
            // false-positive
          ...
          if (bValid) {
            ...
            if (!instrset->LoadVGMFile()) delete instrset;
          }
        }
      }
    }
