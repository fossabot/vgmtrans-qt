  ...
  for (uint32_t i = 0; i < instrset->aInstrs.size(); i++) {
    ...
    for (uint32_t j = 0; j < rgns->size(); j++) {
      ...
      double freq_multiplier =
          (double)(((ft * 32) + 0x100000) / (double)0x100000);
      double cents = log(freq_multiplier) / log((double)2) * 1200;
      if (art->fineTune < 0) cents -= 1200;
...
bool AkaoColl::PreDLSMainCreation() {
  ...
  *newInstrSet = *origInstrSet;
  ...
  if (!((AkaoSeq *)seq)->bUsesIndividualArts)  // only do this if the 0xA1 event
                                               // is actually used
    return true;
  ...
  for (uint32_t i = 0; i < numAddedInstrs; i++) {
    ...
    if (ft < 0) ft += 0x8000;
    double freq_multiplier =
        (double)(((ft * 32) + 0x100000) / (double)0x100000);  // this gives us
                                                              // the pitch
                                                              // multiplier value
                                                              // ex. 1.05946
    double cents = log(freq_multiplier) / log((double)2) * 1200;
...
bool AkaoColl::PostDLSMainCreation() {
  ...
  if (!((AkaoSeq *)seq)->bUsesIndividualArts) return true;
