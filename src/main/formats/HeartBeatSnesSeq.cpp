...
HeartBeatSnesSeq::HeartBeatSnesSeq(RawFile *file, HeartBeatSnesVersion ver,
...
void HeartBeatSnesSeq::ResetVars(void) { VGMSeq::ResetVars(); }
...
HeartBeatSnesTrack::HeartBeatSnesTrack(HeartBeatSnesSeq *parentFile,
  ...
  switch (eventType) {
    ...
    case EVENT_NOTE_LENGTH: {
      ...
      if (noteParam < 0x80) {
        ...
        desc << L"  Duration: " << (int)durIndex << L" (" << (int)spcNoteDurRate
             << L")" << L"  Velocity: " << (int)velIndex << L" ("
             << (int)spcNoteVolume << L")";
      }

      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Note Length & Param", desc.str().c_str(),
                      CLR_CHANGESTATE);
      break;
    }
    ...
    case EVENT_NOTE: {
      ...
      AddNoteByDur(beginOffset, curOffset - beginOffset, key, spcNoteVolume / 2,
                   dur, L"Note");
    ...
    case EVENT_PAN_FADE: {
      ...
      uint8_t panIndex =
          (uint8_t)min<uint8_t>((unsigned)(newPan & 0x1f), (unsigned)20);
    ...
    case EVENT_SUBEVENT: {
      ...
      HeartBeatSnesSeqSubEventType subEventType =
          (HeartBeatSnesSeqSubEventType)0;
