...
bool WDInstrSet::GetHeaderInfo() {
  ...
  if (dwSampSectSize <
      0x40)  // Some songs in the Bouncer have bizarre values here
    dwSampSectSize = 0;
  ...
  uint32_t sampCollOff =
      dwOffset + GetWord(dwOffset + 0x20) + (dwTotalRegions * 0x20);

  sampColl =
      new PSXSampColl(SquarePS2Format::name, this, sampCollOff, dwSampSectSize);
...
bool WDInstrSet::GetInstrPointers() {
  ...
  // check for bouncer WDs with 0xFFFFFFFF as the last instr pointer.  If it's
  // there ignore it
  for (uint32_t i = 0; i < dwNumInstrs; i++) {
    if (GetWord(j + i * 4) == 0xFFFFFFFF) dwNumInstrs = i;
  }
  ...
  for (uint32_t i = 0; i < dwNumInstrs; i++) {
    ...
    name << L"Instrument " << i;
    WDInstr *newWDInstr =
        new WDInstr(this, dwOffset + GetWord(j + (i * 4)), instrLength, 0, i,
                    name.str());  // strStr);
...
WDInstr::WDInstr(VGMInstrSet *instrSet, uint32_t offset, uint32_t length,
...
bool WDInstr::LoadInstr() {
  ...
  while (k * 0x20 < unLength) {
    if (bSecondToLastRgn) bLastRgn = true;
    ...
    rgn->sampOffset = GetWord(k * 0x20 + 0x4 + dwOffset) &
                      0xFFFFFFF0;  // The & is there because FFX points to
                                   // 0x----C offsets for some very odd reason
    ...
    rgn->velHigh = Convert7bitPercentVolValToStdMidiVal(
        GetByte(k * 0x20 + 0x15 + dwOffset));
    ...
    rgn->fineTune = (short)ceil(((finetune_table[rgn->fineTune] - 0x10000) *
                                 0.025766555011594949755217727389848) -
                                50);
  ...
  for (uint32_t k = 0; k < aRgns.size(); k++) {
    // Key Ranges
    if (((WDRgn *)aRgns[k])
            ->bFirstRegion)  //&& !instrument[i].region[k].bLastRegion) //used
                             //in ffx2 0049 YRP battle 1.  check out first
                             //instrument, flags are weird
      aRgns[k]->keyLow = 0;
    else if (k > 0) {
    ...
    if (((WDRgn *)aRgns[k])->bLastRegion) aRgns[k]->keyHigh = 0x7F;
    ...
    if (k == 0)  // if it's the first region of the instrument
      ...
      aRgns[k]->velLow = aRgns[k - 1]->velLow;
      aRgns[k]->velHigh = 0x7F;  // FFX 0022, aka sight of spira, was giving me
                                 // problems, hence this
      aRgns[k - 1]->velHigh =
