...
// 0c93: f0 29     beq   $0cbe
// 0c95: 68 e0     cmp   a,#$e0
// 0c97: 90 30     bcc   $0cc9             ; next note (key-off the current
// note)  0c99: 6d        push  y  0c9a: fd        mov   y,a  0c9b: ae        pop
// a  0c9c: 96 b2 0a  adc   a,$0ab2+y         ; vcmd lengths ($0af6) (skip vcmd by
// using oplens table)
...
BytePattern NinSnesScanner::ptnJumpToVcmdSMW("\x1c\x5d\xe8\x00\x1f\xdc\x0e",
                                             "xxxxx??", 7);
...
BytePattern NinSnesScanner::ptnSetDIRYI("\xe8\x3c\x8d\x5d\x3f\xfa\x05",
                                        "x?xxx??", 7);
...
BytePattern NinSnesScanner::ptnSetDIRCTOW("\x8d\x5d\xe8\x3a\x3f\x0b\x05",
                                          "xxx?x??", 7);
...
BytePattern NinSnesScanner::ptnSetDIRTS("\xe8\x0f\x8d\x5d\x4f\x1e", "x?xxx?",
                                        6);
...
// 07ef: 60        clrc
// 07f0: 85 ff 11  adc   a,$11ff           ; add patch offset for BGM (constant
// value = 12?)  07f3: 2f 09     bra   $07fe ; SFX  07f5: fd        mov   y,a  07f6:
// 10 06     bpl   $07fe             ; if percussion note:  07f8: 80        setc
...
// 08a1: 4d        push  x
// 08a2: 5d        mov   x,a
// 08a3: f5 e0 0f  mov   a,$0fe0+x         ; get real instrument index from
// lookup table  08a6: ce        pop   x  08a7: 2f 09     bra   $08b2 ; SFX  08a9:
// fd        mov   y,a  08aa: 10 06     bpl   $08b2  08ac: 80        setc  08ad: a8
// ca     sbc   a,#$ca  08af: 60        clrc  08b0: 84 39     adc   a,$39
...
// 0a1a: 4d        push  x
// 0a1b: 5d        mov   x,a
// 0a1c: f5 ab ff  mov   a,$ffab+x         ; get real instrument index from
// lookup table  0a1f: ce        pop   x  0a20: 2f 0a     bra   $0a2c ; SFX  0a22:
// fd        mov   y,a  0a23: 10 07     bpl   $0a2c             ; if percussion
// note:  0a25: 80        setc  0a26: a8 ca     sbc   a,#$ca            ;   ca-dd
// => 00-15  0a28: 60        clrc  0a29: 95 00 ff  adc   a,$ff00+x         ; add
// perc patch base
...
void NinSnesScanner::SearchForNinSnesFromARAM(RawFile *file) {
  ...
  // 1202: dd        mov   a,y
  // 1203: 80        setc
  // 1204: a8 d0     sbc   a,#$d0            ; address to offset (decrease
  // $d000)  1206: fd        mov   y,a  1207: ae        pop   a  1208: da 17
  // movw  $17,ya            ; save the offset
  ...
  // DERIVED VERSIONS
  ...
  } else if (file->SearchBytePattern(ptnInitSectionPtrYSFR,
                                     ofsInitSectionPtr)) {
    ...
    BytePattern ptnInitSongListPtrYSFR(ptnInitSongListPtrBytesYSFR, "x?xx?x",
                                       6);
  ...
  // (Minor classification for derived versions should come later as far as
  // possible)
  ...
  if (file->SearchBytePattern(ptnJumpToVcmdYSFR, ofsBranchForVcmd)) {
    ...
    if (file->SearchBytePattern(ptnReadVcmdLengthYs4, ofsReadVcmdLength)) {
      ...
      addrVoiceCmdLengthTable =
          file->GetShort(ofsReadVcmdLength + 9) + firstVoiceCmd;
    } else {
      return;
    }
  } else {
    ...
    if (file->SearchBytePattern(ptnBranchForVcmdReadahead, ofsBranchForVcmd)) {
      ...
      // this search often finds a wrong code, but some games still need it (for
      // example, Human games)
    ...
    if (file->SearchBytePattern(ptnJumpToVcmd, ofsJumpToVcmd)) {
      ...
      addrVoiceCmdLengthTable =
          file->GetShort(ofsJumpToVcmd + 14) + (firstVoiceCmd & 0x7f);
  ...
  if (addrVoiceCmdAddressTable < addrVoiceCmdLengthTable &&
      addrVoiceCmdAddressTable + (0x100 - firstVoiceCmd) * 2 >=
          addrVoiceCmdLengthTable) {
  ...
  if (version == NINSNES_STANDARD) {
    if (konamiBaseAddress != 0xffff) {
      ...
      const uint8_t STD_VCMD_LEN_TABLE[27] = {
          0x01, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x01, 0x02,
      ...
      if (firstVoiceCmd == 0xe0 &&
        ...
        if (addrVoiceCmdAddressTable + sizeof(STD_VCMD_LEN_TABLE) * 2 ==
          ...
          if (file->SearchBytePattern(ptnRD1VCmd_FA_FE, ofsRD1VCmd_FA_FE)) {
            version = NINSNES_RD1;
          } else if (file->SearchBytePattern(ptnRD2VCmdInstrADSR,
                                             ofsRD2VCmdInstrADSR)) {
            // Marvelous
            version = NINSNES_RD2;
          } else if (file->SearchBytePattern(ptnInstrVCmdACTR2, ofsInstrVCmd) &&
        ...
        if (file->SearchBytePattern(ptnIntelliVCmdFA, ofsIntelliVCmdFA)) {
          ...
          if (file->SearchBytePattern(ptnDispatchNoteFE3, ofsDispatchNote)) {
            ...
            if (firstVoiceCmd == 0xd6 &&
                file->MatchBytes(INTELLI_FE3_VCMD_LEN_TABLE,
                                 addrVoiceCmdLengthTable,
                                 sizeof(INTELLI_FE3_VCMD_LEN_TABLE))) {
              version = NINSNES_INTELLI_FE3;
            }
          } else if (file->SearchBytePattern(ptnDispatchNoteFE4,
                                             ofsDispatchNote)) {
            const uint8_t INTELLI_FE4_VCMD_LEN_TABLE[36] = {
                0x01, 0x01, 0x02, 0x03, 0x00, 0x01, 0x02, 0x01, 0x02,
                0x01, 0x01, 0x03, 0x00, 0x01, 0x02, 0x03, 0x01, 0x03,
                0x03, 0x00, 0x01, 0x03, 0x00, 0x03, 0x03, 0x03, 0x01,
                0x00, 0x00, 0x02, 0x01, 0x00, 0x01, 0x01, 0x01, 0x01};
            if (firstVoiceCmd == 0xda &&
                file->MatchBytes(INTELLI_FE4_VCMD_LEN_TABLE,
                                 addrVoiceCmdLengthTable,
            ...
            const uint8_t INTELLI_TA_VCMD_LEN_TABLE[36] = {
                ...
                0x01, 0x01, 0x03, 0x00, 0x01, 0x02, 0x03, 0x01, 0x03,
                0x03, 0x00, 0x01, 0x03, 0x00, 0x03, 0x03, 0x03, 0x01,
                0x00, 0x00, 0x02, 0x02, 0x00, 0x01, 0x01, 0x01, 0x01};
            if (firstVoiceCmd == 0xda &&
                file->MatchBytes(INTELLI_TA_VCMD_LEN_TABLE,
                                 addrVoiceCmdLengthTable,
  ...
  for (uint8_t songIndex = 1; songIndex <= 0x7f; songIndex++) {
    ...
    for (uint8_t trackIndex = 0; trackIndex < 8; trackIndex++) {
      uint16_t addrTrackStart =
          file->GetShort(addrFirstSection + trackIndex * 2);
      if (addrTrackStart != 0) {
  ...
  // (note that the section pointer points to the "next" section actually, in
  // most cases)
  ...
  NinSnesSeq *newSeq = new NinSnesSeq(file, version, addrSongStart, 0,
                                      volumeTable, durRateTable, name);
  ...
  if (file->SearchBytePattern(ptnLoadInstrTableAddress,
  ...
  } else if (file->SearchBytePattern(ptnLoadInstrTableAddressSMW,
                                     ofsLoadInstrTableAddressASM)) {
  ...
  // DERIVED VERSIONS
    ...
    if (file->SearchBytePattern(ptnLoadInstrTableAddressCTOW,
                                ofsLoadInstrTableAddressASM)) {
      addrInstrTable = file->GetByte(ofsLoadInstrTableAddressASM + 7) |
                       (file->GetByte(ofsLoadInstrTableAddressASM + 10) << 8);
    ...
    if (file->SearchBytePattern(ptnLoadInstrTableAddressYSFR,
                                ofsLoadInstrTableAddressASM)) {
  ...
  if (version == NINSNES_KONAMI) {
    if (file->SearchBytePattern(ptnInstrVCmdGD3, ofsInstrVCmd)) {
      ...
      if (konamiAddrTuningTableHigh > konamiAddrTuningTableLow &&
        ...
        konamiTuningTableSize =
            konamiAddrTuningTableHigh - konamiAddrTuningTableLow;
      }
    }
  }

  NinSnesInstrSet *newInstrSet =
      new NinSnesInstrSet(file, version, addrInstrTable, spcDirAddr);
