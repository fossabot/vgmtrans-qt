...
const uint8_t SuzukiSnesSeq::NOTE_DUR_TABLE[13] = {0xc0, 0x90, 0x60, 0x48, 0x30,
                                                   0x24, 0x20, 0x18, 0x10, 0x0c,
                                                   0x08, 0x06, 0x03};

SuzukiSnesSeq::SuzukiSnesSeq(RawFile *file, SuzukiSnesVersion ver,
...
bool SuzukiSnesSeq::GetTrackPointers(void) { return true; }
...
SuzukiSnesTrack::SuzukiSnesTrack(SuzukiSnesSeq *parentFile, long offset,
                                 long length)
...
bool SuzukiSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_TIMER1_FREQ: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Timer 1 Frequency", desc.str().c_str(), CLR_CHANGESTATE,
                      ICON_TEMPO);
      break;
    }
    ...
    case EVENT_TIMER1_FREQ_REL: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Timer 1 Frequency (Relative)", desc.str().c_str(),
                      CLR_CHANGESTATE, ICON_TEMPO);
      break;
    }
    ...
    case EVENT_NOISE_FREQ_REL: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Noise Frequency (Relative)", desc.str().c_str(),
                      CLR_CHANGESTATE, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_PORTAMENTO_TOGGLE: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Portamento On/Off", desc.str().c_str(), CLR_PORTAMENTO,
                      ICON_CONTROL);
      break;
    }

    case EVENT_PAN: {
      // For left pan, the engine will decrease right volume (linear), but will
      // do nothing to left volume. For right pan, the engine will do the
      // opposite. For center pan, it will not decrease any volumes.
    ...
    case EVENT_PAN_FADE: {
      uint8_t fadeLength = GetByte(curOffset++);
      uint8_t pan = GetByte(curOffset++);
      desc << L"Fade Length: " << (int)fadeLength << L"  Pan: "
           << (int)(pan >> 1);
