...
// 0ef2: c4 c1     mov   $c1,a
// 0ef4: ae        pop   a
// 0ef5: c4 c0     mov   $c0,a             ; $C0/1 = (WORD) [$C0] (sequence
// header)  0ef7: 04 c1     or    a,$c1  0ef9: f0 9a     beq   $0e95             ;
// no song  0efb: e8 ff     mov   a,#$ff  0efd: d5 15 05  mov   $0515+x,a  0f00: e8
// 00     mov   a,#$00  0f02: d5 45 05  mov   $0545+x,a  0f05: d5 4d 05  mov
// $054d+x,a  0f08: d5 35 05  mov   $0535+x,a  0f0b: d5 55 05  mov   $0555+x,a ;
//read the sequence header  0f0e: 8d 00     mov   y,#$00  0f10: f7 c0     mov
// a,($c0)+y         ; header+0: initial tempo  0f12: fc        inc   y  0f13: d5
// 25 05  mov   $0525+x,a
...
// 121d: c4 a1     mov   $a1,a
// 121f: ae        pop   a
// 1220: c4 a0     mov   $a0,a             ; $A0/1 = (WORD) [$A0]+1 (sequence
// header)  1222: e8 00     mov   a,#$00  1224: d5 df 03  mov   $03df+x,a  1227: d5
// c7 03  mov   $03c7+x,a  122a: d5 e7 03  mov   $03e7+x,a  122d: e8 02     mov
// a,#$02  122f: d5 d7 03  mov   $03d7+x,a ; read the sequence header  1232: 8d 00
// mov   y,#$00  1234: f7 a0     mov   a,($a0)+y         ; header+0: initial tempo
// 1236: fc        inc   y
...
// 1497: c4 a1     mov   $a1,a
// 1499: ae        pop   a
// 149a: c4 a0     mov   $a0,a             ; $A0/1 = (WORD) [$A0]+1 (sequence
// header)  149c: e8 00     mov   a,#$00  149e: d5 ef 03  mov   $03ef+x,a  14a1: d5
// d7 03  mov   $03d7+x,a  14a4: d5 f7 03  mov   $03f7+x,a  14a7: e8 02     mov
// a,#$02  14a9: d5 e7 03  mov   $03e7+x,a ; read the sequence header  14ac: 8d 00
// mov   y,#$00  14ae: f7 a0     mov   a,($a0)+y         ; header+0: initial tempo
// 14b0: fc        inc   y
...
// 143c: bc        inc   a                 ; skip offset +0: number of
// instruments  143d: fd        mov   y,a  143e: f7 c4     mov   a,($c4)+y
// ; read global instrument number  1440: fd        mov   y,a  1441: f6 67 05  mov
// a,$0567+y  1444: 68 ff     cmp   a,#$ff  1446: d0 02     bne   $144a
//;
// 1448: 00        nop
// 1449: bc        inc   a
...
// 185c: bc        inc   a                 ; skip offset +0: number of
// instruments  185d: bc        inc   a                 ; skip offset +1: ?  185e:
// fd        mov   y,a  185f: f7 a4     mov   a,($a4)+y         ; read global
// instrument number  1861: 65 84 01  cmp   a,$0184  1864: f0 08     beq   $186e
// 1866: fd        mov   y,a
// 1867: f6 ff 03  mov   a,$03ff+y         ; read SRCN by global instrument
// number  186a: 68 ff     cmp   a,#$ff  186c: d0 15     bne   $1883
//;
...
void ChunSnesScanner::SearchForChunSnesFromARAM(RawFile *file) {
  ...
  if (file->SearchBytePattern(ptnLoadSeqWinterV3, ofsLoadSeq)) {
    addrSongList =
        file->GetByte(ofsLoadSeq + 8) | (file->GetByte(ofsLoadSeq + 11) << 8);
  ...
  if (file->SearchBytePattern(ptnSaveSongIndexSummerV2, ofsSaveSongIndex)) {
    ...
    if (addrCurrentPos != 0) {
      ...
      for (int8_t songIndexCandidate = 0; songIndexCandidate < 0x7f;
           songIndexCandidate++) {
        uint16_t addrSeqEntry =
            addrSongList + (songIndexCandidate * CHUNSNES_SEQENT_SIZE);
        ...
        if ((addrSeqHeader & 0xff00) == 0 ||
            (addrSeqHeader & 0xff00) == 0xff00) {
          continue;
        }
  ...
  uint16_t addrSeqHeader =
      file->GetShort(addrSeqEntry + CHUNSNES_SEQENT_OFFSET_OF_HEADER);
  ChunSnesSeq *newSeq =
      new ChunSnesSeq(file, version, minorVersion, addrSeqHeader, name);
  ...
  if (file->SearchBytePattern(ptnProgChangeVCmdSummer, ofsProgChangeVCmd)) {
    addrInstrSetTable = file->GetByte(ofsProgChangeVCmd + 14) |
                        (file->GetByte(ofsProgChangeVCmd + 17) << 8);
    addrSampNumTable = file->GetShort(ofsProgChangeVCmd + 47);
    addrSampleTable = file->GetByte(ofsProgChangeVCmd + 67) |
                      (file->GetByte(ofsProgChangeVCmd + 70) << 8);
  ...
  } else if (file->SearchBytePattern(ptnProgChangeVCmdWinter,
                                     ofsProgChangeVCmd)) {
    ...
    addrSampleTable = file->GetByte(ofsProgChangeVCmd + 97) |
                      (file->GetByte(ofsProgChangeVCmd + 100) << 8);
  ...
  ChunSnesInstrSet *newInstrSet =
      new ChunSnesInstrSet(file, version, addrInstrSet, addrSampNumTable,
                           addrSampleTable, spcDirAddr);
