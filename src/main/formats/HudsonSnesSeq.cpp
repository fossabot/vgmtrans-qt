...
HudsonSnesSeq::HudsonSnesSeq(RawFile *file, HudsonSnesVersion ver,
...
bool HudsonSnesSeq::GetHeaderInfo(void) {
  ...
  if (version == HUDSONSNES_V0 || version == HUDSONSNES_V1) {
    VGMHeader *trackPtrHeader =
        header->AddHeader(curOffset, 0, L"Track Pointers");
  ...
  while (true) {
    ...
    switch (eventType) {
      ...
      case HEADER_EVENT_TIMEBASE: {
        ...
        // however, we always use SEQ_PPQN and adjust note lengths when
        // necessary instead.
      ...
      case HEADER_EVENT_TRACKS: {
        VGMHeader *trackPtrHeader =
            header->AddHeader(beginOffset, 0, L"Track Pointers");
      ...
      case HEADER_EVENT_INSTRUMENTS_V1: {
        VGMHeader *instrHeader =
            header->AddHeader(beginOffset, 1, L"Instruments");
      ...
      case HEADER_EVENT_PERCUSSIONS_V1: {
        VGMHeader *percHeader =
            header->AddHeader(beginOffset, 1, L"Percussions");
        ...
        for (uint8_t percNote = 0; percNote < tableLength; percNote++) {
          ...
          VGMHeader *percNoteHeader = percHeader->AddHeader(
              addrPercItem, 4, percNoteName.str().c_str());
      ...
      case HEADER_EVENT_NOTE_VELOCITY: {
        VGMHeader *aHeader =
            header->AddHeader(beginOffset, 1, L"Note Velocity");
        aHeader->AddSimpleItem(beginOffset, 1, L"Event ID");
...
bool HudsonSnesSeq::GetTrackPointersInHeaderInfo(VGMHeader *header,
                                                 uint32_t &offset) {
...
bool HudsonSnesSeq::GetTrackPointers(void) {
  for (int trackIndex = 0; trackIndex < MAX_TRACKS; trackIndex++) {
    if ((TrackAvailableBits & (1 << trackIndex)) != 0) {
      HudsonSnesTrack *track =
          new HudsonSnesTrack(this, TrackAddresses[trackIndex]);
...
HudsonSnesTrack::HudsonSnesTrack(HudsonSnesSeq *parentFile, long offset,
                                 long length)
...
bool HudsonSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_NOTE: {
      const uint8_t NOTE_DUR_TABLE[8] = {0xc0, 0x60, 0x30, 0x18,
                                         0x0c, 0x06, 0x03, 0x01};
    ...
    case EVENT_REVERSE_PHASE: {
      ...
      desc << L"Reverse Left: " << (reverseLeft ? L"On" : L"Off")
           << L"Reverse Right: " << (reverseRight ? L"On" : L"Off");
    ...
    case EVENT_LOOP_START: {
      ...
      AddGenericEvent(beginOffset, 2, L"Loop Start", desc.str().c_str(),
    ...
    case EVENT_LOOP_END: {
      ...
      if (--count == 0) {
        ...
        curOffset = (spcCallStack[spcCallStackPtr - 3] |
                     (spcCallStack[spcCallStackPtr - 2] << 8)) +
                    1;
      }
    ...
    case EVENT_SUBROUTINE: {
      ...
      AddGenericEvent(beginOffset, 3, L"Pattern Play", desc.str().c_str(),
                      CLR_LOOP, ICON_STARTREP);
    ...
    case EVENT_VIBRATO: {
      if (parentSeq->version == HUDSONSNES_V0 ||
          parentSeq->version == HUDSONSNES_V1) {
    ...
    case EVENT_ECHO_VOLUME: {
      ...
      desc << L"Left Volume: " << (int)volumeLeft << L"  Right Volume: "
           << (int)volumeRight;
    ...
    case EVENT_ECHO_PARAM: {
      ...
      desc << L"Echo Delay: " << (int)delay << L"  Echo Feedback: "
    ...
    case EVENT_ECHO_ON: {
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Echo On",
                      desc.str().c_str(), CLR_REVERB, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_PITCH_ATTACK_ENV_ON: {
      ...
      desc << L"Speed: " << (int)speed << L"  Depth: " << (int)depth
           << L"  Direction: " << (upTo ? L"Up" : L"Down");
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Attack Pitch Envelope On", desc.str().c_str(),
    ...
    case EVENT_PITCH_ATTACK_ENV_OFF: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Attack Pitch Envelope Off", desc.str().c_str(),
                      CLR_PITCHBEND, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_LOOP_POINT: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Infinite Loop Point", desc.str().c_str(), CLR_LOOP,
                      ICON_STARTREP);
      break;
    }
    ...
    case EVENT_VOLUME_FROM_TABLE: {
      const uint8_t VOLUME_TABLE[91] = {
          0x00, 0x01, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x02, 0x03,
          0x03, 0x03, 0x03, 0x03, 0x03, 0x04, 0x04, 0x04, 0x04, 0x05, 0x05,
          0x05, 0x05, 0x06, 0x06, 0x06, 0x07, 0x07, 0x08, 0x08, 0x09, 0x09,
          0x0a, 0x0a, 0x0b, 0x0b, 0x0c, 0x0d, 0x0e, 0x0e, 0x0f, 0x10, 0x11,
          0x12, 0x13, 0x14, 0x15, 0x17, 0x18, 0x1a, 0x1b, 0x1d, 0x1e, 0x20,
          0x22, 0x24, 0x26, 0x28, 0x2b, 0x2d, 0x30, 0x33, 0x36, 0x39, 0x3c,
          0x40, 0x44, 0x48, 0x4c, 0x51, 0x55, 0x5a, 0x60, 0x66, 0x6c, 0x72,
          0x79, 0x80, 0x87, 0x8f, 0x98, 0xa1, 0xaa, 0xb5, 0xbf, 0xcb, 0xd7,
          0xe3, 0xf1, 0xff,
      };
    ...
    case EVENT_END: {
      if (spcCallStackPtr == 0) {
        ...
        curOffset = (spcCallStack[spcCallStackPtr - 2] |
                     (spcCallStack[spcCallStackPtr - 1] << 8)) +
                    2;
        spcCallStackPtr -= 2;
      }
      break;
    ...
    case EVENT_SUBEVENT: {
      ...
      switch (subEventType) {
        case SUBEVENT_UNKNOWN0:
          desc << L"Subevent: 0x" << std::hex << std::setfill(L'0')
               << std::setw(2) << std::uppercase << (int)subStatusByte;
          AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                     desc.str().c_str());
          break;
        ...
        case SUBEVENT_UNKNOWN1: {
          ...
          desc << L"Subevent: 0x" << std::hex << std::setfill(L'0')
               << std::setw(2) << std::uppercase << (int)subStatusByte
               << std::dec << std::setfill(L' ') << std::setw(0) << L"  Arg1: "
               << (int)arg1;
          AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                     desc.str().c_str());
          break;
        ...
        case SUBEVENT_UNKNOWN3: {
          ...
          desc << L"Subevent: 0x" << std::hex << std::setfill(L'0')
               << std::setw(2) << std::uppercase << (int)subStatusByte
               << std::dec << std::setfill(L' ') << std::setw(0) << L"  Arg1: "
               << (int)arg1 << L"  Arg2: " << (int)arg2 << L"  Arg3: "
               << (int)arg3;
          AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                     desc.str().c_str());
          break;
        ...
        case SUBEVENT_PERC_ON: {
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"Percussion On", desc.str().c_str(), CLR_CHANGESTATE,
                          ICON_CONTROL);
          break;
        }
        ...
        case SUBEVENT_PERC_OFF: {
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"Percussion Off", desc.str().c_str(),
        ...
        case SUBEVENT_NOTE_VEL_OFF: {
          ...
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"Note Velocity Off", desc.str().c_str(), CLR_MISC,
                          ICON_CONTROL);
          break;
        }
        ...
        case SUBEVENT_MOV_IMM: {
          ...
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"MOV (Immediate)", desc.str().c_str(), CLR_MISC);
        ...
        case SUBEVENT_MOV: {
          ...
          desc << L"Destination Register: " << (int)regDst
               << L"  Source Register: " << (int)regSrc;
          AddGenericEvent(beginOffset, curOffset - beginOffset, L"MOV",
                          desc.str().c_str(), CLR_MISC);

          if (regDst < HUDSONSNES_USERRAM_SIZE &&
              regSrc < HUDSONSNES_USERRAM_SIZE) {
        ...
        case SUBEVENT_CMP_IMM: {
          ...
          desc << L"Register: " << (int)reg << L"  Value: " << (int)val;
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"CMP (Immediate)", desc.str().c_str(), CLR_MISC);
        ...
        case SUBEVENT_ADSR_AR: {
          ...
          desc << L"AR: " << (int)newAR;
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"ADSR Attack Rate", desc.str().c_str(), CLR_ADSR,
                          ICON_CONTROL);
          break;
        }
        ...
        case SUBEVENT_ADSR_DR: {
          ...
          desc << L"DR: " << (int)newDR;
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"ADSR Decay Rate", desc.str().c_str(), CLR_ADSR,
                          ICON_CONTROL);
          break;
        }
        ...
        case SUBEVENT_ADSR_SL: {
          ...
          desc << L"SL: " << (int)newSL;
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"ADSR Sustain Level", desc.str().c_str(), CLR_ADSR,
                          ICON_CONTROL);
          break;
        }
        ...
        case SUBEVENT_ADSR_SR: {
          ...
          desc << L"SR: " << (int)newSR;
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"ADSR Sustain Rate", desc.str().c_str(), CLR_ADSR,
                          ICON_CONTROL);
          break;
        }
        ...
        case SUBEVENT_ADSR_RR: {
          ...
          desc << L"SR: " << (int)newSR;
          AddGenericEvent(beginOffset, curOffset - beginOffset,
                          L"ADSR Release Rate", desc.str().c_str(), CLR_ADSR,
                          ICON_CONTROL);
          break;
        }
