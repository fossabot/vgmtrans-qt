...
// 151c: ee        pop   y
// 151d: 60        clrc
// 151e: 97 08     adc   a,($08)+y         ; offset +0: sample index + (n *
// 0x10)  1520: fc        inc   y  1521: 6d        push  y  1522: fd        mov
// y,a  1523: f6 25 0a  mov   a,$0a25+y         ; read actual SRCN  1526: 8d 04
// mov   y,#$04  1528: 3f 37 14  call  $1437             ; set SRCN  152b: ee
// pop   y
...
void HeartBeatSnesScanner::SearchForHeartBeatSnesFromARAM(RawFile *file) {
  ...
  if (file->SearchBytePattern(ptnReadSongList, ofsReadSongList)) {
    ...
    if (addrSongListLo >= addrSongListHi ||
        addrSongListHi - addrSongListLo >= 0x10) {
      return;
    }
  ...
  if (file->SearchBytePattern(ptnSaveSeqHeaderAddress,
                              ofsSaveSeqHeaderAddress)) {
  ...
  // guess song index
  int8_t songIndex = -1;
  for (int8_t songIndexCandidate = 0; songIndexCandidate < maxSongIndex;
       songIndexCandidate++) {
    uint16_t addrSeqHeader =
        file->GetByte(addrSongListLo + songIndexCandidate) |
  ...
  HeartBeatSnesSeq *newSeq =
      new HeartBeatSnesSeq(file, version, addrSeqHeader, name);
  ...
  if (ofsInstrTable >= ofsFirstTrack ||
      addrSeqHeader + ofsInstrTable >= 0x10000) {
    return;
  }
  ...
  for (uint16_t newTableSize = 0; newTableSize < instrTableSize;
       newTableSize++) {
  ...
  HeartBeatSnesInstrSet *newInstrSet =
      new HeartBeatSnesInstrSet(file, version, addrInstrTable, instrTableSize,
                                addrSRCNTable, songIndex, spcDirAddr);
