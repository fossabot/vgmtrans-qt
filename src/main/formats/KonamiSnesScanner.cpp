...
// 194d: 8f 03 05  mov   $05,#$03          ; $04/5 = $03e6 (sequence header
// table)  1950: 9c        dec   a  1951: 8d 05     mov   y,#$05            ; 5
// bytes  1953: cf        mul   ya  1954: 7a 04     addw  ya,$04  1956: da 04
// movw  $04,ya  1958: 8d 00     mov   y,#$00  195a: cd 00     mov   x,#$00  195c:
// f7 04     mov   a,($04)+y  195e: c4 20     mov   $20,a             ; offset +0:
// ?  1960: fc        inc   y  1961: f7 04     mov   a,($04)+y  1963: c4 06     mov
// $06,a             ; offset +1: ?  1965: e4 0c     mov   a,$0c  1967: 68 4d
// cmp   a,#$4d  1969: b0 5a     bcs   $19c5             ; branch if song >= 77
// 196b: cd 0c     mov   x,#$0c
// 196d: 68 41     cmp   a,#$41
...
// 1950: 2d        push  a                 ; push vcmd func address, as a return
// address  1951: f6 fb 1a  mov   a,$1afb+y  1954: f0 08     beq   $195e
...
BytePattern KonamiSnesScanner::ptnSetDIRGG4("\x8f\x5d\xf2\x8f\x04\xf3",
                                            "xxxx?x", 6);
...
// 17e6: c4 05     mov   $05,a             ; sample map = *(u16)($058a +
// bank_offset)  17e8: ae        pop   a  17e9: 3f f5 17  call  $17f5  17ec: 5f 12
// 15  jmp   $1512
...
// 13e3: dd        mov   a,y
// 13e4: 68 1f     cmp   a,#$1f
// 13e6: b0 0c     bcs   $13f4             ; use another map if patch number is
// large  13e8: 8f 68 04  mov   $04,#$68  13eb: 8f 05 05  mov   $05,#$05          ;
// common sample map = #$0568  13ee: 3f 31 14  call  $1431  13f1: 5f 45 11  jmp
// $1145 ; use another map  13f4: a8 1f     sbc   a,#$1f            ; patch -=
// 0x1f  13f6: 2d        push  a  13f7: eb 25     mov   y,$25             ; bank
// offset  13f9: f6 58 05  mov   a,$0558+y  13fc: c4 04     mov   $04,a  13fe: f6 59
// 05  mov   a,$0559+y  1401: c4 05     mov   $05,a             ; sample map =
// *(u16)($0558 + bank_offset)  1403: ae        pop   a  1404: 3f 31 14  call
// $1431  1407: 5f 45 11  jmp   $1145
...
// 1b9e: dd        mov   a,y
// 1b9f: 68 28     cmp   a,#$28
// 1ba1: b0 0c     bcs   $1baf             ; use another map if patch number is
// large  1ba3: 8f 3c 04  mov   $04,#$3c  1ba6: 8f 0a 05  mov   $05,#$0a          ;
// common sample map = #$0a3c  1ba9: 3f ee 1b  call  $1bee  1bac: 5f e2 18  jmp
// $18e2 ; use another map  1baf: a8 28     sbc   a,#$28            ; patch -=
// 0x28  1bb1: 2d        push  a  1bb2: eb 25     mov   y,$25             ; bank
// offset  1bb4: f6 20 0a  mov   a,$0a20+y  1bb7: c4 04     mov   $04,a  1bb9: f6 21
// 0a  mov   a,$0a21+y  1bbc: c4 05     mov   $05,a             ; sample map =
// *(u16)($0a20 + bank_offset)  1bbe: ae        pop   a  1bbf: 3f ee 1b  call
// $1bee  1bc2: 5f e2 18  jmp   $18e2
...
// 132f: c4 05     mov   $05,a             ; sample map = *(u16)($0a20 +
// bank_offset)  1331: ae        pop   a  1332: 3f 3e 13  call  $133e  1335: 5f 61
// 10  jmp   $1061 ; update sample (percussion note)  1338: 8f e8 04  mov
// $04,#$e8  133b: 8f 07 05  mov   $05,#$07          ; $04/5 = $07e8 ; load
//instrument attributes from instrument table ; a = patch number, $04 =
//instrument table address  133e: 1c        asl   a  133f: 1c        asl   a  1340:
// 1c        asl   a  1341: 8d 00     mov   y,#$00  1343: 7a 04     addw  ya,$04
// 1345: da 04     movw  $04,ya            ; load address by index `$04 +=
// (patch * 8)`
...
// 0f1d: c4 05     mov   $05,a             ; sample map = *(u16)($0628 + bank *
// 2)  0f1f: f5 97 02  mov   a,$0297+x         ; instrument #  0f22: 80        setc
...
// 0f3b: da 04     movw  $04,ya            ; load address by index `$04 +=
// (patch * 8)`
...
// 1bf3: da 04     movw  $04,ya            ; load address by index `$04 +=
// (patch * 7)`
...
void KonamiSnesScanner::SearchForKonamiSnesFromARAM(RawFile *file) {
  ...
  if (file->SearchBytePattern(ptnSetSongHeaderAddressGG4,
                              ofsSetSongHeaderAddress)) {
    ...
    addrSongList = file->GetByte(ofsReadSongList + 3) |
                   (file->GetByte(ofsReadSongList + 6) << 8);
  ...
  if (file->SearchBytePattern(ptnJumpToVcmdGG4, ofsJumpToVcmd)) {
    ...
    if (file->SearchBytePattern(ptnBranchForVcmd6xCNTR3, ofsBranchForVcmd6x)) {
      ...
      vcmd6XCountInList = 5;
    } else if (file->SearchBytePattern(ptnBranchForVcmd6xMDR2,
                                       ofsBranchForVcmd6x)) {
    ...
    // check table length
    if (addrVcmdLengthTable + (vcmd6XCountInList + 0x20) * vcmdLenItemSize >=
        0x10000) {
      return;
    }
  } else {
  ...
  if (hasSongList) {
    ...
    if (file->GetByte(addrVcmdLengthTable + (0xed - 0xe0) * vcmdLenItemSize) ==
        3) {
      version = KONAMISNES_V4;
    } else if (file->GetByte(addrVcmdLengthTable +
                             (0xfc - 0xe0) * vcmdLenItemSize) == 2) {
  ...
  KonamiSnesSeq *newSeq =
      new KonamiSnesSeq(file, version, addrSongHeader, name);
  ...
  } else if ((itrDSP = file->tag.binaries.find(L"dsp")) !=
             file->tag.binaries.end()) {
  ...
  if (file->SearchBytePattern(ptnLoadInstrJOP, ofsLoadInstr)) {
    addrCommonInstrTable = file->GetByte(ofsLoadInstr + 8) |
                           (file->GetByte(ofsLoadInstr + 11) << 8);
    ...
    addrBankedInstrTable =
        file->GetShort(addrInstrTableBanks + file->GetByte(addrCurrentBank));
    ...
    if (file->SearchBytePattern(ptnLoadPercInstrGG4, ofsLoadPercInstr)) {
      addrPercInstrTable = file->GetByte(ofsLoadPercInstr + 1) |
                           (file->GetByte(ofsLoadPercInstr + 4) << 8);
    } else {
      return;
    }
  } else if (file->SearchBytePattern(ptnLoadInstrGP, ofsLoadInstr)) {
    addrCommonInstrTable = file->GetByte(ofsLoadInstr + 14) |
                           (file->GetByte(ofsLoadInstr + 17) << 8);
    ...
    addrCommonInstrTable = file->GetByte(ofsLoadInstr + 15) |
                           (file->GetByte(ofsLoadInstr + 18) << 8);
    ...
    addrCommonInstrTable = file->GetByte(ofsLoadInstr + 12) |
                           (file->GetByte(ofsLoadInstr + 15) << 8);
    ...
    addrPercInstrTable = file->GetByte(ofsLoadInstr + 46) |
                         (file->GetByte(ofsLoadInstr + 49) << 8);
  } else if (file->SearchBytePattern(ptnLoadInstrCNTR3, ofsLoadInstr)) {
    addrCommonInstrTable = file->GetByte(ofsLoadInstr + 15) |
                           (file->GetByte(ofsLoadInstr + 19) << 8);
    firstBankedInstr = file->GetByte(ofsLoadInstr + 11);

    uint16_t addrCurrentBank = file->GetShort(ofsLoadInstr + 32);
    uint16_t addrInstrTableBanks = file->GetShort(ofsLoadInstr + 37);
    addrBankedInstrTable = file->GetShort(addrInstrTableBanks +
                                          file->GetByte(addrCurrentBank) * 2);

    addrPercInstrTable = file->GetByte(ofsLoadInstr + 59) |
                         (file->GetByte(ofsLoadInstr + 63) << 8);
  } else {
    return;
  }

  KonamiSnesInstrSet *newInstrSet = new KonamiSnesInstrSet(
      file, version, addrCommonInstrTable, addrBankedInstrTable,
