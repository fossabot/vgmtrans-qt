// The following sequence analysis code is based on the work of Sound Tester 774
// from 2ch.net, author of so2mml. The code is based on his write-up of the
// format specifications.  Many thanks to him.
...
bool TriAcePS1Seq::GetTrackPointers(void) {
  VGMHeader *TrkInfoHeader =
      header->AddHeader(dwOffset + 0x16, 6 * 32, L"Track Info Blocks");
...
void TriAcePS1Seq::ResetVars(void) { VGMSeq::ResetVars(); }

// **************
// TriAcePS1Track
// **************

TriAcePS1Track::TriAcePS1Track(TriAcePS1Seq *parentSeq, long offset,
                               long length)
...
void TriAcePS1Track::LoadTrackMainLoop(uint32_t stopOffset, int32_t stopTime) {
  ...
  while (scorePatternOffset != 0xFFFF) {
    if (seq->patternMap[scorePatternOffset])
      ...
      TriAcePS1ScorePattern *pattern =
          new TriAcePS1ScorePattern(seq, scorePatternOffset);
    ...
    if (seq->curScorePattern)
      seq->curScorePattern->unLength =
          endOffset - seq->curScorePattern->dwOffset;
...
bool TriAcePS1Track::ReadEvent(void) {
  ...
  if (status_byte <= 0x7F) {
    event_dur = GetByte(
        curOffset++);  // Delta time from "Note on" to "Next command(op-code)".
    uint8_t note_dur;
    uint8_t velocity;
    if (!impliedNoteDur)
      note_dur =
          GetByte(curOffset++);  // Delta time from "Note on" to "Note off".
    else
    ...
    AddNoteByDur_Extend(beginOffset, curOffset - beginOffset, status_byte,
                        velocity, note_dur);
  } else
    switch (status_byte) {
      case 0x80:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Score Pattern End", L"", CLR_TRACKEND);
        return false;
      ...
      case 0x8D:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Dal Segno: start point", L"", CLR_UNKNOWN);
        break;
      ...
      case 0x8E:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Dal Segno: end point", L"", CLR_UNKNOWN);
        break;
      ...
      case 0x9E:
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Imply Note Params", L"", CLR_CHANGESTATE);
        break;
  ...
  if (event_dur) AddTime(event_dur);

  return true;
}

// The following two functions are overridden so that events become children of
// the Score Patterns and not the tracks.
bool TriAcePS1Track::IsOffsetUsed(uint32_t offset) { return false; }
