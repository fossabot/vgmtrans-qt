...
bool FFTSeq::GetHeaderInfo(void) {
  //-----------------------------------------------------------
  //	Written by "Sound tester 774" in "内蔵音源をMIDI変換するスレ(in
  //http://www.2ch.net)" 	2009. 6.17 (Wed.) 	2009. 6.30 (Thu.)
  //-----------------------------------------------------------
  unLength = GetShort(dwOffset + 0x08);
  nNumTracks = GetByte(
      dwOffset + 0x14);  // uint8_t (8bit)		GetWord() から修正
  uint8_t cNumPercussion = GetByte(
      dwOffset + 0x15);  // uint8_t (8bit)	Quantity of Percussion struct
  //	unsigned char	cBankNum		= GetByte(dwOffset+0x16);
  assocWdsID =
      GetShort(dwOffset + 0x16);  // uint16_t (16bit)	Default program bank No.
  uint16_t ptSongTitle = GetShort(
      dwOffset +
      0x1E);  // uint16_t (16bit)	Pointer of music title (AscII strings)
  uint16_t ptPercussionTbl = GetShort(
      dwOffset + 0x20);  // uint16_t (16bit)	Pointer of Percussion struct
  ...
  VGMHeader *trackPtrs =
      AddHeader(dwOffset + 0x22, nNumTracks * 2, L"Track Pointers");
  for (unsigned int i = 0; i < nNumTracks; i++)
    trackPtrs->AddSimpleItem(dwOffset + 0x22 + i * 2, 2, L"Track Pointer");
  VGMHeader *titleHdr =
      AddHeader(dwOffset + ptSongTitle, titleLength, L"Song Name");

  //	if(cNumPercussion!=0){
  ////これ、やっぱ、いらない。
  //		hdr->AddSimpleItem(dwOffset+ptPercussionTbl, cNumPercussion*5,
  //L"Drumkit Struct");
  //	}
  //-----------------------------------------------------------
  ...
  //	while (j && j != '.')
  //	{
  ////		j = GetByte(dwOffset+0x22+nNumTracks*2+2 + i++);
  /////修正
...
bool FFTSeq::GetTrackPointers(void) {
  for (unsigned int i = 0; i < nNumTracks; i++)
    aTracks.push_back(
        new FFTTrack(this, GetShort(dwOffset + 0x22 + (i * 2)) + dwOffset));
  return true;
}
...
bool FFTTrack::ReadEvent(void) {
  ...
  if (status_byte < 0x80)  // then it's a note on event
  {
    ...
    if (iDeltaTime == 0) iDeltaTime = GetByte(curOffset++);  // Delta time
    if (bNoteOn) AddNoteOffNoItem(prevKey);
    AddNoteOn(beginOffset, curOffset - beginOffset, octave * 12 + relative_key,
              vel);
    AddTime(iDeltaTime);
    ...
    switch (status_byte) {
      ...
      case 0x91:
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Track Repeat Point", L"", CLR_LOOP);
        // AddEventItem(_T("Repeat"), ICON_STARTREP, beginOffset,
        // curOffset-beginOffset, BG_CLR_WHEAT);
        break;
      ...
      case 0x98: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat Begin",
                        L"", CLR_LOOP);
        // AddEventItem("Loop Begin", ICON_STARTREP, beginOffset,
        // curOffset-beginOffset, BG_CLR_WHEAT);
        break;
      }
      ...
      case 0x99:
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat End",
                        L"", CLR_LOOP);
        // AddEventItem("Loop End", ICON_ENDREP, beginOffset,
        // curOffset-beginOffset, BG_CLR_WHEAT);
        ...
        if (loopID[loop_layer] != curOffset) {
          ...
          loop_counter[loop_layer]++;
                   ...
                   loop_repeats[loop_layer])  // need to repeat loop, not first
                                              // time
        {
          loop_counter[loop_layer]++;
          curOffset = loop_begin_loc[loop_layer];
        ...
        } else  // loop counter exhausted, end of loop
        {
          loopID[loop_layer] = 0;
          loop_layer--;
          if (loop_counter[loop_layer] == 0) bInLoop = false;
        }
        break;
      ...
      case 0x9A:
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat Break",
                        L"", CLR_LOOP);
        // AddEventItem("Loop Break", ICON_ENDREP, beginOffset,
        // curOffset-beginOffset, BG_CLR_WHEAT);
        ...
        if (loop_counter[loop_layer] == loop_repeats[loop_layer]) {
          ...
          octave =
              loop_end_octave[loop_layer];  // set base_key to loop end base key
          loopID[loop_layer] = 0;
          loop_layer--;
          if (loop_counter[loop_layer] == 0) bInLoop = false;
        }
      ...
      case 0xA2: {
        ...
        uint8_t cTempoSlideTarget =
            (GetByte(curOffset++) * 256) / 218;  // Target Panpot
      ...
      case 0xBB:
        ...
        // ADSR Reset
      case 0xC0:
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR: Reset",
                        L"", CLR_ADSR);
        break;
      ...
      case 0xC2:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"ADSR: Attack Rate?", L"", CLR_ADSR);
        break;

      case 0xC4:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"ADSR: Sustain Rate?", L"", CLR_ADSR);
        break;

      case 0xC5:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"ADSR: Release Rate?", L"", CLR_ADSR);
        break;
      ...
      case 0xC9:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"ADSR: Decay Rate?", L"", CLR_ADSR);
        break;

      case 0xCA:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"ADSR: Sustain Level?", L"", CLR_ADSR);
        break;
      ...
      case 0xD2:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Bend?",
                        L"", CLR_PITCHBEND);
        break;
      ...
      case 0xD7: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO Depth (Pitch bend)", L"", CLR_LFO);
        break;
      }
      ...
      case 0xD8: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO Length (Pitch bend)", L"", CLR_LFO);
        break;
      }
      ...
      case 0xE5:
        ...
        curOffset++;
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"LFO ? (Volume)",
                        L"", CLR_LFO);
        break;
      ...
      case 0xED:
        ...
        curOffset++;
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"LFO ? (Panpot)",
                        L"", CLR_LFO);
        break;
      ...
      case 0xFE:
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Program bank select", L"", CLR_PROGCHANGE);
        break;
    }
  return true;
}
