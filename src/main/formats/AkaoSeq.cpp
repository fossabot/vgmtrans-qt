...
AkaoSeq::AkaoSeq(RawFile *file, uint32_t offset)
  ...
  UseLinearAmplitudeScale();  // I think this applies, but not certain, see FF9
                              // 320, track 3 for example of problem
...
uint8_t AkaoSeq::GetNumPositiveBits(uint32_t ulWord) {
  return ((ulWord & 0x80000000) > 0) + ((ulWord & 0x40000000) > 0) +
         ((ulWord & 0x20000000) > 0) + ((ulWord & 0x10000000) > 0) +
         ((ulWord & 0x8000000) > 0) + ((ulWord & 0x4000000) > 0) +
         ((ulWord & 0x2000000) > 0) + ((ulWord & 0x1000000) > 0) +
         ((ulWord & 0x800000) > 0) + ((ulWord & 0x400000) > 0) +
         ((ulWord & 0x200000) > 0) + ((ulWord & 0x100000) > 0) +
         ((ulWord & 0x80000) > 0) + ((ulWord & 0x40000) > 0) +
         ((ulWord & 0x20000) > 0) + ((ulWord & 0x10000) > 0) +
         ((ulWord & 0x8000) > 0) + ((ulWord & 0x4000) > 0) +
         ((ulWord & 0x2000) > 0) + ((ulWord & 0x1000) > 0) +
         ((ulWord & 0x800) > 0) + ((ulWord & 0x400) > 0) +
         ((ulWord & 0x200) > 0) + ((ulWord & 0x100) > 0) +
...
bool AkaoSeq::GetHeaderInfo(void) {
  // first do a version check to see if it's older or newer version of AKAO
  // sequence format
  ...
  // There must be either a melodic instrument section, a drumkit, or both.  We
  // determiine  the start of the InstrSet based on whether a melodic instrument
  // section is given.
  uint32_t instrOff = GetWord(dwOffset + 0x30);
  uint32_t drumkitOff = GetWord(dwOffset + 0x34);
  if (instrOff != 0) instrOff += 0x30 + dwOffset;
  if (drumkitOff != 0) drumkitOff += 0x34 + dwOffset;
  uint32_t instrSetLength;
  if (instrOff != 0)
    instrSetLength = unLength - (instrOff - dwOffset);
  else
    instrSetLength = unLength - (drumkitOff - dwOffset);
  instrset = new AkaoInstrSet(rawfile, instrSetLength, instrOff, drumkitOff, id,
                              L"Akao Instr Set");
...
bool AkaoSeq::GetTrackPointers(void) {
  for (unsigned int i = 0; i < nNumTracks; i++)
    aTracks.push_back(new AkaoTrack(
        this, GetShort(dwOffset + 0x40 + (i * 2)) + i * 2 + 0x40 + dwOffset));
  return true;
}
...
//--------------------------------------------------
// Revisions:
//	2009. 6.17(Wed.) :	Re-make by "Sound tester 774" in
//"内蔵音源をMIDI変換するスレ(in http://www.2ch.net)"
...
bool AkaoTrack::ReadEvent(void) {
  ...
  if (status_byte <
      0x9A)  // it's either a  note-on message, a tie message, or a rest message
  {
    // looking at offset 8005E5C8 in the FFO FF2 exe for this delta time table
    // code  4294967296  0x100000000/0xBA2E8BA3   = 1.374999... = 11/8  0x68 after
    // eq = 9
    /*i = ((status_byte * 0xBA2E8BA3) & 0xFF00000000) >> 32; // emulating multu
    and mfhi instruction
    //i >>= 3;  //srl 3 */
    i = status_byte / 11;
    ...
    if (status_byte < 0x83)  // it's a note-on message
    {
      ...
      AddNoteOn(beginOffset, curOffset - beginOffset, base_key + relative_key,
                vel);
      bNotePlaying = true;

      AddTime(delta_time_table[k]);
    } else if (status_byte <
               0x8F)  // if it's between 0x83 and 0x8E it is a tie event
    {
      AddTime(delta_time_table[k]);
      if (loop_counter[loop_layer] == 0 && loop_layer == 0)  // do this so we
                                                             // don't repeat this
                                                             // for every single
                                                             // time it loops
    ...
    bNotePlaying = true;
    AddNoteOn(beginOffset, curOffset - beginOffset + 1, base_key + relative_key,
              vel);
    ...
    switch (status_byte) {
      ...
      case 0xA1: {
        ...
        uint8_t progNum = (uint8_t)(
            ((AkaoSeq *)parentSeq)->instrset->aInstrs.size() + artNum);
      ...
      case 0xA2:
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Next note length", L"", CLR_CHANGESTATE);
        break;
      ...
      case 0xA4: {
        ...
        uint8_t steps = GetByte(curOffset++);  // second byte is number of
                                               // halfsteps to slide... not sure
                                               // if signed or not, only seen
                                               // positive
        // AddPitchBendSlide(
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Portamento",
                        L"", CLR_PORTAMENTO);
        break;
      }
      ...
      case 0xA8: {
        ...
        // vel = Convert7bitPercentVolValToStdMidiVal(vel);		//I THINK
        // THIS APPLIES, BUT NOT POSITIVE  AddGenericEvent(beginOffset,
        // curOffset-beginOffset, L"Set Velocity", NULL, BG_CLR_CYAN);
      ...
      case 0xAC:  // unknown
        ...
        <CBongo> B1 = set sustain release  B2 = set release
         ...
         B3 appears to be "reset ADSR to the initial values from the
        instrument/sample data"*/

      case 0xAD:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR", L"",
                        CLR_ADSR);
        break;
      case 0xAE:
        curOffset++;
      ...
      case 0xB5:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Pitch bend) Depth", L"", CLR_LFO);
        break;
      case 0xB6:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Pitch bend) off", L"", CLR_LFO);
        break;
      case 0xB7:
        curOffset++;
        AddUnknown(beginOffset, curOffset - beginOffset);
      ...
      case 0xB9:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Expression) Depth", L"", CLR_LFO);
        break;
      case 0xBA:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Expression) off", L"", CLR_LFO);
        break;
      case 0xBB:
        curOffset++;
        AddUnknown(beginOffset, curOffset - beginOffset);
      ...
      case 0xBC:
        curOffset += 2;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Panpot) Length, cycle", L"", CLR_LFO);
        break;
      case 0xBD:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Panpot) Depth", L"", CLR_LFO);
        break;
      case 0xBE:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Panpot) off", L"", CLR_LFO);
        break;
      case 0xBF:
        curOffset++;
        AddUnknown(beginOffset, curOffset - beginOffset);
      ...
      case 0xC1: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Transpose move",
                        L"", CLR_TRANSPOSE);
        break;
      }
      ...
      case 0xC2:
        ...
        break;
      case 0xC3:
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Reverb Off",
                        L"", CLR_REVERB);
        break;
      ...
      case 0xC6:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"FM Modulation On", L"", CLR_UNKNOWN);
        break;
      case 0xC7:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"FM Modulation Off", L"", CLR_UNKNOWN);
        break;

      // set loop begin marker
      case 0xC8:
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat Start",
                        L"", CLR_LOOP);
        loop_begin_layer++;
      ...
      case 0xC9: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Repeat End",
                        L"", CLR_LOOP);

        // if loop_begin_layer == 0, then there was never a matching loop begin
        // event!  this is seen in ff9 402 and ff9 hunter's chance
        if (loop_begin_layer == 0) break;
      ...
      case 0xD1:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Deactivate Notes?", L"", CLR_UNKNOWN);
        break;
      ...
      // pitch bend
      case 0xD8: {
        // signed data byte.  range of 1 octave (0x7F = +1 octave 0x80 = -1
        // octave)
      ...
      case 0xD9:
        curOffset++;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Pitch bend move", L"", CLR_UNKNOWN);
        break;
      ...
      case 0xDD:
        curOffset += 2;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Pitch bend) times", L"", CLR_UNKNOWN);
        break;

      case 0xDE:
        curOffset += 2;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Expression) times", L"", CLR_UNKNOWN);
        break;

      case 0xDF:
        curOffset += 2;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"LFO(Panpot) times", L"", CLR_UNKNOWN);
        break;
      ...
      case 0xFC:
        ...
        AddTime(GetByte(curOffset++));
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Tie (custom)",
                        L"", CLR_TIE);
        break;
      ...
      case 0xFE:
        ...
        switch (GetByte(curOffset++)) {
          ...
          case 0x00: {
            ...
            double dValue1 =
                ((value2 << 8) + value1) / 218.4555555555555555555555555;
          ...
          case 0x03:
            ...
            curOffset++;
            AddGenericEvent(beginOffset, curOffset - beginOffset,
                            L"Reverb Fade", L"", CLR_REVERB);
            break;

          // signals this track uses the drum kit
          case 0x04:
            AddProgramChange(beginOffset, curOffset - beginOffset, 127,
                             L"Drum kit On");
            // channel = 9;
            break;
          ...
          case 0x06: {
            ...
            // Check the remaining area that will be processed by CPU-controlled
            // jump.
            for (vector<uint32_t>::iterator itAddr = vCondJumpAddr.begin();
                 itAddr != vCondJumpAddr.end(); ++itAddr) {
              if (!IsOffsetUsed(*itAddr)) {
                // There is an area not visited yet, VGMTrans must try to go
                // there.
                bContinue = true;
                break;
              }
            ...
            if (nScanMode == MODE_CONVERT_MIDI)		//if we are converting
            the midi, the actually branch
            {
                ...
                curOffset += 2;
            //otherwise, just skip over the relative branch offset*/
            break;
          }
          ...
          case 0x07: {
            ...
            // This event performs conditional jump if certain CPU variable
            // matches to the condValue. VGMTrans will simply try to parse all
            // events as far as possible, instead. (Test case: FF9 416 Final
            // Battle)
            ...
            AddGenericEvent(beginOffset, eventLength, L"Dal Segno (Loop) Break",
                            L"", CLR_LOOP);
            break;
          }
          ...
          case 0x15: {
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset,
                            L"Time Signature", L"", CLR_TIMESIG, ICON_TIMESIG);
            break;
          }
          ...
          default:
            AddGenericEvent(beginOffset, curOffset - beginOffset,
                            L"UNKNOWN META EVENT", L"", CLR_UNRECOGNIZED);
            break;
        }
        break;

      default:
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"UNKNOWN EVENT",
                        L"", CLR_UNRECOGNIZED);
        break;
    }
  return true;
