...
void NamcoSnesScanner::SearchForNamcoSnesFromARAM(RawFile *file) {
  ...
  if (file->SearchBytePattern(ptnReadSongList, ofsReadSongList)) {
    addrSongList = file->GetByte(ofsReadSongList + 5) |
                   (file->GetByte(ofsReadSongList + 9) << 8);
  ...
  if (addrSeqHeader + 3 < 0x10000) {
    if (file->GetByte(addrSeqHeader) > 3 ||
        (file->GetShort(addrSeqHeader + 1) & 0xff00) == 0) {
      songIndex = 1;
    }
  ...
  if (file->SearchBytePattern(ptnLoadInstrTuning, ofsLoadInstrTuning)) {
    addrTuningTable = file->GetByte(ofsLoadInstrTuning + 8) |
                      (file->GetByte(ofsLoadInstrTuning + 12) << 8);
  } else {
    return;
  }
