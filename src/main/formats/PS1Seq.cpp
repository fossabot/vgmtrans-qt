...
PS1Seq::PS1Seq(RawFile *file, uint32_t offset)
  ...
  UseReverb();
  // bWriteInitialTempo = false; // false, because the initial tempo is added by
  // tempo event
}
...
bool PS1Seq::GetHeaderInfo(void) {
  ...
  seqHeader->AddSig(offset() + 0x0D,
                    2);  // Rhythm (Numerator) and Rhythm (Denominator) (2^n)

  if (GetByte(offset() + 0xF) == 0 && GetByte(offset() + 0x10) == 0) {
    SetEventsOffset(offset() + 0x0F + 4);
    PS1Seq *newPS1Seq =
        new PS1Seq(rawfile, offset() + GetShortBE(offset() + 0x11) + 0x13 - 6);
...
void PS1Seq::ResetVars(void) {
  ...
  uint32_t initialTempo =
      (GetShortBE(offset() + 10) << 8) | GetByte(offset() + 12);
  ...
  switch (status_byte & 0xF0) {
    ...
    case 0xB0: {
      ...
      switch (controlNum) {
        // bank select
        case 0:
          AddGenericEvent(beginOffset, curOffset - beginOffset, L"Bank Select",
                          L"", CLR_MISC);
        ...
        case 100:
          AddGenericEvent(beginOffset, curOffset - beginOffset, L"RPN 1", L"",
                          CLR_MISC);
          if (VGMSeq::readMode == READMODE_CONVERT_TO_MIDI) {
            pMidiTrack->AddControllerEvent(channel, controlNum, value);
          }
        ...
        case 101:
          AddGenericEvent(beginOffset, curOffset - beginOffset, L"RPN 2", L"",
                          CLR_MISC);
          if (VGMSeq::readMode == READMODE_CONVERT_TO_MIDI) {
            pMidiTrack->AddControllerEvent(channel, controlNum, value);
          }
    ...
    case 0xF0: {
      if (status_byte == 0xFF) {
        switch (GetByte(curOffset++)) {
          // tempo.  This is different from SMF, where we'd expect a 51 then 03.
          // Also, supports a string of tempo events
          case 0x51:
