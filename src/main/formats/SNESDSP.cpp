...
// Emulate GAIN envelope while (increase: env < env_to, or decrease: env >
// env_to) return elapsed time in sample count, and final env value if
// requested.
uint32_t EmulateSDSPGAIN(uint8_t gain, int16_t env_from, int16_t env_to,
  ...
  if (sf2_envelope_time_ptr != NULL) {
    ...
    if (mode < 4) {  // direct
      ...
      sf2_time =
          LinAmpDecayTimeToLinDBDecayTime(total_samples_full / 32000.0, 0x800);
    } else if (mode == 5) {  // 5: exponential decrease
      ...
      if (tick == 0) {
        ...
        if (env_from > 255) {
          // exponential part
          double decibelAtStart =
              ConvertPercentAmplitudeToAttenDB(env_from / 2047.0);
          double decibelAtExpFinal =
              ConvertPercentAmplitudeToAttenDB(env_exp_final / 2047.0);
          double timeAtExpFinal =
              (tick_exp * SDSP_COUNTER_RATES[rate]) / 32000.0;
          sf2_time =
              timeAtExpFinal * (-100.0 / (decibelAtExpFinal - decibelAtStart));
        } else {
          ...
          if (env == 0 && tick == 1) {
            ...
            double timeAtExpFinal =
                (tick_total * SDSP_COUNTER_RATES[rate]) / 32000.0;
            sf2_time =
                timeAtExpFinal * (-100.0 / (decibelAtFinal - decibelAtStart));
          }
          ...
          // double decibelDiff = ConvertPercentAmplitudeToAttenDB(env_start /
          // 2047.0) - ConvertPercentAmplitudeToAttenDB((env_start + 1) /
          // 2047.0);  double timePerTick = SDSP_COUNTER_RATES[rate] / 32000.0;
      ...
      uint32_t total_samples_full = (0x800 / 0x20) * SDSP_COUNTER_RATES[rate];
      sf2_time =
          total_samples_full / 32000.0;  // linear attack time from 0 to full
    }
...
void ConvertSNESADSR(uint8_t adsr1, uint8_t adsr2, uint8_t gain,
                     uint16_t env_from, double *ptr_attack_time,
                     double *ptr_decay_time, double *ptr_sustain_level,
  ...
  if (adsr_enabled) {
    ...
    if (sr == 0) {
      ...
      EmulateSDSPGAIN(0xa0 | sr, env, 0, &env_after,
                      &sustain_time);  // exponential decrease
    }
    ...
    if (mode < 4) {  // direct
      ...
      if (mode >= 6) {
        ...
        samples = (env_to + 7) / 8;
        release_time =
            LinAmpDecayTimeToLinDBDecayTime(samples / 32000.0, 0x7ff);
      } else {
        attack_time = 0.0;
...
SNESSampColl::SNESSampColl(const std::string &format, RawFile *rawfile,
                           uint32_t offset, uint32_t maxNumSamps)
    : VGMSampColl(format, rawfile, offset, 0), spcDirAddr(offset) {
  SetDefaultTargets(maxNumSamps);
}

SNESSampColl::SNESSampColl(const std::string &format, VGMInstrSet *instrset,
                           uint32_t offset, uint32_t maxNumSamps)
...
SNESSampColl::SNESSampColl(const std::string &format, RawFile *rawfile,
                           uint32_t offset,
                           const std::vector<uint8_t> &targetSRCNs,
                           std::wstring name)
...
bool SNESSampColl::GetSampleInfo() {
  ...
  for (std::vector<uint8_t>::iterator itr = this->targetSRCNs.begin();
    ...
    bool loop;
    uint32_t length =
        SNESSamp::GetSampleLength(GetRawFile(), addrSampStart, loop);
    ...
    name.str(L"");
    name << L"Sample " << srcn;
    SNESSamp *samp = new SNESSamp(this, addrSampStart, length, addrSampStart,
                                  length, addrSampLoop, name.str());
...
SNESSamp::SNESSamp(VGMSampColl *sampColl, uint32_t offset, uint32_t length,
                   uint32_t dataOffset, uint32_t dataLen, uint32_t loopOffset,
                   std::wstring name)
    : VGMSamp(sampColl, offset, length, dataOffset, dataLen, 1, 16, 32000,
              name),
...
static inline int32_t absolute(int32_t x) { return ((x < 0) ? -x : x); }
...
void SNESSamp::DecompBRRBlk(int16_t *pSmp, BRRBlk *pVBlk, int32_t *prev1,
                            int32_t *prev2) {
  int32_t out, S1, S2;
