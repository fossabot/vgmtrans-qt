...
// 08ff: 88 00     adc   a,#$00
// 0901: c4 55     mov   $55,a             ; set percussion table address to
// $54/5
...
void CompileSnesScanner::SearchForCompileSnesFromARAM(RawFile *file) {
  ...
  if (addrSongHeaderPtr + 2 <= 0x10000) {
    ...
    if (numTracks > 0 && numTracks <= 8) {
      CompileSnesSeq *newSeq =
          new CompileSnesSeq(file, version, addrSongHeader, name);
  ...
  CompileSnesInstrSet *newInstrSet = new CompileSnesInstrSet(
      file, version, addrTuningTable, addrPitchTablePtrs, spcDirAddr);
