...
bool TriAcePS1InstrSet::GetHeaderInfo() {
  VGMHeader *header = AddHeader(
      dwOffset, sizeof(TriAcePS1InstrSet::_InstrHeader));  // 1,Sep.2009 revise
  ...
  //-----------------------
  //	GetBytes(dwOffset, sizeof(TriAcePS1InstrSet::_InstrHeader),
  //&InstrHeader); 		↑　↑　↑
  unLength = GetWord(dwOffset);               // to do delete
  instrSectionSize = GetShort(dwOffset + 4);  // to do delete
  //-----------------------

  // sampColl = new TriAcePS1SampColl(this, dwOffset+instrSectionSize,
  // unLength-instrSectionSize);
  sampColl =
      new PSXSampColl(TriAcePS1Format::name, this, dwOffset + instrSectionSize,
                      unLength - instrSectionSize);
...
bool TriAcePS1InstrSet::GetInstrPointers() {
  uint32_t firstWord = GetWord(
      dwOffset + sizeof(TriAcePS1InstrSet::_InstrHeader));  // 1,Sep.2009 revise

  // 0xFFFFFFFFになるまで繰り返す。
  for (uint32_t
           i = dwOffset +
               sizeof(TriAcePS1InstrSet::_InstrHeader);  // 1,Sep.2009 revise
    ...
    newInstr->AddSimpleItem(i + 0, sizeof(short),
                            L"Instrument Number");  // 1,Sep.2009 revise
    newInstr->AddSimpleItem(i + 2, sizeof(short),
                            L"ADSR1");  // 1,Sep.2009 revise
    newInstr->AddSimpleItem(i + 4, sizeof(short),
                            L"ADSR2");              // 1,Sep.2009 revise
    newInstr->AddUnknownItem(i + 6, sizeof(char));  // 1,Sep.2009 revise
    newInstr->AddSimpleItem(i + 7, sizeof(char),
                            L"Number of Rgns");  // 1,Sep.2009 revise
...
TriAcePS1Instr::TriAcePS1Instr(VGMInstrSet *instrSet, uint32_t offset,
...
bool TriAcePS1Instr::LoadInstr() {
  ...
  //	TriAcePS1InstrSet*	_parInstrSet	=
  //(TriAcePS1InstrSet*)parInstrSet;

  // Get the rgn data
  rgns = new RgnInfo[instrinfo.numRgns];
  GetBytes((dwOffset + sizeof(InstrInfo)),
           (sizeof(RgnInfo) * instrinfo.numRgns), rgns);

  // Set the program num and bank num, adjusting for values > 0x7F
  this->instrNum = instrinfo.progNum & 0x7F;  // 1,Sep.2009 revise
  this->bank = ((instrinfo.progNum + (instrinfo.bankNum << 8)) & 0xFF80) >>
               7;  // 1,Sep.2009 revise
  ...
  for (int i = 0; i < instrinfo.numRgns; i++) {
    ...
    VGMRgn *rgn =
        new VGMRgn(this, dwOffset + sizeof(InstrInfo) + sizeof(RgnInfo) * i,
    ...
    rgn->sampOffset =
        rgninfo
            ->sampOffset;  //+
                           //((VGMInstrSet*)this->vgmfile)->sampColl->dwOffset;
    ...
    rgn->AddUnityKey(
        (int8_t)0x3B - rgninfo->pitchTuneSemitones,
    ...
    // Attenuation is not using a linear scale. Will need to look at the
    // original code.  For now, it's ignored.
    //  also, be aware the same scale may be employed for vol and expression
    //  events (haven't investigated).
    // long dlsAtten =
    // -(ConvertPercentVolToAttenDB(rgninfo->attenuation/((double)255)) *
