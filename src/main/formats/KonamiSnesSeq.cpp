...
const uint8_t KonamiSnesSeq::PAN_TABLE[] = {
    0x00, 0x04, 0x08, 0x0e, 0x14, 0x1a, 0x20, 0x28, 0x30, 0x38, 0x40,
...
KonamiSnesSeq::KonamiSnesSeq(RawFile *file, KonamiSnesVersion ver,
...
bool KonamiSnesSeq::GetHeaderInfo(void) {
  ...
  VGMHeader *seqHeader =
      AddHeader(dwOffset, nNumTracks * 2, L"Sequence Header");
  for (uint32_t trackNumber = 0; trackNumber < nNumTracks; trackNumber++) {
    ...
    seqHeader->AddPointer(trackPointerOffset, 2, trkOff, true,
                          L"Track Pointer");
...
double KonamiSnesSeq::GetTempoInBPM() { return GetTempoInBPM(tempo); }
...
KonamiSnesTrack::KonamiSnesTrack(KonamiSnesSeq *parentFile, long offset,
                                 long length)
  ...
  switch (eventType) {
    ...
    case EVENT_UNKNOWN5: {
      ...
      desc << L"Event: 0x" << std::hex << std::setfill(L'0') << std::setw(2)
           ...
           << std::setfill(L' ') << std::setw(0) << L"  Arg1: " << (int)arg1
           << L"  Arg2: " << (int)arg2 << L"  Arg3: " << (int)arg3
           << L"  Arg4: " << (int)arg4 << L"  Arg5: " << (int)arg5;
      AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                 desc.str().c_str());
      break;
    }
    ...
    case EVENT_PERCUSSION_OFF: {
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Percussion Off",
                      desc.str().c_str(), CLR_CHANGESTATE);
      if (percussion) {
        AddProgramChange(beginOffset, curOffset - beginOffset, instrument,
                         true);
        percussion = false;
      }
      break;
    ...
    case EVENT_GAIN: {
      ...
      desc << L"GAIN: " << (int)newGAINAmount << L" ($" << std::hex
           << std::setfill(L'0') << std::setw(2) << std::uppercase
           << (int)newGAIN << L")";
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"GAIN",
    ...
    case EVENT_PAN: {
      ...
      if (instrumentPanOff) {
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Per-Instrument Pan Off", desc.str().c_str(), CLR_PAN,
                        ICON_CONTROL);
      } else if (instrumentPanOn) {
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Per-Instrument Pan On", desc.str().c_str(), CLR_PAN,
                        ICON_CONTROL);
      } else {
        uint8_t volumeLeft;
        uint8_t volumeRight;
    ...
    case EVENT_TEMPO_FADE: {
      ...
      desc << L"BPM: " << parentSeq->GetTempoInBPM(newTempo)
           << L"  Fade Length: " << (int)fadeSpeed;
    ...
    case EVENT_ADSR1: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR(1)",
                      desc.str().c_str(), CLR_ADSR, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_ADSR2: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR(2)",
                      desc.str().c_str(), CLR_ADSR, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_VOLUME_FADE: {
      uint8_t newVolume = GetByte(curOffset++);
      uint8_t fadeSpeed = GetByte(curOffset++);
      desc << L"Volume: " << (int)newVolume << L"  Fade Length: "
           << (int)fadeSpeed;
    ...
    case EVENT_PITCH_ENVELOPE_V1: {
      ...
      desc << L"Delay: " << (int)pitchEnvDelay << L"  Speed: "
           << (int)pitchEnvSpeed << L"  Depth: " << (int)-pitchEnvDepth;
    ...
    case EVENT_PITCH_ENVELOPE_V2: {
      ...
      desc << L"Delay: " << (int)pitchEnvDelay << L"  Length: "
           << (int)pitchEnvLength << L"  Offset: " << (int)-pitchEnvOffset
           << L" semitones" << L"  Delta: " << (pitchDelta / 256.0)
           << L" semitones";
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Envelope",
                      desc.str().c_str(), CLR_MODULATION, ICON_CONTROL);
      break;
    ...
    case EVENT_PITCH_SLIDE_V2: {
      ...
      if (arg2 != 0) {
        ...
        desc << L"Arg4: " << (int)arg4 << L"  Arg5: " << (int)arg5
             << L"  Arg6: " << (int)arg6;
      }
    ...
    case EVENT_PITCH_SLIDE_V3: {
      ...
      desc << L"Delay: " << (int)pitchSlideDelay << L"  Length: "
           << (int)pitchSlideLength << L"  Final Note: "
           << (int)pitchSlideNoteNumber << L"  Delta: " << (pitchDelta / 256.0)
           << L" semitones";
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Slide",
                      desc.str().c_str(), CLR_PITCHBEND, ICON_CONTROL);
      break;
    ...
    case EVENT_ECHO: {
      ...
      desc << L"EON: " << (int)echoChannels << L"  EVOL(L): "
           << (int)echoVolumeL << L"  EVOL(R): " << (int)echoVolumeR;
    ...
    case EVENT_ECHO_PARAM: {
      ...
      desc << L"EDL: " << (int)echoDelay << L"  EFB: " << (int)echoFeedback
           << L"  Arg3: " << (int)echoArg3;
    ...
    case EVENT_LOOP_WITH_VOLTA_START: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Loop With Volta Start", desc.str().c_str(), CLR_LOOP,
                      ICON_STARTREP);
    ...
    case EVENT_LOOP_WITH_VOLTA_END: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Loop With Volta End", desc.str().c_str(), CLR_LOOP,
                      ICON_STARTREP);
      ...
      if (voltaEndMeansPlayFromStart) {
        ...
        // third time - start of first volta bracket: play the new bracket (or
        // quit from the entire loop)
    ...
    case EVENT_ADSR_GAIN: {
      ...
      desc << L"ADSR(1): $" << std::hex << std::setfill(L'0') << std::setw(2)
           << std::uppercase << (int)newADSR1 << L"  ADSR(2): $"
           << (int)newADSR2 << L"  GAIN: $" << (int)newGAIN;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR(2)",
                      desc.str().c_str(), CLR_ADSR, ICON_CONTROL);
      break;
    }
