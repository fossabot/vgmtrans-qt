...
bool HOSAInstrSet::GetHeaderInfo() {
  ...
  wdsHeader->AddSimpleItem(dwOffset + 8, sizeof(uint32_t),
                           L"Number of Instruments");

  //波形objectの生成
  //	sampColl = new PSXSampColl(HOSAFormat::name, this, 0x00160800); // moved
  //to HOSAScanner 	sampColl->Load();
...
bool HOSAInstrSet::GetInstrPointers() {
  uint32_t iOffset =
      dwOffset + sizeof(InstrHeader);  // pointer of attribute table
...
//		Constructor
//--------------------------------------------------------------
HOSAInstr::HOSAInstr(VGMInstrSet *instrSet, uint32_t offset, uint32_t length,
...
bool HOSAInstr::LoadInstr() {
  ...
  for (unsigned int i = 0; i < instrinfo.numRgns; i++) {
    ...
    rgn->AddUnityKey((int8_t)0x3C + 0x3C - rgninfo->iSemiToneTune,
                     rgn->dwOffset + 0x06);
    ...
    // The ADSR value ordering is all messed up for the hell of it.  This was a
    // bitch to reverse-engineer.
    rgn->AddSimpleItem(rgn->dwOffset + 0x0C, 4,
                       L"ADSR Values (non-standard ordering)");
