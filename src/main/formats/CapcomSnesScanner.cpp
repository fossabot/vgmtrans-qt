...
// 03fa: c4 c0     mov   $c0,a
// 03fc: f5 02 0e  mov   a,$0e02+x         ; read song header address from song
// list 03ff: c4 c1     mov   $c1,a 0401: 04 c0     or    a,$c0 0403: f0 dd
// beq   $03e2             ; return if song header address == $0000
...
// 05a6: 8f af a0  mov   $a0,#$af          ; song address = $0daf (+1 for actual
// start address) ; fall through... ; or when song header first byte == 0: 05a9:
// 3f 82 05  call  $0582 05ac: 8d 00     mov   y,#$00 05ae: dd        mov   a,y
...
//; Mega Man X SPC
// 0451: 8d 03     mov   y,#$03
// 0453: f6 63 04  mov   a,$0463+y         ; DSP reg initialize table -1
// (address) 0456: c5 f2 00  mov   $00f2,a 0459: f6 66 04  mov   a,$0466+y
// ; DSP reg initialize table -1 (value) 045c: c5 f3 00  mov   $00f3,a
// ; initialize DSP reg 045f: fe f2     dbnz  y,$0453
...
// 097d: 98 47 a1  adc   $a1,#$47          ; $a0 = instrument entry address
// (0x47ac + (patch * 6))
...
void CapcomSnesScanner::SearchForCapcomSnesFromARAM(RawFile *file) {
  ...
  if (hasSongList) {
    addrSongList = min(file->GetShort(ofsReadSongListASM + 3),
                       file->GetShort(ofsReadSongListASM + 8));
  }

  // find BGM address
  bgmAtFixedAddress =
      file->SearchBytePattern(ptnReadBGMAddress, ofsReadBGMAddressASM);
  ...
  if (bgmAtFixedAddress) {
    CapcomSnesSeq *newSeq =
        new CapcomSnesSeq(file, version, addrBGMHeader + 1, false, name);
    ...
    if (loadOnlyCurrentSong) {
      ...
      if (guessedSongIndex != -1) {
        uint16_t addrSongHeader =
            file->GetShortBE(addrSongList + guessedSongIndex * 2);
        CapcomSnesSeq *newSeq =
      ...
      for (int songIndex = 0; songIndex < songListLength; songIndex++) {
        uint16_t addrSongHeader =
            file->GetShortBE(addrSongList + songIndex * 2);
        if (addrSongHeader == 0) {
          continue;
        }

        CapcomSnesSeq *newSeq = new CapcomSnesSeq(
            file, version, addrSongHeader, true,
  ...
  CapcomSnesInstrSet *newInstrSet =
      new CapcomSnesInstrSet(file, addrInstrTable, spcDIR << 8);
...
uint16_t CapcomSnesScanner::GetCurrentPlayAddressFromARAM(
    RawFile *file, CapcomSnesVersion version, uint8_t channel) {
  uint16_t currentAddress;
  if (version == CAPCOMSNES_V1_BGM_IN_LIST) {
    currentAddress = file->GetByte(0x00 + channel * 2 + 1) |
                     (file->GetByte(0x10 + channel * 2 + 1) << 8);
  } else {
    currentAddress =
        file->GetByte(0x00 + channel) | (file->GetByte(0x08 + channel) << 8);
  }
  return currentAddress;
}

int CapcomSnesScanner::GetLengthOfSongList(RawFile *file,
...
int8_t CapcomSnesScanner::GuessCurrentSongFromARAM(RawFile *file,
  ...
  for (int songIndex = 0; songIndex < songListLength; songIndex++) {
    ...
    for (int track = 0; track < 8; track++) {
      ...
      uint16_t currentAddress =
          this->GetCurrentPlayAddressFromARAM(file, version, 7 - track);
...
bool CapcomSnesScanner::IsValidBGMHeader(RawFile *file,
                                         uint32_t addrSongHeader) {
...
std::map<uint8_t, uint8_t> CapcomSnesScanner::GetInitDspRegMap(RawFile *file) {
  ...
  } else if (file->SearchBytePattern(ptnDspRegInitOldVer,
                                     ofsDspRegInitOldVerASM)) {
  ...
  // check address range
  if (addrDspRegList + dspRegCount > 0x10000 ||
      addrDspValueList + dspRegCount > 0x10000) {
    return dspRegMap;
  }
