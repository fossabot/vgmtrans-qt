...
PrismSnesInstrSet::PrismSnesInstrSet(RawFile *file, PrismSnesVersion ver,
                                     uint32_t spcDirAddr,
...
bool PrismSnesInstrSet::GetHeaderInfo() { return true; }
...
bool PrismSnesInstrSet::GetInstrPointers() {
  ...
  for (uint16_t srcn16 = 0; srcn16 < 0x100; srcn16++) {
    ...
    PrismSnesInstr *newInstr = new PrismSnesInstr(
        this, version, srcn, spcDirAddr, ofsADSR1Entry, ofsADSR2Entry,
  ...
  SNESSampColl *newSampColl = new SNESSampColl(
      PrismSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
PrismSnesInstr::PrismSnesInstr(VGMInstrSet *instrSet, PrismSnesVersion ver,
                               uint8_t srcn, uint32_t spcDirAddr,
                               uint16_t addrADSR1Entry, uint16_t addrADSR2Entry,
...
bool PrismSnesInstr::LoadInstr() {
  ...
  PrismSnesRgn *rgn =
      new PrismSnesRgn(this, version, srcn, spcDirAddr, addrADSR1Entry,
...
PrismSnesRgn::PrismSnesRgn(PrismSnesInstr *instr, PrismSnesVersion ver,
                           uint8_t srcn, uint32_t spcDirAddr,
                           uint16_t addrADSR1Entry, uint16_t addrADSR2Entry,
                           uint16_t addrTuningEntryHigh,
                           uint16_t addrTuningEntryLow)
  ...
  int16_t tuning =
      GetByte(addrTuningEntryLow) | (GetByte(addrTuningEntryHigh) << 8);
  ...
  // put a random release time, it would be better than plain key off (actual
  // music engine never do key off)
  release_time =
      LinAmpDecayTimeToLinDBDecayTime(0.002 * SDSP_COUNTER_RATES[0x14], 0x7ff);
...
bool PrismSnesRgn::LoadRgn() { return true; }
