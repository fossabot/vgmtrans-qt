...
BytePattern HudsonSnesScanner::ptnNoteLenTable(
    "\xc0\x60\x30\x18\x0c\x06\x03\x01", "xxxxxxxx", 8);
...
// 08d6: da 13     movw  $13,ya            ; set music structure address from
// $07c2/3 to $13/4  08d8: e5 c4 07  mov   a,$07c4  08db: ec c5 07  mov   y,$07c5
...
// 11af: d5 5a 02  mov   $025a+x,a         ; default global loop point = song
// start
...
// 0bfa: 8d 5d     mov   y,#$5d
// 0bfc: 4f 0c     pcall $0c
BytePattern HudsonSnesScanner::ptnLoadDIRV0("\xe4\x4b\x8d\x5d\x4f\x0c",
                                            "x?xxxx", 6);
...
void HudsonSnesScanner::SearchForHudsonSnesFromARAM(RawFile *file) {
  ...
  } else if (file->SearchBytePattern(ptnGetSeqTableAddrV0,
                                     ofsGetSeqTableAddr)) {
  ...
  if (file->SearchBytePattern(ptnLoadTrackAddress, ofsLoadTrackAddress)) {
    uint16_t addrCurrentLoopPointPtrLo =
        file->GetShort(ofsLoadTrackAddress + 20);
    uint16_t addrCurrentLoopPointPtrHi =
        file->GetShort(ofsLoadTrackAddress + 29);
  ...
  if (newSeq->InstrumentTableSize != 0) {
    ...
    HudsonSnesInstrSet *newInstrSet = new HudsonSnesInstrSet(
        file, version, newSeq->InstrumentTableAddress,
        newSeq->InstrumentTableSize, spcDirAddr, addrSampTuningTable, name);
