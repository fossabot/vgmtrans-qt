...
//     - Switchable samples
//     - Percussive samples
// KonamiSnesInstrSet tries to load all these samples to merge them into a
// single DLS.
KonamiSnesInstrSet::KonamiSnesInstrSet(
    RawFile *file, KonamiSnesVersion ver, uint32_t offset,
    uint32_t bankedInstrOffset, uint8_t firstBankedInstr,
    uint32_t percInstrOffset, uint32_t spcDirAddr, const std::wstring &name)
...
bool KonamiSnesInstrSet::GetHeaderInfo() { return true; }
...
bool KonamiSnesInstrSet::GetInstrPointers() {
  ...
  for (int instr = 0; instr <= 0xff; instr++) {
    ...
    if (!KonamiSnesInstr::IsValidHeader(this->rawfile, version, addrInstrHeader,
                                        spcDirAddr, false)) {
      if (instr < firstBankedInstr) {
        continue;
      } else {
    ...
    KonamiSnesInstr *newInstr =
        new KonamiSnesInstr(this, version, addrInstrHeader, instr >> 7,
                            instr & 0x7f, spcDirAddr, false, instrName.str());
  ...
  KonamiSnesInstr *newInstr = new KonamiSnesInstr(
      this, version, percInstrOffset, 127, 0, spcDirAddr, true, L"Percussions");
  aInstrs.push_back(newInstr);

  std::sort(usedSRCNs.begin(), usedSRCNs.end());
  SNESSampColl *newSampColl = new SNESSampColl(
      KonamiSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
KonamiSnesInstr::KonamiSnesInstr(VGMInstrSet *instrSet, KonamiSnesVersion ver,
                                 uint32_t offset, uint32_t theBank,
                                 uint32_t theInstrNum, uint32_t spcDirAddr,
                                 bool percussion, const std::wstring &name)
    : VGMInstr(instrSet, offset, KonamiSnesInstr::ExpectedSize(ver), theBank,
               theInstrNum, name),
      spcDirAddr(spcDirAddr),
...
KonamiSnesRgn::KonamiSnesRgn(KonamiSnesInstr *instr, KonamiSnesVersion ver,
                             uint32_t offset, bool percussion)
  ...
  const double pitch_fixer =
      log(4096.0 / 4286.0) / log(2);  // from pitch table ($10be vs $1000)
  ...
  // the following value 72.0 is chosen as a "average channel volume level
  // (before pan processing)"
...
bool KonamiSnesRgn::LoadRgn() { return true; }
