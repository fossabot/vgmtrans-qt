...
CompileSnesSeq::CompileSnesSeq(RawFile *file, CompileSnesVersion ver,
...
void CompileSnesSeq::ResetVars(void) { VGMSeq::ResetVars(); }
...
bool CompileSnesSeq::GetHeaderInfo(void) {
  ...
  for (uint8_t trackIndex = 0; trackIndex < nNumTracks; trackIndex++) {
    ...
    VGMHeader *trackHeader =
        header->AddHeader(curOffset, 14, trackName.str().c_str());
...
void CompileSnesSeq::LoadEventMap() {
  ...
  for (unsigned int statusByte = STATUS_DURATION_MIN;
       statusByte <= STATUS_DURATION_MAX; statusByte++) {
...
double CompileSnesSeq::GetTempoInBPM(uint8_t tempo) {
  ...
  return 60000000.0 / (SEQ_PPQN * (1000000.0 / SNES_NTSC_FRAMERATE)) *
         (tempoValue / 256.0);
}
...
CompileSnesTrack::CompileSnesTrack(CompileSnesSeq *parentFile, long offset,
                                   long length)
...
void CompileSnesTrack::AddInitialMidiEvents(int trackNum) {
  ...
  AddPanNoItem(Convert7bitLinearPercentPanValToStdMidiVal(
      (uint8_t)(spcPan + 0x80) / 2, &volumeScale));
...
bool CompileSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_LOOP_END: {
      ...
      desc << L"Nest Level: " << (int)repeatNest << L"  Destination: $"
    ...
    case EVENT_NOTE: {
      ...
      if (rest) {
        ...
        AddNoteByDur(beginOffset, curOffset - beginOffset, noteNumber, 100,
                     spcNoteDuration,
    ...
    case EVENT_FLAGS: {
      ...
      spcFlags = flags;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Flags",
    ...
    case EVENT_DURATION_DIRECT: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Duration (Direct)", desc.str().c_str(), CLR_DURNOTE,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_PERCUSSION_NOTE: {
      ...
      uint8_t percNoteNumber =
          statusByte - parentSeq->STATUS_PERCUSSION_NOTE_MIN;
      AddPercNoteByDur(
          beginOffset, curOffset - beginOffset, percNoteNumber, 100,
          spcNoteDuration,
...
bool CompileSnesTrack::ReadDurationBytes(uint32_t &offset, uint8_t &duration) {
  ...
  while (curOffset < 0x10000) {
    ...
    if (statusByte == parentSeq->STATUS_DURATION_DIRECT) {
      ...
      duration = CompileSnesSeq::noteDurTable[statusByte -
                                              parentSeq->STATUS_DURATION_MIN];
      offset = curOffset;
      durationDispatched = true;
    } else {
      break;
