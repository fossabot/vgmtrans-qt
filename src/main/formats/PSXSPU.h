...
// All of the ADSR calculations herein (except where inaccurate) are derived
// from Neill Corlett's work in reverse-engineering the Playstation 1/2 SPU
// unit.
...
template <class T>
void PSXConvADSR(T *realADSR, unsigned short ADSR1, unsigned short ADSR2,
                 bool bPS2) {
  ...
  // The following are unimplemented in conversion (because DLS and SF2 do not
  // support Sustain Rate)
...
template <class T>
void PSXConvADSR(T *realADSR, uint8_t Am, uint8_t Ar, uint8_t Dr, uint8_t Sl,
                 uint8_t Sm, uint8_t Sd, uint8_t Sr, uint8_t Rm, uint8_t Rr,
                 bool bPS2) {
  // Make sure all the ADSR values are within the valid ranges
  if (((Am & ~0x01) != 0) || ((Ar & ~0x7F) != 0) || ((Dr & ~0x0F) != 0) ||
      ((Sl & ~0x0F) != 0) || ((Rm & ~0x01) != 0) || ((Rr & ~0x1F) != 0) ||
      ((Sm & ~0x01) != 0) || ((Sd & ~0x01) != 0) || ((Sr & ~0x7F) != 0)) {
    pRoot->AddLogItem(
        new LogItem(L"PSX ADSR Out Of Range.", LOG_LEVEL_ERR, L"PSXConvADSR"));
    return;
  }
  ...
  // to get the dls 32 bit time cents, take log base 2 of number of seconds *
  // 1200 * 65536 (dls1v11a.pdf p25).
  ...
  for (l = 0; envelope_level > 0; l++) {
    if (4 * (Dr ^ 0x1F) < 0x18) Dr = 0;
    switch ((envelope_level >> 28) & 0x7) {
      case 0:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 0) + 32];
        break;
      case 1:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 4) + 32];
        break;
      case 2:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 6) + 32];
        break;
      case 3:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 8) + 32];
        break;
      case 4:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 9) + 32];
        break;
      case 5:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 10) + 32];
        break;
      case 6:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 11) + 32];
        break;
      case 7:
        envelope_level -=
            RateTable[RoundToZero((4 * (Dr ^ 0x1F)) - 0x18 + 12) + 32];
        break;
    }
    ...
    if (Sr == 0x7F)
      ...
      if (Sm == 0) {
        ...
        while (envelope_level > 0) {
          ...
          long steps = (envelope_level - envelope_level_target +
                        (envelope_level_diff - 1)) /
                       envelope_level_diff;
      ...
      realADSR->sustain_time =
          /*Sm ? timeInSecs : */ LinAmpDecayTimeToLinDBDecayTime(timeInSecs,
                                                                 0x800);
    }
  }
  ...
  // * 0.030517578139210854);	//in DLS, sustain level is measured as a
  // percentage
  if (Sl == 0) realSustainLevel = 0x07FFFFFF;
  realADSR->sustain_level = realSustainLevel / (double)0x7FFFFFFF;

  // If decay is going unused, and there's a sustain rate with sustain level
  // close to max...
  //  we'll put the sustain_rate in place of the decay rate.
  if ((realADSR->decay_time < 2 || (Dr == 0x0F && Sl >= 0x0C)) && Sr < 0x7E &&
      Sd == 1) {
  ...
  // We do this because we measure release time from max volume to 0, not from
  // sustain level to 0
  envelope_level = 0x7FFFFFFF;
  ...
  if (Rm == 0) {
    ...
    if ((Rr ^ 0x1F) * 4 < 0x18) Rr = 0;
    for (l = 0; envelope_level > 0; l++) {
      switch ((envelope_level >> 28) & 0x7) {
        case 0:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 0) + 32];
          break;
        case 1:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 4) + 32];
          break;
        case 2:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 6) + 32];
          break;
        case 3:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 8) + 32];
          break;
        case 4:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 9) + 32];
          break;
        case 5:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 10) + 32];
          break;
        case 6:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 11) + 32];
          break;
        case 7:
          envelope_level -=
              RateTable[RoundToZero((4 * (Rr ^ 0x1F)) - 0x18 + 12) + 32];
          break;
      }
    }
    samples = l;
  ...
  // timeInSecs = 0x7FFFFFFF * theRate;	//the release time value is more like
  // a rate.  It is the time from max value to 0, not from sustain level.  if (Rm
  // == 0) // if it's linear 	timeInSecs *=  LINEAR_RELEASE_COMPENSATION;

  realADSR->release_time =
      /*Rm ? timeInSecs : */ LinAmpDecayTimeToLinDBDecayTime(timeInSecs, 0x800);

  // We need to compensate the decay and release times to represent them as the
  // time from full vol to -100db where the drop in db is a fixed amount per
  // time unit (SoundFont2 spec for vol envelopes, pg44.)
  //  We assume the psx envelope is using a linear scale wherein envelope_level
  //  / 2 == half loudness. For a linear release mode (Rm == 0), the time to
  //  reach half volume is simply half the time to reach 0.
  // Half perceived loudness is -10db. Therefore, time_to_half_vol * 10 ==
  // full_time * 5 == the correct SF2 time
  // realADSR->decay_time =
  // LinAmpDecayTimeToLinDBDecayTime(realADSR->decay_time, 0x800);
  // realADSR->sustain_time =
  // LinAmpDecayTimeToLinDBDecayTime(realADSR->sustain_time, 0x800);
  // realADSR->release_time =
  // LinAmpDecayTimeToLinDBDecayTime(realADSR->release_time, 0x800);
...
class PSXSampColl : public VGMSampColl {
 public:
  PSXSampColl(const std::string &format, RawFile *rawfile, uint32_t offset,
              uint32_t length = 0);
  ...
  virtual bool GetSampleInfo();  // retrieve sample info, including pointers to
                                 // data, # channels, rate, etc.
  static PSXSampColl *SearchForPSXADPCM(RawFile *file,
                                        const std::string &format);
  static const std::vector<PSXSampColl *> SearchForPSXADPCMs(
      RawFile *file, const std::string &format);
...
class PSXSamp : public VGMSamp {
 public:
  PSXSamp(VGMSampColl *sampColl, uint32_t offset, uint32_t length,
          uint32_t dataOffset, uint32_t dataLen, uint8_t nChannels,
          uint16_t theBPS, uint32_t theRate, std::wstring name,
          bool bSetLoopOnConversion = true);
  virtual ~PSXSamp(void);

  // ratio of space conserved.  should generally be > 1
  // used to calculate both uncompressed sample size and loopOff after
  // conversion
