...
void SonyPS2Scanner::SearchForSeq(RawFile *file) {
  ...
  for (uint32_t i = 0; i + 0x40 < nFileLength; i++) {
    ...
    if (!newSeq->LoadVGMFile()) delete newSeq;
  }
}
...
void SonyPS2Scanner::SearchForInstrSet(RawFile *file) {
  ...
  for (uint32_t i = 0; i + 0x40 < nFileLength; i++) {
    ...
    if (!newInstrSet->LoadVGMFile()) delete newInstrSet;
  }
}

void SonyPS2Scanner::SearchForSampColl(RawFile *file) {
  uint32_t nFileLength = file->size();
  if (nFileLength < 32) return;

  if (StringToLower(file->GetExtension()) == L"bd") {
    // Hack for incorrectly ripped bd files.  Should ALWAYS start with 16 0x00
    // bytes (must... suppress... rage) If it doesn't, we'll throw out this file
    // and create a new one with the correct formating
    uint8_t buf[16];
    file->GetBytes(0, 16, buf);
    int num =
        CountBytesOfVal(buf, 16, 0);  // The first 16 bytes must be all 0x00
    if (num != 16) {
