...
MoriSnesInstrSet::MoriSnesInstrSet(
    RawFile *file, MoriSnesVersion ver, uint32_t spcDirAddr,
...
bool MoriSnesInstrSet::GetHeaderInfo() { return true; }
...
bool MoriSnesInstrSet::GetInstrPointers() {
  ...
  for (uint8_t instrNum = 0; instrNum < instrumentAddresses.size();
       instrNum++) {
    ...
    if (!instrumentHints[instrAddress].percussion) {
      ...
      for (uint8_t percNoteKey = 0;
           percNoteKey < instrumentHints[instrAddress].percHints.size();
           percNoteKey++) {
  ...
  for (uint8_t instrNum = 0; instrNum < instrumentAddresses.size();
    ...
    MoriSnesInstr *newInstr =
        new MoriSnesInstr(this, version, instrNum, spcDirAddr,
  ...
  SNESSampColl *newSampColl = new SNESSampColl(
      MoriSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
MoriSnesInstr::MoriSnesInstr(VGMInstrSet *instrSet, MoriSnesVersion ver,
                             uint8_t instrNum, uint32_t spcDirAddr,
                             const MoriSnesInstrHintDir &instrHintDir,
                             const std::wstring &name)
    : VGMInstr(instrSet, instrHintDir.startAddress, instrHintDir.size, 0,
               instrNum, name),
      version(ver),
      spcDirAddr(spcDirAddr),
...
bool MoriSnesInstr::LoadInstr() {
  ...
  if (!instrHintDir.percussion) {
    ...
    for (uint8_t percNoteKey = 0; percNoteKey < instrHintDir.percHints.size();
         percNoteKey++) {
      ...
      uint16_t addrSampStart = GetShort(offDirEnt);
      MoriSnesRgn *rgn =
          new MoriSnesRgn(this, version, spcDirAddr, *instrHint, percNoteKey);
      rgn->sampOffset = addrSampStart - spcDirAddr;
      aRgns.push_back(rgn);
    }
  }
...
MoriSnesRgn::MoriSnesRgn(MoriSnesInstr *instr, MoriSnesVersion ver,
                         uint32_t spcDirAddr,
  ...
  if (percNoteKey >= 0) {
    int8_t unityKeyCandidate =
        percNoteKey - (instrHint.transpose - unityKey);  // correct?
...
bool MoriSnesRgn::LoadRgn() { return true; }
