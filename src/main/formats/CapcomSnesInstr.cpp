...
CapcomSnesInstrSet::CapcomSnesInstrSet(RawFile *file, uint32_t offset,
                                       uint32_t spcDirAddr,
...
bool CapcomSnesInstrSet::GetHeaderInfo() { return true; }
...
bool CapcomSnesInstrSet::GetInstrPointers() {
  ...
  for (int instr = 0; instr <= 0xff; instr++) {
    ...
    if (!CapcomSnesInstr::IsValidHeader(this->rawfile, addrInstrHeader,
                                        spcDirAddr, false)) {
      break;
    }
    if (!CapcomSnesInstr::IsValidHeader(this->rawfile, addrInstrHeader,
    ...
    CapcomSnesInstr *newInstr =
        new CapcomSnesInstr(this, addrInstrHeader, instr >> 7, instr & 0x7f,
  ...
  SNESSampColl *newSampColl = new SNESSampColl(
      CapcomSnesFormat::name, this->rawfile, spcDirAddr, usedSRCNs);
...
CapcomSnesInstr::CapcomSnesInstr(VGMInstrSet *instrSet, uint32_t offset,
...
bool CapcomSnesInstr::IsValidHeader(RawFile *file, uint32_t addrInstrHeader,
...
bool CapcomSnesRgn::LoadRgn() { return true; }
