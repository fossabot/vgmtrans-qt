...
struct WdsHdr {
  ...
  uint32_t
      iFFT_InstrSetSize;  // size of entire instrset (header + instrs + samps)
...
struct WdsRgnData {
  ...
  uint8_t iFineTune;  // Pitch table is at 800290D8 in FFT.  See function at
                      // 80017424
...
class WdsInstr : public VGMInstr {
 public:
  WdsInstr(VGMInstrSet *instrSet, uint32_t offset, uint32_t length,
