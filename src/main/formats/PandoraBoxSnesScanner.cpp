...
BytePattern PandoraBoxSnesScanner::ptnSetDIR("\xe8\xfb\x8d\x5d\x61", "x?xxx",
                                             5);
...
// f678: fc        inc   y
// f679: dc        dec   y                 ; SRCN is index of the global
// instrument number (0 if not found)
...
void PandoraBoxSnesScanner::SearchForPandoraBoxSnesFromARAM(RawFile *file) {
  ...
  PandoraBoxSnesSeq *newSeq =
      new PandoraBoxSnesSeq(file, version, addrSeqHeader, name);
  ...
  PandoraBoxSnesInstrSet *newInstrSet = new PandoraBoxSnesInstrSet(
      file, version, spcDirAddr, addrLocalInstrTable, addrGlobalInstrTable,
      globalInstrumentCount, newSeq->instrADSRHints);
