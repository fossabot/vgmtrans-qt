...
bool BGMSeq::GetHeaderInfo(void) {
  ...
  theName << L"BGM " << seqID;
  if (seqID != assocWDID) theName << L"using WD " << assocWDID;
  name = theName.str();
  return true;
}
...
bool BGMSeq::GetTrackPointers(void) {
  ...
  for (unsigned int i = 0; i < nNumTracks; i++) {
    ...
    if (pos >= GetRawFile()->size()) return true;
    // END HACK
    uint32_t trackSize =
        GetWord(pos);  // get the track size (first word before track data)
...
bool BGMTrack::ReadEvent(void) {
  ...
  if (!vgmfile->IsValidOffset(curOffset)) {
    if (readMode == ReadMode::READMODE_ADD_TO_UI) {
      ...
      message << *vgmfile->GetName()
              << L": Address out of range. Conversion aborted.";
      pRoot->AddLogItem(
          new LogItem(message.str(), LOG_LEVEL_WARN, L"SquarePS2Seq"));
    }
    return false;
  }
  ...
  switch (status_byte) {
    ...
    case 0x02:
      ...
      //	AddBGMEvent("Loop Begin", ICON_STARTREP,
      //aBGMTracks[cur_track]->pTreeItem, offsetAtDelta, j-offsetAtDelta,
      //BG_CLR_CYAN);
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop Begin", L"",
                      CLR_LOOP);
      break;
    ...
    case 0x0C: {
      ...
      // for (value3 = 0; ((value2&1) != TRUE) && (value3 < 8); ++value3)
      // //while 	value2 >>= 1;  RecordMidiTimeSig(current_delta_time,
      // value1, value3, usiPPQN/4, 8, hFile);
      break;
    }
    ...
    case 0x22:
      ...
      // I am positive that volume values in this driver do not align with
      // standard MIDI. (see FFXI 213 Ru'Lude Gardens.psf2 for example)
    ...
    case 0x50:  // found in bgm 048 of FFX-2, that's ost song "101 Eternity
                // ~Memory of Lightwaves~.psf2"
      curOffset += 3;
      AddUnknown(beginOffset, curOffset - beginOffset);
      break;
    ...
    // pitch bend		I SHOULD GO BACK AND VERIFY THE RANGE OF THE PITCH
    // BEND
    case 0x5C: {
