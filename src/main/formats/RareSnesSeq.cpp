...
const uint16_t RareSnesSeq::NOTE_PITCH_TABLE[128] = {
    0x0000, 0x0040, 0x0044, 0x0048, 0x004c, 0x0051, 0x0055, 0x005b, 0x0060,
    0x0066, 0x006c, 0x0072, 0x0079, 0x0080, 0x0088, 0x0090, 0x0098, 0x00a1,
    0x00ab, 0x00b5, 0x00c0, 0x00cb, 0x00d7, 0x00e4, 0x00f2, 0x0100, 0x010f,
    0x011f, 0x0130, 0x0143, 0x0156, 0x016a, 0x0180, 0x0196, 0x01af, 0x01c8,
    0x01e3, 0x0200, 0x021e, 0x023f, 0x0261, 0x0285, 0x02ab, 0x02d4, 0x02ff,
    0x032d, 0x035d, 0x0390, 0x03c7, 0x0400, 0x043d, 0x047d, 0x04c2, 0x050a,
    0x0557, 0x05a8, 0x05fe, 0x065a, 0x06ba, 0x0721, 0x078d, 0x0800, 0x087a,
    0x08fb, 0x0984, 0x0a14, 0x0aae, 0x0b50, 0x0bfd, 0x0cb3, 0x0d74, 0x0e41,
    0x0f1a, 0x1000, 0x10f4, 0x11f6, 0x1307, 0x1429, 0x155c, 0x16a1, 0x17f9,
    0x1966, 0x1ae9, 0x1c82, 0x1e34, 0x2000, 0x21e7, 0x23eb, 0x260e, 0x2851,
    0x2ab7, 0x2d41, 0x2ff2, 0x32cc, 0x35d1, 0x3904, 0x3c68, 0x3fff, 0x3fff,
    0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff,
    0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff,
    0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff, 0x3fff,
    0x3fff, 0x3fff};

RareSnesSeq::RareSnesSeq(RawFile *file, RareSnesVersion ver,
                         uint32_t seqdataOffset, wstring newName)
...
bool RareSnesSeq::GetHeaderInfo(void) {
  ...
  VGMHeader *seqHeader =
      AddHeader(dwOffset, MAX_TRACKS * 2 + 2, L"Sequence Header");
  uint32_t curHeaderOffset = dwOffset;
  for (int i = 0; i < MAX_TRACKS; i++) {
    uint16_t trkOff = GetShort(curHeaderOffset);
    seqHeader->AddPointer(curHeaderOffset, 2, trkOff, (trkOff != 0),
                          L"Track Pointer");
    curHeaderOffset += 2;
  }
...
bool RareSnesSeq::GetTrackPointers(void) {
  for (int i = 0; i < MAX_TRACKS; i++) {
    ...
    if (trkOff != 0) aTracks.push_back(new RareSnesTrack(this, trkOff));
  }
  return true;
}
...
double RareSnesSeq::GetTempoInBPM(uint8_t tempo, uint8_t timerFreq) {
  if (timerFreq != 0 && tempo != 0) {
    return (double)60000000 / (SEQ_PPQN * (125 * timerFreq)) *
           ((double)tempo / 256);
  } else {
...
void RareSnesTrack::CalcVolPanFromVolLR(int8_t volL, int8_t volR,
                                        uint8_t &midiVol, uint8_t &midiPan) {
...
bool RareSnesTrack::ReadEvent(void) {
  ...
  if (statusByte >= 0x80) {
    ...
    if (noteByte == 0x80) {
      ...
      // ssTrace << L"Rest: " << dur << L" " << defNoteDur << L" " <<
      // (useLongDur ? L"L" : L"S") << std::endl;
      ...
      if (parentSeq->instrUnityKeyHints.find(spcInstr) ==
        ...
        parentSeq->instrPitchHints[spcInstr] =
            (int16_t)roundi(GetTuningInSemitones(spcTuning) * 100.0);
      } else {
        // check difference between preserved tuning and current tuning
        // example case: Donkey Kong Country 2 - Forest Interlude (Pads)
        instrTuningDelta =
            spcTransposeAbs - parentSeq->instrUnityKeyHints[spcInstr];
      }
      if (parentSeq->instrADSRHints.find(spcInstr) ==
          parentSeq->instrADSRHints.end()) {
        parentSeq->instrADSRHints[spcInstr] = spcADSR;
      }

      spcNotePitch = RareSnesSeq::NOTE_PITCH_TABLE[spcKey];
      spcNotePitch =
          (spcNotePitch * (1024 + spcTuning) + (spcTuning < 0 ? 1023 : 0)) /
          1024;
      ...
      uint8_t vel = 127;
      AddNoteByDur(beginOffset, curOffset - beginOffset, key + instrTuningDelta,
                   vel, dur);
      AddTime(dur);
    }
  } else {
    ...
    switch (eventType) {
      ...
      case EVENT_GOTO: {
        ...
        if (!IsOffsetUsed(dest) ||
            rptNestLevel !=
                0)  // nest level check is required for Stickerbrush Symphony
      ...
      case EVENT_CALLNTIMES: {
        ...
        desc << L"Times: " << (int)times << L"  Destination: $" << std::hex
             << std::setfill(L'0') << std::setw(4) << std::uppercase
             << (int)dest;
      ...
      case EVENT_DEFDURON: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Default Duration On", desc.str().c_str(), CLR_DURNOTE,
                        ICON_NOTE);
        break;
      }
      ...
      case EVENT_DEFDUROFF:
        defNoteDur = 0;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Default Duration Off", desc.str().c_str(),
      ...
      case EVENT_PITCHSLIDEDOWN: {
        curOffset += 5;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Pitch Slide Down", desc.str().c_str(), CLR_PITCHBEND,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_PITCHSLIDEOFF:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Pitch Slide Off", desc.str().c_str(), CLR_PITCHBEND,
                        ICON_CONTROL);
        break;
      ...
      case EVENT_TEMPO: {
        ...
        AddTempoBPM(
            beginOffset, curOffset - beginOffset,
      ...
      case EVENT_VIBRATOSHORT: {
        curOffset += 3;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Vibrato (Short)", desc.str().c_str(), CLR_MODULATION,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_ADSR: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR",
      ...
      case EVENT_MASTVOLLR: {
        int8_t newVolL = (int8_t)GetByte(curOffset++);
        int8_t newVolR = (int8_t)GetByte(curOffset++);
        int8_t newVol = min(abs((int)newVolL) + abs((int)newVolR), 255) /
      ...
      case EVENT_TUNING: {
        ...
        desc << L"Tuning: " << (int)newTuning << L" ("
             ...
             << L" cents)";
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Tuning",
                        desc.str().c_str(), CLR_PITCHBEND, ICON_CONTROL);
        break;
      }

      case EVENT_TRANSPABS:  // should be used for pitch correction of
                             // instrument
      {
        int8_t newTransp = (int8_t)GetByte(curOffset++);
        spcTranspose = spcTransposeAbs = newTransp;
        // AddTranspose(beginOffset, curOffset-beginOffset, 0, L"Transpose
        // (Abs)");

        // add event without MIDI event
        desc << L"Transpose: " << newTransp;
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Transpose",
      ...
      case EVENT_TRANSPREL: {
        ...
        // AddTranspose(beginOffset, curOffset-beginOffset, spcTransposeAbs -
        // spcTranspose, L"Transpose (Rel)");

        // add event without MIDI event
        desc << L"Transpose: " << deltaTransp;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Transpose (Relative)", desc.str(), CLR_TRANSPOSE,
                        ICON_CONTROL);
      ...
      case EVENT_ECHOPARAM: {
        ...
        int8_t newVolL = (int8_t)GetByte(curOffset++);
        int8_t newVolR = (int8_t)GetByte(curOffset++);
        parentSeq->midiReverb =
            min(abs((int)newVolL) + abs((int)newVolR), 255) / 2;
        // TODO: update MIDI reverb value for each tracks?

        desc << L"Feedback: " << (int)newFeedback << L"  Volume: "
             << (int)newVolL << L", " << (int)newVolR;
      ...
      case EVENT_ECHOFIR: {
        ...
        for (int iFIRIndex = 0; iFIRIndex < 8; iFIRIndex++) {
          if (iFIRIndex != 0) desc << L" ";
          desc << std::hex << std::setfill(L'0') << std::setw(2)
               << std::uppercase << (int)newFIR[iFIRIndex];
        }
      ...
      case EVENT_NOISECLK: {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Noise Frequency", desc.str().c_str(), CLR_CHANGESTATE,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_PITCHSLIDEDOWNSHORT: {
        curOffset += 4;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Pitch Slide Down (Short)", desc.str().c_str(),
      ...
      case EVENT_PITCHSLIDEUPSHORT: {
        curOffset += 4;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Pitch Slide Up (Short)", desc.str().c_str(),
                        CLR_PITCHBEND, ICON_CONTROL);
        break;
      }
      ...
      case EVENT_LONGDURON:
        useLongDur = true;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Long Duration On", desc.str().c_str(), CLR_DURNOTE,
                        ICON_NOTE);
        break;
      ...
      case EVENT_LONGDUROFF:
        useLongDur = false;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Long Duration Off", desc.str().c_str(), CLR_DURNOTE,
                        ICON_NOTE);
        break;
      ...
      case EVENT_SETVOLADSRPRESET1: {
        ...
        desc << L"Left Volume: " << newVolL << L"  Right Volume: " << newVolR
             << L"  AR: " << (int)ar << L"  DR: " << (int)dr << L"  SL: "
             << (int)sl << L"  SR: " << (int)sr;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Set Vol/ADSR Preset 1", desc.str(), CLR_VOLUME,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_SETVOLADSRPRESET2: {
        ...
        desc << L"Left Volume: " << newVolL << L"  Right Volume: " << newVolR
             ...
             << (int)sl << L"  SR: " << (int)sr;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Set Vol/ADSR Preset 2", desc.str(), CLR_VOLUME,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_SETVOLADSRPRESET3: {
        ...
        desc << L"Left Volume: " << newVolL << L"  Right Volume: " << newVolR
             ...
             << (int)sl << L"  SR: " << (int)sr;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Set Vol/ADSR Preset 3", desc.str(), CLR_VOLUME,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_SETVOLADSRPRESET4: {
        ...
        desc << L"Left Volume: " << newVolL << L"  Right Volume: " << newVolR
             ...
             << (int)sl << L"  SR: " << (int)sr;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Set Vol/ADSR Preset 4", desc.str(), CLR_VOLUME,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_SETVOLADSRPRESET5: {
        ...
        desc << L"Left Volume: " << newVolL << L"  Right Volume: " << newVolR
             ...
             << (int)sl << L"  SR: " << (int)sr;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Set Vol/ADSR Preset 5", desc.str(), CLR_VOLUME,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_RESETADSR:
        spcADSR = 0x8FE0;
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Reset ADSR",
                        L"ADSR: 8FE0", CLR_ADSR, ICON_CONTROL);
        break;

      case EVENT_RESETADSRSOFT:
        spcADSR = 0x8EE0;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Reset ADSR (Soft)", L"ADSR: 8EE0", CLR_ADSR,
                        ICON_CONTROL);
        break;
      ...
      case EVENT_VOICEPARAMSHORT: {
        ...
        desc << L"Program Number: " << (int)newProg << L"  Transpose: "
             << (int)newTransp << L"  Tuning: " << (int)newTuning << L" ("
             << (int)(GetTuningInSemitones(newTuning) * 100 + 0.5)
             << L" cents)";
        ;
      ...
      case EVENT_VOICEPARAM: {
        ...
        spcInstr = newProg;
        AddProgramChange(
            beginOffset, curOffset - beginOffset, newProg, true,
      ...
      case EVENT_SETVOLPRESETS: {
        ...
        // add event without MIDI events
        CalcVolPanFromVolLR(parentSeq->presetVolL[0], parentSeq->presetVolR[0],
                            newMidiVol, newMidiPan);
        desc << L"Left Volume 1: " << newVolL1 << L"  Right Volume 1: "
             << newVolR1 << L"  Left Volume 2: " << newVolL2
             << L"  Right Volume 2: " << newVolR2;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Set Volume Preset", desc.str(), CLR_VOLUME,
                        ICON_CONTROL);
        break;
      }
      ...
      case EVENT_LFOOFF:
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Pitch Slide/Vibrato/Tremolo Off", desc.str(),
      ...
      default:
        ...
        pRoot->AddLogItem(new LogItem(
            (std::wstring(L"Unknown Event - ") + desc.str()).c_str(),
            LOG_LEVEL_ERR, L"RareSnesSeq"));
        bContinue = false;
        break;
    }
...
void RareSnesTrack::AddVolLR(uint32_t offset, uint32_t length, int8_t spcVolL,
                             int8_t spcVolR, const std::wstring &sEventName) {
  uint8_t newMidiVol;
  uint8_t newMidiPan;
  CalcVolPanFromVolLR(spcVolL, spcVolR, newMidiVol, newMidiPan);
  ...
  AddGenericEvent(offset, length, sEventName, desc.str(), CLR_VOLUME,
                  ICON_CONTROL);
