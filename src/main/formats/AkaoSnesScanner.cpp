...
BytePattern AkaoSnesScanner::ptnReadNoteLengthV4(
    "\xcd\x0e\x9e\xf8\xa2\xf6\xaa\x16", "xxxx?x??", 8);
...
// 0a68: da 00     movw  $00,ya            ; $00/1 - offset from ROM to ARAM
// address
...
BytePattern AkaoSnesScanner::ptnLoadDIRV1("\xe8\x1e\x8d\x5d\x3f\xe9\x10",
                                          "x?xxx??", 7);
...
BytePattern AkaoSnesScanner::ptnLoadDIRV3("\x8d\x5d\xe8\x1b\x3f\x55\x06",
                                          "xxx?x??", 7);
...
void AkaoSnesScanner::SearchForAkaoSnesFromARAM(RawFile *file) {
  ...
  if (file->SearchBytePattern(ptnReadSeqHeaderV4, ofsReadSeqHeader)) {
    ...
    addrAPURelocBase = (file->GetByte(ofsReadSeqHeader + 13) << 8) |
                       file->GetByte(ofsReadSeqHeader + 11);
    relocatable = true;
  ...
  if (verReadNoteLength == AKAOSNES_V1 && firstVCmd == 0xd2 && !relocatable) {
    version = AKAOSNES_V1;
  } else if (verReadNoteLength == AKAOSNES_V2 && firstVCmd == 0xd2 &&
             !relocatable) {
    version = AKAOSNES_V2;
  } else if (verReadNoteLength == AKAOSNES_V2 && firstVCmd == 0xd2 &&
             relocatable) {
    version = AKAOSNES_V3;
  } else if (verReadNoteLength == AKAOSNES_V4 && firstVCmd == 0xc4 &&
             relocatable) {
    version = AKAOSNES_V4;
  } else {
    return;
  ...
  const uint8_t RS2_VCMD_LEN_TABLE[] = {
      ...
      0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x02,
      0x02, 0x01, 0x03, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  const uint8_t LAL_VCMD_LEN_TABLE[] = {
      0x01, 0x02, 0x01, 0x02, 0x02, 0x03, 0x00, 0x03, 0x00, 0x02, 0x00, 0x01,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x01, 0x01, 0x01,
      0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x02,
      0x02, 0x01, 0x03, 0x02, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00};
  const uint8_t FF6_VCMD_LEN_TABLE[] = {
      0x01, 0x02, 0x01, 0x02, 0x02, 0x03, 0x00, 0x03, 0x00, 0x02, 0x00, 0x01,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x01, 0x01, 0x01,
      0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x02,
      0x01, 0x03, 0x02, 0x02, 0x02, 0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00};
  ...
  const uint8_t RS3_VCMD_LEN_TABLE[] = {
      ...
      0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x02,
      0x01, 0x03, 0x02, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x01, 0x00, 0x00};
  ...
  const uint8_t BSGAME_VCMD_LEN_TABLE[] = {
      ...
      0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x01, 0x02,
      0x01, 0x03, 0x02, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00, 0x01, 0x00, 0x00};
  if (file->MatchBytes(FF4_VCMD_LEN_TABLE, addrVCmdLengthTable,
                       sizeof(FF4_VCMD_LEN_TABLE))) {
  ...
  // load sequence
  AkaoSnesSeq *newSeq = new AkaoSnesSeq(file, version, minorVersion,
                                        addrSeqHeader, addrAPURelocBase, name);
  ...
  if (version == AKAOSNES_V1 &&
      file->SearchBytePattern(ptnLoadInstrV1, ofsLoadInstr)) {
    addrTuningTable = file->GetShort(ofsLoadInstr + 5);
    addrADSRTable = 0;  // N/A
  } else if (version == AKAOSNES_V2 &&
             file->SearchBytePattern(ptnLoadInstrV2, ofsLoadInstr)) {
  ...
  if (file->SearchBytePattern(ptnReadPercussionTableV4,
                              ofsReadPercussionTable)) {
    addrPercussionTable = file->GetShort(ofsReadPercussionTable + 19);
  } else {
    addrPercussionTable = 0;
  }

  AkaoSnesInstrSet *newInstrSet =
      new AkaoSnesInstrSet(file, version, spcDirAddr, addrTuningTable,
