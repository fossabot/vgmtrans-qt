...
// 062b: e5 01 12  mov   a,$1201           ; $1201/2: first track address (ROM
// address, not APU RAM address)  062e: 80        setc  062f: a8 18     sbc
// a,#$18  0631: c4 0f     mov   $0f,a  0633: e5 02 12  mov   a,$1202  0636: a8 00
// sbc   a,#$00  0638: c4 10     mov   $10,a             ; $0f/10 = first track
// address - 24 header bytes
...
void GraphResSnesScanner::SearchForGraphResSnesFromARAM(RawFile *file) {
  ...
  if (file->SearchBytePattern(ptnLoadSeq, ofsLoadSeq)) {
    addrSeqHeader =
        file->GetByte(ofsLoadSeq + 4) | (file->GetByte(ofsLoadSeq + 8) << 8);
  } else {
    return;
  }
  ...
  GraphResSnesSeq *newSeq =
      new GraphResSnesSeq(file, version, addrSeqHeader, name);
  ...
  // scan SRCN table
  GraphResSnesInstrSet *newInstrSet = new GraphResSnesInstrSet(
      file, version, spcDirAddr, newSeq->instrADSRHints);
...
std::map<uint8_t, uint8_t> GraphResSnesScanner::GetInitDspRegMap(
    RawFile *file) {
