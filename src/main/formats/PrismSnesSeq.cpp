...
PrismSnesSeq::PrismSnesSeq(RawFile *file, PrismSnesVersion ver,
...
bool PrismSnesSeq::GetTrackPointers(void) { return true; }
...
PrismSnesTrack::PrismSnesTrack(PrismSnesSeq *parentFile, long offset,
                               long length)
...
bool PrismSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_NOP2: {
      curOffset += 2;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"NOP", desc.str(),
    ...
    case EVENT_NOISE_NOTE: {
      ...
      if (prevNoteSlurred && key == prevNoteKey) {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset, L"Note (Tied)",
                        desc.str(), CLR_DURNOTE, ICON_NOTE);
      } else {
    ...
    case EVENT_PITCH_SLIDE: {
      ...
      desc << L"Note Number (From): " << noteNumberFrom << L" ("
           << ((noteFrom & 0x80) != 0 ? L"Noise"
                                      : MidiEvent::GetNoteName(noteNumberFrom))
           << L")" << L"  Note Number (To): " << noteNumberTo << L" ("
           << ((noteTo & 0x80) != 0 ? L"Noise"
                                    : MidiEvent::GetNoteName(noteNumberTo))
           << L")" << L"  Length: " << len << L"  Duration: " << dur;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pitch Slide",
                      desc.str(), CLR_PITCHBEND, ICON_CONTROL);
    ...
    case EVENT_TIE_WITH_DUR: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Tie with Duration", desc.str(), CLR_TIE, ICON_NOTE);
      MakePrevDurNoteEnd(GetTime() + dur);
      AddTime(len);
      break;
    }
    ...
    case EVENT_UNKNOWN_EVENT_ED: {
      ...
      curOffset += 2;
      desc << L"Arg1: " << arg1 << L"  Arg2: $" << std::hex
           << std::setfill(L'0') << std::setw(4) << std::uppercase << arg2;
      AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                 desc.str());
      break;
    }
    ...
    case EVENT_REST: {
      ...
      desc << L"Duration: " << len;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Rest", desc.str(),
                      CLR_REST, ICON_REST);
      AddTime(len);
      break;
    }
    ...
    case EVENT_CONDITIONAL_JUMP: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Conditional Jump",
                      desc.str(), CLR_LOOP);
    ...
    case EVENT_CONDITION: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Condition On",
    ...
    case EVENT_RESTORE_ECHO_PARAM: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Restore Echo Param", desc.str(), CLR_REVERB,
                      ICON_CONTROL);
      break;
    }

    case EVENT_SAVE_ECHO_PARAM: {
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Save Echo Param",
    ...
    case EVENT_SLUR_OFF: {
      slur = false;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Slur Off",
    ...
    case EVENT_DEFAULT_PAN_TABLE_1: {
      panTable.assign(std::begin(PrismSnesSeq::PAN_TABLE_1),
                      std::end(PrismSnesSeq::PAN_TABLE_1));
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Default Pan Table #1", desc.str(), CLR_PAN,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_DEFAULT_PAN_TABLE_2: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Default Pan Table #2", desc.str(), CLR_PAN,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_INC_APU_PORT_3: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Increment APU Port 3", desc.str(), CLR_CHANGESTATE);
      break;
    }

    case EVENT_INC_APU_PORT_2: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Increment APU Port 2", desc.str(), CLR_CHANGESTATE);
      break;
    }
    ...
    case EVENT_PLAY_SONG_3: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Play Song (3)",
                      desc.str(), CLR_CHANGESTATE);
      break;
    }
    ...
    case EVENT_PLAY_SONG_2: {
      uint8_t songIndex = GetByte(curOffset++);
      desc << L"Song Index: " << songIndex;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Play Song (2)",
                      desc.str(), CLR_CHANGESTATE);
      break;
    }
    ...
    case EVENT_PLAY_SONG_1: {
      uint8_t songIndex = GetByte(curOffset++);
      desc << L"Song Index: " << songIndex;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Play Song (1)",
                      desc.str(), CLR_CHANGESTATE);
      break;
    }
    ...
    case EVENT_PAN_ENVELOPE: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pan Envelope",
    ...
    case EVENT_PAN_TABLE: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pan Table",
                      desc.str(), CLR_PAN, ICON_CONTROL);
    ...
    case EVENT_DEFAULT_LENGTH_OFF: {
      defaultLength = 0;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Default Length Off", desc.str(), CLR_DURNOTE);
      break;
    }
    ...
    case EVENT_DEFAULT_LENGTH: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Default Length",
    ...
    case EVENT_LOOP_UNTIL: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop Until",
                      desc.str(), CLR_LOOP, ICON_ENDREP);
    ...
    case EVENT_LOOP_UNTIL_ALT: {
      ...
      desc << L"Loop Count: " << count << L"  Destination: $" << std::hex
           << std::setfill(L'0') << std::setw(4) << std::uppercase << (int)dest;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Loop Until (Alt)",
                      desc.str(), CLR_LOOP, ICON_ENDREP);
    ...
    case EVENT_CALL: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Pattern Play",
                      desc.str(), CLR_LOOP, ICON_STARTREP);
    ...
    case EVENT_GOTO: {
      ...
      if (!IsOffsetUsed(dest)) {
        AddGenericEvent(beginOffset, length, L"Jump", desc.str(),
                        CLR_LOOPFOREVER);
      } else {
    ...
    case EVENT_VIBRATO_DELAY: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Vibrato Delay",
                      desc.str(), CLR_MODULATION, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_VIBRATO: {
      ...
      uint8_t arg2 = GetByte(curOffset++);
      uint8_t arg3 = GetByte(curOffset++);
      desc << L"Delay: " << (int)lfoDelay << L"  Arg2: " << (int)arg2
           << L"  Arg3: " << (int)arg3;
    ...
    case EVENT_PAN: {
      ...
      double volumeLeft;
      double volumeRight;
      // actual engine divides pan by 256, though pan value must be always
      // 7-bit, perhaps
      ...
      double volumeScale;
      int8_t midiPan = ConvertVolumeBalanceToStdMidiPan(volumeLeft, volumeRight,
                                                        &volumeScale);
    ...
    case EVENT_GAIN_ENVELOPE_REST: {
      ...
      desc << L"Envelope: $" << std::hex << std::setfill(L'0') << std::setw(4)
           << std::uppercase << envelopeAddress;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN Envelope (Rest)", desc.str(), CLR_ADSR,
                      ICON_CONTROL);
    ...
    case EVENT_GAIN_ENVELOPE_DECAY_TIME: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN Envelope Decay Time", desc.str(), CLR_ADSR,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_MANUAL_DURATION_OFF: {
      manualDuration = false;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Manual Duration Off", desc.str(), CLR_DURNOTE);
      break;
    }
    ...
    case EVENT_MANUAL_DURATION_ON: {
      manualDuration = true;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Manual Duration On", desc.str(), CLR_DURNOTE);
      break;
    }
    ...
    case EVENT_AUTO_DURATION_THRESHOLD: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Auto Duration Threshold", desc.str(), CLR_DURNOTE);
      break;
    }
    ...
    case EVENT_GAIN_ENVELOPE_SUSTAIN: {
      ...
      desc << L"Envelope: $" << std::hex << std::setfill(L'0') << std::setw(4)
           << std::uppercase << envelopeAddress;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN Envelope (Sustain)", desc.str(), CLR_ADSR,
                      ICON_CONTROL);
      AddGAINEnvelope(envelopeAddress);
      break;
    }
    ...
    case EVENT_ECHO_VOLUME_ENVELOPE: {
      ...
      desc << L"Envelope: $" << std::hex << std::setfill(L'0') << std::setw(4)
           << std::uppercase << envelopeAddress;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Echo Volume Envelope", desc.str(), CLR_REVERB,
                      ICON_CONTROL);
    ...
    case EVENT_ECHO_VOLUME: {
      ...
      desc << L"Left Volume: " << echoVolumeLeft << L"  Right Volume: "
    ...
    case EVENT_ECHO_PARAM: {
      ...
      int8_t echoVolumeRight = GetByte(curOffset++);
      int8_t echoVolumeMono = GetByte(curOffset++);
      desc << L"Feedback: " << echoFeedback << L"  Left Volume: "
           << echoVolumeLeft << L"  Right Volume: " << echoVolumeRight
    ...
    case EVENT_ADSR: {
      ...
      if (param >= 0x80) {
        ...
        desc << L"ADSR(1): $" << std::hex << std::setfill(L'0') << std::setw(2)
             << std::uppercase << adsr1 << L"  ADSR(2): $" << std::hex
             << std::setfill(L'0') << std::setw(2) << std::uppercase << adsr2;
      } else {
    ...
    case EVENT_GAIN_ENVELOPE_DECAY: {
      ...
      desc << L"Envelope: $" << std::hex << std::setfill(L'0') << std::setw(4)
           << std::uppercase << envelopeAddress;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN Envelope (Decay)", desc.str(), CLR_ADSR,
                      ICON_CONTROL);
      AddGAINEnvelope(envelopeAddress);
      break;
    }
...
bool PrismSnesTrack::ReadDuration(uint32_t &curOffset, uint8_t len,
                                  uint8_t &durDelta) {
  if (curOffset + 1 > 0x10000) {
    return false;
  }
...
uint8_t PrismSnesTrack::GetDuration(uint32_t curOffset, uint8_t len,
                                    uint8_t durDelta) {
  // TODO: adjust duration for slur/tie
  if (curOffset + 1 <= 0x10000) {
    uint8_t nextStatusByte = GetByte(curOffset);
    if (nextStatusByte == 0xee || nextStatusByte == 0xf4 ||
        nextStatusByte == 0xce || nextStatusByte == 0xf5) {
      durDelta = 0;
    }
  }
...
void PrismSnesTrack::AddVolumeEnvelope(uint16_t envelopeAddress) {
  ...
  if (!IsOffsetUsed(envelopeAddress)) {
    ...
    envelopeName << L"Volume Envelope ($" << std::hex << std::setfill(L'0')
                 << std::setw(4) << std::uppercase << envelopeAddress << L")";
    VGMHeader *envHeader = parentSeq->envContainer->AddHeader(
        envelopeAddress, 0, envelopeName.str());
    ...
    while (envelopeAddress + envOffset + 2 <= 0x10000) {
      ...
      if (volumeFrom == 0 && volumeDelta == 0) {
        ...
        eventName << L"Jump to $" << std::hex << std::setfill(L'0')
                  << std::setw(4) << std::uppercase << newAddress;
        envHeader->AddSimpleItem(envelopeAddress + envOffset, 4,
                                 eventName.str());
        envOffset += 4;
        break;
      }
      ...
      eventName << L"Volume: " << volumeFrom << L"  Volume Delta: "
                << volumeDelta << L"  Envelope Speed: " << envelopeSpeed
...
void PrismSnesTrack::AddPanEnvelope(uint16_t envelopeAddress) {
  ...
  if (!IsOffsetUsed(envelopeAddress)) {
    std::wostringstream envelopeName;
    envelopeName << L"Pan Envelope ($" << std::hex << std::setfill(L'0')
                 << std::setw(4) << std::uppercase << envelopeAddress << L")";
    VGMHeader *envHeader = parentSeq->envContainer->AddHeader(
        envelopeAddress, 0, envelopeName.str());
    ...
    while (envOffset < 0x100) {
      ...
      if (newPan >= 0x80) {
        ...
        eventName << L"Jump to $" << std::hex << std::setfill(L'0')
                  << std::setw(4) << std::uppercase
                  << (envelopeAddress + newOffset);
        envHeader->AddSimpleItem(envelopeAddress + envOffset, 2,
                                 eventName.str());
        envOffset += 2;
        break;
      }
...
void PrismSnesTrack::AddEchoVolumeEnvelope(uint16_t envelopeAddress) {
  ...
  if (!IsOffsetUsed(envelopeAddress)) {
    std::wostringstream envelopeName;
    envelopeName << L"Echo Volume Envelope ($" << std::hex << std::setfill(L'0')
                 << std::setw(4) << std::uppercase << envelopeAddress << L")";
    VGMHeader *envHeader = parentSeq->envContainer->AddHeader(
        envelopeAddress, 0, envelopeName.str());
    ...
    while (envOffset < 0x100) {
      ...
      eventName << L"Delta-Time: " << deltaTime << L"  Left Volume: "
                << echoVolumeLeft << L"  Right Volume: " << echoVolumeRight
                << L"  Mono Volume: " << echoVolumeMono;
      envHeader->AddSimpleItem(envelopeAddress + envOffset, 4, eventName.str());
      envOffset += 4;
...
void PrismSnesTrack::AddGAINEnvelope(uint16_t envelopeAddress) {
  ...
  if (!IsOffsetUsed(envelopeAddress)) {
    std::wostringstream envelopeName;
    envelopeName << L"GAIN Envelope ($" << std::hex << std::setfill(L'0')
                 << std::setw(4) << std::uppercase << envelopeAddress << L")";
    VGMHeader *envHeader = parentSeq->envContainer->AddHeader(
        envelopeAddress, 0, envelopeName.str());
    ...
    while (envOffset < 0x100) {
      ...
      eventName << L"GAIN: $" << std::hex << std::setfill(L'0') << std::setw(2)
                << std::uppercase << gain << std::dec << std::setfill(L' ')
                << std::setw(0) << L", Delta-Time: " << deltaTime;
...
void PrismSnesTrack::AddPanTable(uint16_t panTableAddress) {
  ...
  if (!IsOffsetUsed(panTableAddress)) {
    std::wostringstream eventName;
    eventName << L"Pan Table ($" << std::hex << std::setfill(L'0')
              << std::setw(4) << std::uppercase << panTableAddress << L")";
    parentSeq->envContainer->AddSimpleItem(panTableAddress, 21,
                                           eventName.str());
  }
}
