...
NinSnesSeq::NinSnesSeq(RawFile *file, NinSnesVersion ver, uint32_t offset,
...
bool NinSnesSeq::ReadEvent(long stopTime) {
  ...
  if (sectionAddress == 0) {
    ...
    if (!IsOffsetUsed(beginOffset)) {
      header->AddSimpleItem(beginOffset, curOffset - beginOffset,
                            L"Section Playlist End");
    }
    bContinue = false;
    ...
    NinSnesSection *section =
        (NinSnesSection *)GetSectionFromOffset(sectionAddress);
    if (section == NULL) {
...
void NinSnesSeq::LoadEventMap() {
  ...
  const uint8_t NINSNES_DURVOL_TABLE_INTELLI_FE3[64] = {
      0x00, 0x0c, 0x19, 0x26, 0x33, 0x3f, 0x4c, 0x59, 0x66, 0x72, 0x75,
      0x77, 0x70, 0x7c, 0x7f, 0x82, 0x84, 0x87, 0x89, 0x8c, 0x8e, 0x91,
      0x93, 0x96, 0x99, 0x9b, 0x9e, 0xa0, 0xa3, 0xa5, 0xa8, 0xaa, 0xad,
      0xaf, 0xb2, 0xb5, 0xb7, 0xba, 0xbc, 0xbf, 0xc1, 0xc4, 0xc6, 0xc9,
      0xcc, 0xce, 0xd1, 0xd3, 0xd6, 0xd8, 0xdb, 0xdd, 0xe0, 0xe2, 0xe5,
      0xe8, 0xea, 0xed, 0xef, 0xf2, 0xf4, 0xf7, 0xf9, 0xfc,
  };

  const uint8_t NINSNES_DURVOL_TABLE_INTELLI_FE4[64] = {
      0x19, 0x26, 0x33, 0x3f, 0x4c, 0x59, 0x66, 0x6d, 0x70, 0x72, 0x75,
      0x77, 0x70, 0x7c, 0x7f, 0x82, 0x84, 0x87, 0x89, 0x8c, 0x8e, 0x91,
      0x93, 0x96, 0x99, 0x9b, 0x9e, 0xa0, 0xa3, 0xa5, 0xa8, 0xaa, 0xad,
      0xaf, 0xb2, 0xb5, 0xb7, 0xba, 0xbc, 0xbf, 0xc1, 0xc4, 0xc6, 0xc9,
      0xcc, 0xce, 0xd1, 0xd3, 0xd6, 0xd8, 0xdb, 0xdd, 0xe0, 0xe2, 0xe5,
  ...
  for (statusByte = STATUS_NOTE_MIN; statusByte <= STATUS_NOTE_MAX;
       statusByte++) {
  ...
  for (statusByte = STATUS_PERCUSSION_NOTE_MIN;
...
double NinSnesSeq::GetTempoInBPM(uint8_t tempo) {
  if (tempo != 0) {
    ...
    return 1.0;  // since tempo 0 cannot be expressed, this function returns a
                 // very small value.
  }
}
...
NinSnesTrackSharedData::NinSnesTrackSharedData() { ResetVars(); }
...
NinSnesTrack::NinSnesTrack(NinSnesSection *parentSection, long offset,
                           long length, const std::wstring &theName)
...
bool NinSnesTrack::ReadEvent(void) {
  ...
  switch (eventType) {
    ...
    case EVENT_UNKNOWN1: {
      ...
      desc << L"Event: 0x" << std::hex << std::setfill(L'0') << std::setw(2)
           << std::uppercase << (int)statusByte << std::dec
           << std::setfill(L' ') << std::setw(0) << L"  Arg1: " << (int)arg1;
      AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                 desc.str().c_str());
      break;
    }
    ...
    case EVENT_UNKNOWN3: {
      ...
      desc << L"Event: 0x" << std::hex << std::setfill(L'0') << std::setw(2)
           << std::uppercase << (int)statusByte << std::dec
           << std::setfill(L' ') << std::setw(0) << L"  Arg1: " << (int)arg1
           << L"  Arg2: " << (int)arg2 << L"  Arg3: " << (int)arg3;
      AddUnknown(beginOffset, curOffset - beginOffset, L"Unknown Event",
                 desc.str().c_str());
      break;
    }
    ...
    OnEventNoteParam:
      ...
      if (curOffset + 1 < 0x10000 && GetByte(curOffset) <= 0x7f) {
        ...
        desc << L"  Quantize: " << (int)durIndex << L" ("
             << (int)shared->spcNoteDurRate << L"/256)" << L"  Velocity: "
             << (int)velIndex << L" (" << (int)shared->spcNoteVolume
             << L"/256)";
      }

      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Note Param",
    ...
    case EVENT_NOTE: {
      ...
      uint8_t duration =
          (shared->spcNoteDuration * shared->spcNoteDurRate) >> 8;
      duration = min(max(duration, (uint8_t)1),
                     (uint8_t)(shared->spcNoteDuration - 2));
    ...
    case EVENT_PAN: {
      ...
      bool reverseRight;
      int8_t midiPan =
          CalcPanValue(newPan, volumeScale, reverseLeft, reverseRight);
    ...
    case EVENT_PAN_FADE: {
      ...
      uint8_t midiPan =
          ConvertVolumeBalanceToStdMidiPan(volumeLeft, volumeRight);
    ...
    case EVENT_MASTER_VOLUME_FADE: {
      ...
      AddMastVolSlide(beginOffset, curOffset - beginOffset, fadeLength,
                      newVol / 2);
      break;
    }
    ...
    case EVENT_CALL: {
      ...
      desc << L"Destination: $" << std::hex << std::setfill(L'0')
           << std::setw(4) << std::uppercase << (int)dest << std::dec
           << std::setfill(L' ') << std::setw(0) << L"  Times: " << (int)times;
      ...
      if (curOffset < 0x10000 && GetByte(curOffset) == parentSeq->STATUS_END) {
        if (shared->loopCount == 0) {
          AddGenericEvent(curOffset, 1, L"Section End", desc.str().c_str(),
                          CLR_TRACKEND, ICON_TRACKEND);
        } else {
          AddGenericEvent(curOffset, 1, L"Pattern End", desc.str().c_str(),
                          CLR_TRACKEND, ICON_TRACKEND);
        }
      }
    ...
    case EVENT_PITCH_ENVELOPE_TO: {
      ...
      desc << L"Delay: " << (int)pitchEnvDelay << L"  Length: "
           << (int)pitchEnvLength << L"  Semitones: " << (int)pitchEnvSemitones;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Pitch Envelope (To)", desc.str().c_str(), CLR_PITCHBEND,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_PITCH_ENVELOPE_FROM: {
      ...
      desc << L"Delay: " << (int)pitchEnvDelay << L"  Length: "
           << (int)pitchEnvLength << L"  Semitones: " << (int)pitchEnvSemitones;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Pitch Envelope (From)", desc.str().c_str(),
    ...
    case EVENT_PITCH_ENVELOPE_OFF: {
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Pitch Envelope Off", desc.str().c_str(), CLR_PITCHBEND,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_TUNING: {
      ...
      AddFineTuning(beginOffset, curOffset - beginOffset,
                    (newTuning / 256.0) * 100.0);
      break;
    }

    case EVENT_QUINTET_TUNING: {
      // TODO: Correct fine tuning on Quintet games
      // In Quintet games (at least in Terranigma), the fine tuning command
      // overwrites the fractional part of instrument tuning. In other words, we
      // cannot calculate the tuning amount without reading the instrument
      // table.
      uint8_t newTuning = GetByte(curOffset++);
    ...
    case EVENT_ECHO_ON: {
      ...
      desc << L"  Volume Left: " << (int)spcEVOL_L << L"  Volume Right: "
           << (int)spcEVOL_R;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Echo",
    ...
    case EVENT_ECHO_VOLUME_FADE: {
      ...
      desc << L"Length: " << (int)fadeLength << L"  Volume Left: "
           << (int)spcEVOL_L << L"  Volume Right: " << (int)spcEVOL_R;
    ...
    case EVENT_PITCH_SLIDE: {
      ...
      desc << L"Delay: " << (int)pitchSlideDelay << L"  Length: "
           << (int)pitchSlideLength << L"  Note: "
    ...
    case EVENT_KONAMI_LOOP_END: {
      ...
      desc << L"Times: " << (int)times << L"  Volume Delta: "
           << (int)volumeDelta << L"  Pitch Delta: " << (int)pitchDelta
           << L"/16 semitones";
    ...
    case EVENT_KONAMI_ADSR_AND_GAIN: {
      ...
      desc << std::hex << std::setfill(L'0') << std::setw(2) << std::uppercase
           << L"ADSR(1): $" << adsr1 << L"  ADSR(2): $" << adsr2 << L"  GAIN: $"
           << gain;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR/GAIN",
    ...
    case EVENT_LEMMINGS_NOTE_PARAM: {
      ...
      if (curOffset + 1 < 0x10000 && GetByte(curOffset) <= 0x7f) {
        ...
        desc << L"  Quantize: " << durByte << L" (" << shared->spcNoteDurRate
             << L"/256)";
    ...
    case EVENT_INTELLI_LEGATO_ON: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Legato On",
                      desc.str(), CLR_PORTAMENTO, ICON_CONTROL);
      break;
    }

    case EVENT_INTELLI_LEGATO_OFF: {
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Legato Off",
                      desc.str(), CLR_PORTAMENTO, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_INTELLI_JUMP_SHORT_CONDITIONAL: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"Conditional Jump (Short)", desc.str().c_str(),
                      CLR_MISC);
    ...
    case EVENT_INTELLI_FE3_EVENT_F5: {
      ...
      if (param < 0xf0) {
        ...
        desc << L"Value: " << param;
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Wait for APU Port #2", desc.str(), CLR_CHANGESTATE);
      } else {
        ...
        switch (bit) {
          ...
          case 7:
            ...
            AddGenericEvent(beginOffset, curOffset - beginOffset,
                            L"Use Custom Note Param", desc.str(),
                            CLR_CHANGESTATE);
            break;
    ...
    case EVENT_INTELLI_WRITE_APU_PORT: {
      ...
      desc << L"Value: " << value;
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Write APU Port",
    ...
    case EVENT_INTELLI_DEFINE_VOICE_PARAM: {
      ...
      if (param >= 0) {
        ...
        AddGenericEvent(beginOffset, curOffset - beginOffset,
                        L"Voice Param Table", desc.str(), CLR_MISC);
      } else {
    ...
    case EVENT_INTELLI_LOAD_VOICE_PARAM: {
      ...
      if (paramIndex < parentSeq->intelliVoiceParamTableSize) {
        uint16_t addrVoiceParam =
            parentSeq->intelliVoiceParamTable + (paramIndex * 4);
        ...
        switch (parentSeq->version) {
          case NINSNES_INTELLI_FE3: {
            ...
            if (transposeIndex != 0) {
              const int8_t FE3_TRANSPOSE_TABLE[7] = {-24, -12, -1, 0,
                                                     1,   12,  24};
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"Load Voice Param",
                      desc.str(), CLR_PROGCHANGE, ICON_PROGCHANGE);
      break;
    }
    ...
    case EVENT_INTELLI_ADSR: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR", desc.str(),
    ...
    case EVENT_QUINTET_ADSR: {
      ...
      desc << L"ADSR(1): $" << std::hex << std::setfill(L'0') << std::setw(2)
           << std::uppercase << adsr1 << L"  Sustain Rate: " << std::dec
           << sustain_rate << L"  Sustain Level: " << sustain_level
           << L"  (ADSR(2): $" << std::hex << std::setfill(L'0') << std::setw(2)
           << std::uppercase << adsr2 << L")";
      AddGenericEvent(beginOffset, curOffset - beginOffset, L"ADSR", desc.str(),
                      CLR_ADSR, ICON_CONTROL);
      break;
    }
    ...
    case EVENT_INTELLI_GAIN_SUSTAIN_TIME_AND_RATE: {
      ...
      desc << L"Duration for Sustain: " << sustainDurRate << L"/256" << std::hex
           << std::setfill(L'0') << std::setw(2) << std::uppercase
           << L"  GAIN for Sustain: $" << sustainGAIN;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN Sustain Time/Rate", desc.str(), CLR_ADSR,
                      ICON_CONTROL);
      break;
    }
    ...
    case EVENT_INTELLI_GAIN_SUSTAIN_TIME: {
      ...
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN Sustain Time", desc.str(), CLR_ADSR, ICON_CONTROL);
      break;
    }

    case EVENT_INTELLI_GAIN: {
      // This event will update GAIN immediately,
      // however, note that Fire Emblem 4 does not switch to GAIN mode until
      // note off.
      uint8_t gain = GetByte(curOffset++);
      desc << std::hex << std::setfill(L'0') << std::setw(2) << std::uppercase
           << L"  GAIN: $" << gain;
      AddGenericEvent(beginOffset, curOffset - beginOffset,
                      L"GAIN (Release Rate)", desc.str(), CLR_ADSR,
                      ICON_CONTROL);
      break;
    }
      ...
      switch (type) {
        ...
        case 0x02: {
          ...
          if (param == 0x80) {
            ...
            desc << L"Value: $" << std::hex << std::setfill(L'0')
                 << std::setw(2) << std::uppercase << param;
            if (type == 0x01) {
              AddGenericEvent(beginOffset, curOffset - beginOffset,
                              L"Set Flags On", desc.str(), CLR_CHANGESTATE);
            } else {
              AddGenericEvent(beginOffset, curOffset - beginOffset,
                              L"Set Flags Off", desc.str(), CLR_CHANGESTATE);
            }
          }
...
void NinSnesTrack::GetVolumeBalance(uint16_t pan, double &volumeLeft,
                                    double &volumeRight) {
  NinSnesSeq *parentSeq = (NinSnesSeq *)this->parentSeq;
...
int8_t NinSnesTrack::CalcPanValue(uint8_t pan, double &volumeScale,
                                  bool &reverseLeft, bool &reverseRight) {
  NinSnesSeq *parentSeq = (NinSnesSeq *)this->parentSeq;
