...
class DLSFile : public RiffFile {
 public:
  ...
  DLSWave *AddWave(uint16_t formatTag, uint16_t channels, int samplesPerSec,
...
class ConnectionBlock {
 public:
  ...
  ConnectionBlock(uint16_t source, uint16_t control, uint16_t destination,
...
class DLSArt {
 public:
  ...
  void AddADSR(long attack_time, uint16_t atk_transform, long decay_time,
...
class DLSWsmp {
 public:
  DLSWsmp(void) {}
  DLSWsmp(uint16_t unityNote, int16_t fineTune, int32_t attenuation,
          char sampleLoops, uint32_t loopType, uint32_t loopStart,
          uint32_t loopLength)
...
class DLSWave {
 public:
  DLSWave(void) : Wsmp(NULL), data(NULL), name("Untitled Wave") {
    RiffFile::AlignName(name);
  }
  DLSWave(uint16_t formatTag, uint16_t channels, int samplesPerSec,
  ...
  //	This function will always return an even value, to maintain the
  //alignment
