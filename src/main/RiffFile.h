...
class Chunk {
 public:
  ...
  uint32_t
      size;  //  The size of the chunk data in bytes, excluding any pad byte.
  uint8_t *
      data;  //  The actual data not including a possible pad byte to word align
 ...
 public:
  ...
  virtual uint32_t GetSize();  //  Returns the size of the chunk in bytes,
                               //  including any pad byte.
...
//					  and the rest of the data is a collection of
//child chunks
////////////////////////////////////////////////////////////////////////////
class ListTypeChunk : public Chunk {
 public:
  char type[4];  // 4 byte sig that begins the data field, "LIST" or "sfbk" for
                 // ex
...
class RiffFile : public RIFFChunk {
 public:
  ...
  static void WriteLIST(std::vector<uint8_t> &buf, uint32_t listName,
                        uint32_t listSize) {
